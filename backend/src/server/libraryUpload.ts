...
export type LibraryMediaType='document'|'picture'|'video';
...
export function safeLibraryFilename(value:unknown,requiredExtension?:string){
  ...
  if(!libraryMimeTypes.has(extension))throw new Error('Supported files are PDF, Word, Excel, or TXT documents; JPG, JPEG, PNG, or WEBP pictures; and MP4 or WEBM videos.');
...
export async function validateStagedLibraryFile(input:{path:string;originalName:string;mimeType?:string;sizeBytes:number}):Promise<ValidatedLibraryFile>{
  ...
  const type=libraryMimeTypes.get(extension)!;
  const maxBytes=type.mediaType==='video'?LIBRARY_LIMITS_BYTES.videos:type.mediaType==='picture'?LIBRARY_LIMITS_BYTES.pictures:LIBRARY_LIMITS_BYTES.documents;
  const maxMb=type.mediaType==='video'?LIBRARY_LIMITS_MB.videos:type.mediaType==='picture'?LIBRARY_LIMITS_MB.pictures:LIBRARY_LIMITS_MB.documents;
  ...
  if(supplied&&supplied!=='application/octet-stream'&&supplied!==type.mimeType)throw new Error(`${displayFilename} has a mismatched content type.`);
  const handle=await fs.promises.open(input.path,'r');
  try{
    const header=Buffer.alloc(64);const {bytesRead}=await handle.read(header,0,header.length,0);const bytes=header.subarray(0,bytesRead);
    ...
    else if(extension==='.mp4')matches=bytes.length>=12&&bytes.subarray(4,8).toString('ascii')==='ftyp';
    else if(extension==='.webm')matches=bytes.length>=4&&bytes.subarray(0,4).equals(Buffer.from([0x1a,0x45,0xdf,0xa3]));
    else if(extension==='.txt'){
      matches=true;const chunk=Buffer.alloc(64*1024);let position=0;
      while(position<input.sizeBytes){const read=await handle.read(chunk,0,Math.min(chunk.length,input.sizeBytes-position),position);if(!read.bytesRead)break;if(chunk.subarray(0,read.bytesRead).includes(0)){matches=false;break;}position+=read.bytesRead;}
    }
    if(!matches)throw new Error(`${displayFilename} does not match its file type.`);
    return {displayFilename,extension,mimeType:type.mimeType,mediaType:type.mediaType};
