...
function buildPmImportStage(parsed:ParsedPmWorkbook,buffer:Buffer,filename:string,importedByUserId:number,options:{token?:string;previewedAt?:string;store?:boolean}={}):PmImportStage {
  ...
  for (const row of parsed.historyRows) {
    ...
    const taskExists=dbTasks.some(task=>!task.deleted&&task.asset_id===asset.id&&task.interval_type===row.intervalType&&normalizedPmKey(task.title)===normalizedPmKey(row.taskType))||trackerActions.some(action=>action.assetId===asset.id&&action.row.intervalType===row.intervalType&&normalizedPmKey(action.row.taskTitle)===normalizedPmKey(row.taskType));
  ...
  const replacementEligible=importableRows===0&&parsed.trackerRows.length>0&&!blockingTrackerConflict&&!rejectedRows.some(item=>item.sheet==='Machine Pm Tracker')&&[...trackerAssetResolutions.values()].every(resolution=>resolution.status==='resolved');
