...
const machineDocumentUpload = multer({ storage: multer.diskStorage({destination:(_req,_file,callback)=>callback(null,machineDocumentIncomingDir),filename:(_req,_file,callback)=>callback(null,`${crypto.randomUUID()}.upload`)}), limits: { files: 20, fileSize: LIBRARY_LIMITS_BYTES.videos } });
const equipmentDocumentUpload = multer({ storage: multer.diskStorage({destination:(_req,_file,callback)=>callback(null,equipmentDocumentIncomingDir),filename:(_req,_file,callback)=>callback(null,`${crypto.randomUUID()}.upload`)}), limits: { files: 20, fileSize: LIBRARY_LIMITS_BYTES.videos } });
    ...
    for (const includedFolder of masterBackupFolderCandidates) {
      ...
      const filter=includedFolder==='pm-work-orders'
        ...
        : includedFolder==='uploads'
          ? (source:string)=>path.basename(source)!=='.incoming'
          : undefined;
...
async function validatedMachineDocument(file: Express.Multer.File) {
  return validateStagedLibraryFile({path:file.path,originalName:file.originalname,mimeType:file.mimetype,sizeBytes:file.size});
}
async function receiveMachineDocuments(req: Request,res:Response,next:NextFunction) {
  ...
  machineDocumentUpload.array('documents',20)(req,res,error=>{
    if (!error) return next();
    const message=error instanceof multer.MulterError&&error.code==='LIMIT_FILE_SIZE'?`Videos must be ${LIBRARY_LIMITS_MB.videos} MB or smaller; documents and pictures must be ${LIBRARY_LIMITS_MB.documents} MB or smaller.`:safeErrorMessage(error,[],'File upload failed.');
    res.status(400).json({ok:false,error:message});
  });
}
...
app.get('/api/machine-library/assets/:assetId/document-folders', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  ...
  res.json({ok:true,assetId:asset.id,folders,summary:{folderCount:folders.length,documentCount},limits:{documentsMb:LIBRARY_LIMITS_MB.documents,picturesMb:LIBRARY_LIMITS_MB.pictures,videosMb:LIBRARY_LIMITS_MB.videos}});
});
