...
function applyPmAssetMeter(input:{assetId:number;library:AssetLibrary;meterType:PmMeterType;newReading:number;actor:User;source:string;assetLabel:string;override?:{type:string;reason:string}|null;timestamp?:string}){
  const timestamp=input.timestamp??now();const previous=pmAssetMeter(input.assetId,input.library,input.meterType);const previousReading=previous===undefined?null:Number(previous.current_reading);const override=input.override??pmMeterOverrideFromInput({},previousReading,input.newReading);
  if(previousReading!==null&&input.newReading<previousReading&&!override)throw new Error(`A decreasing meter reading requires an authorized correction. Stored reading: ${previousReading}.`);
  ...
  if(previousReading===null||previousReading!==input.newReading){const changeType=override?.type??'normal';const reason=override?.reason??'';run('INSERT INTO pm_meter_history (meter_id,asset_id,asset_library,meter_type,previous_reading,new_reading,change_type,reason,source,user_id,user_name,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',[meter.id,input.assetId,input.library,input.meterType,previousReading,input.newReading,changeType,reason,input.source,input.actor.id,input.actor.full_name,timestamp]);recordHistoryLog({section:'preventive_maintenance',action:override?'pm_machine_meter_override':'pm_machine_meter_updated',entityType:'pm_asset_meter',entityId:meter.id,entityLabel:`${input.assetLabel} ${input.meterType==='hours'?'Hour Meter':'Cycle Counter'}`,assetId:String(input.assetId),machineName:input.library==='machine'?input.assetLabel:'',equipmentName:input.library==='equipment'?input.assetLabel:'',oldValue:{reading:previousReading,meterType:input.meterType},newValue:{reading:input.newReading,meterType:input.meterType,source:input.source,overrideType:override?.type??null},reasonNote:reason,actor:input.actor});}
...
function buildPmImportStage(parsed:ParsedPmWorkbook,buffer:Buffer,filename:string,actor:User):PmImportStage {
  ...
  for(const group of sharedReadings.values())if(group.values.size>1)conflicts.push({sheet:'Machine Pm Tracker',assetNumber:group.assetNumber,code:'INCONSISTENT_SHARED_METER',message:`${group.assetNumber} has conflicting ${group.intervalType==='hourly'?'hour':'cycle'} readings on rows ${group.rows.join(', ')}. One physical machine meter must use one Current value.`});
...
async function retryPmWorkbookSync(actor:User) {
  ...
  return performPmWorkbookSync({actor,sourcePath,originalFilename:state?.original_filename||'PM_report.xlsx',trackerUpdates,trackerRemovals,historyRows});
}
...
app.post('/api/pm-excel/confirm',requireAuth,requirePermission('machine.pm_manage'),async(req:AuthRequest,res)=>{
  try {
    ...
    try {
      ...
      for(const action of importedMeterGroups.values()){const meterType:PmMeterType=action.row.intervalType==='hourly'?'hours':'cycles';const previous=pmAssetMeter(action.assetId,'machine',meterType);const reading=action.row.currentMeter!;const override=pmMeterOverrideFromInput(overrides.get(action.rowNumber)??{},previous?Number(previous.current_reading):null,reading);const asset=machineAssetById(action.assetId)!;const applied=applyPmAssetMeter({assetId:action.assetId,library:'machine',meterType,newReading:reading,actor:req.user!,source:'pm_excel_import',assetLabel:asset.asset_number,override,timestamp});for(const task of applied.tasks)if(!changedTaskIds.includes(task.id))changedTaskIds.push(task.id);}
    ...
    pmImportStages.delete(token);const trackerUpdates:TrackerWorkbookUpdate[]=[];for(const taskId of [...new Set(changedTaskIds)]){const task=pmTaskById(taskId);if(!task)continue;const asset=machineAssetById(task.asset_id);if(!asset)continue;const action=stage.trackerActions.find(item=>item.taskId===task.id||(item.assetId===task.asset_id&&item.row.intervalType===task.interval_type&&normalizedPmKey(item.row.taskTitle)===normalizedPmKey(task.title)));trackerUpdates.push(pmTrackerUpdate(task,asset.asset_number,action?.row.currentDate??undefined,undefined,asset.asset_name));}
