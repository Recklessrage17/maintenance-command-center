...
import { LIBRARY_LIMITS_BYTES, LIBRARY_LIMITS_MB, acquireLibraryUploadSlot, cleanupStagedFiles, cleanupStagingDirectory, libraryMimeTypes, promoteStagedFile, safeLibraryFilename, validateStagedLibraryFile } from './libraryUpload.js';
...
const machineDocumentTypes = new Map([...libraryMimeTypes].map(([extension,value])=>[extension,value.mimeType]));
...
function publicMachineDocument(row:MachineDocumentRow) {
  ...
  const mediaType=libraryMimeTypes.get(row.extension)?.mediaType??'document';
...
for(const mode of ['open','download'] as const)app.get(`/api/machine-library/assets/:assetId/documents/:documentId/${mode}`,requireAuth,requirePermission('machine.view'),(req:AuthRequest,res)=>{
  try{
    ...
    const mediaType=libraryMimeTypes.get(document.extension)?.mediaType??'document';streamLibraryFile(req,res,filePath,document.mime_type,document.display_filename,mode==='download'||(mediaType==='document'&&document.extension!=='.pdf'),mediaType==='video');
...
function publicEquipmentDocument(row:MachineDocumentRow){const base=`/api/equipment-library/assets/${row.asset_id}/documents/${row.id}`;const mediaType=libraryMimeTypes.get(row.extension)?.mediaType??'document';return {id:row.id,assetId:row.asset_id,folderId:row.folder_id,folderName:row.folder_name??'',folderPath:row.folder_path??row.folder_name??'',mediaType,originalFilename:row.original_filename,displayFilename:row.display_filename,extension:row.extension,mimeType:row.mime_type,sizeBytes:Number(row.size_bytes),description:row.description,revision:row.revision,uploadedAt:row.uploaded_at,updatedAt:row.updated_at,uploadedBy:row.uploaded_by_name??'Unknown user',openUrl:`${base}/open`,downloadUrl:`${base}/download`,canPrint:row.extension==='.pdf'||mediaType==='picture'};}
