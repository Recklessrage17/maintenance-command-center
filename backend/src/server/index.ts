...
function buildPmImportStage(parsed:ParsedPmWorkbook,buffer:Buffer,filename:string,importedByUserId:number,options:{token?:string;previewedAt?:string;store?:boolean}={}):PmImportStage {
  ...
  const dbTasks=all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=0 ORDER BY id");
...
async function retryPmWorkbookSync(actor:User,lockHeld=false) {
  ...
  const trackerRemovals:TrackerWorkbookRemoval[]=[];
  ...
  for(const task of all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=1 ORDER BY id")){const asset=machineAssetById(task.asset_id,true);if(!asset)continue;taskWorkbookAssets.set(task.id,asset.asset_number);trackerRemovals.push({assetNumber:asset.asset_number,taskTitle:task.title,intervalType:pmWorkbookInterval(task.interval_type)});}
    ...
    try {
      for(const action of stage.trackerActions){const row=action.row;const values=pmTaskValuesFromWorkbook(row);let task:PmTaskRow;
        if(action.kind==='addition'){
          ...
          const existing=pmTaskById(action.taskId!);if(!existing)throw new Error(`PM task for workbook row ${row.rowNumber} changed after preview.`);const oldValue=pmHistoryValue(existing);run('UPDATE pm_tasks SET title=?,interval_type=?,interval_value=?,last_completed_date=?,last_completed_meter=?,current_meter=?,next_due_date=?,next_due_meter=?,updated_by_user_id=?,updated_at=? WHERE id=? AND asset_library=\'machine\'',[values.title,values.intervalType,values.intervalValue,values.lastCompletedDate,values.lastCompletedMeter,values.currentMeter,values.nextDueDate,values.nextDueMeter,req.user!.id,timestamp,existing.id]);task=pmTaskById(existing.id)!;const asset=machineAssetById(task.asset_id)!;recordPmAudit({action:'pm_import_updated',task,asset,actor:req.user!,oldValue,newValue:{...pmHistoryValue(task),workbookRow:row.rowNumber,importedByUserId:stage.importedByUserId,confirmedByUserId:req.user!.id}});updated+=1;
        }
        changedTaskIds.push(task.id);
      }
      const importedMeterGroups=new Map<string,PmImportTrackerAction>();
      for(const action of stage.trackerActions)if((action.row.intervalType==='hourly'||action.row.intervalType==='cycles')&&action.row.currentMeter!==null)importedMeterGroups.set(`${action.assetId}:${action.row.intervalType}`,action);
      for(const action of importedMeterGroups.values()){const meterType:PmMeterType=action.row.intervalType==='hourly'?'hours':'cycles';const reading=action.row.currentMeter!;const asset=machineAssetById(action.assetId)!;const applied=applyPmAssetMeter({assetId:action.assetId,library:'machine',meterType,newReading:reading,actor:req.user!,source:'pm_excel_import',assetLabel:asset.asset_number,allowDecrease:true,timestamp});for(const task of applied.tasks)if(!changedTaskIds.includes(task.id))changedTaskIds.push(task.id);}
