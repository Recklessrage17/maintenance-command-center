...
async function retryPmWorkbookSync(actor:User,lockHeld=false) {
  ...
  for(const task of all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=1 ORDER BY id")){const asset=machineAssetById(task.asset_id,true);if(!asset)continue;taskWorkbookAssets.set(task.id,asset.asset_number);if(['hourly','cycles','days','annual'].includes(task.interval_type))trackerRemovals.push({assetNumber:asset.asset_number,taskTitle:task.title,intervalType:pmWorkbookInterval(task.interval_type)});}
  for (const [taskId,workbookAssetNumber] of taskWorkbookAssets) {const task=pmTaskById(taskId,'machine',true)!;if(!['hourly','cycles','days','annual'].includes(task.interval_type))continue;for(const history of all<PmHistoryRow>("SELECT * FROM pm_history WHERE pm_task_id=? AND asset_library='machine' ORDER BY id",[task.id])) historyRows.push(pmHistoryWorkbookRow(history,task,workbookAssetNumber));}
