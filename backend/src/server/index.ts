...
async function retryPmWorkbookSync(actor:User) {
  ...
  for(const task of all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' ORDER BY id")){const asset=machineAssetById(task.asset_id);if(!asset)throw new Error(`Retry cannot resolve the MCC machine asset for PM task ${task.title}.`);trackerUpdates.push(pmTrackerUpdate(task,asset.asset_number,undefined,undefined,asset.asset_name));taskWorkbookAssets.set(task.id,asset.asset_number);}
...
app.post('/api/pm-excel/confirm',requireAuth,requirePermission('machine.pm_manage'),async(req:AuthRequest,res)=>{
  try {
    ...
    pmImportStages.delete(token);const trackerUpdates:TrackerWorkbookUpdate[]=[];for(const action of stage.trackerActions){const task=all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=?",[action.assetId]).find(item=>item.interval_type===action.row.intervalType&&normalizedPmKey(item.title)===normalizedPmKey(action.row.taskTitle));const asset=machineAssetById(action.assetId);if(task)trackerUpdates.push(pmTrackerUpdate(task,action.row.assetNumber,action.row.currentDate??undefined,undefined,asset?.asset_name));}
