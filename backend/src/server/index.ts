...
const canonicalHttps = canonicalHttpsAccess(process.env.MCC_HTTPS_HOSTNAME, process.env.MCC_HTTPS_MODE);
...
const portableUploadMaxBytes = Number.isFinite(configuredPortableUploadMaxMb) && configuredPortableUploadMaxMb > 0 && configuredPortableUploadMaxMb <= 3584
...
const machineDocumentIncomingDir = path.join(machineDocumentLibraryDir, '.incoming');
...
fs.mkdirSync(machineDocumentIncomingDir, { recursive: true });
fs.mkdirSync(equipmentAssetNotesDir, { recursive: true });
fs.mkdirSync(equipmentDocumentLibraryDir, { recursive: true });
fs.mkdirSync(equipmentDocumentIncomingDir, { recursive: true });
cleanupStagingDirectory(machineDocumentIncomingDir);
cleanupStagingDirectory(equipmentDocumentIncomingDir);
...
CREATE TABLE IF NOT EXISTS machine_inspection_records (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_id INTEGER NOT NULL, original_filename TEXT NOT NULL, mime_type TEXT NOT NULL, file_size INTEGER NOT NULL, record_date TEXT NOT NULL, stored_file_reference TEXT NOT NULL, uploaded_at TEXT NOT NULL, uploaded_by_user_id INTEGER);
 CREATE TABLE IF NOT EXISTS machine_asset_notes (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_id INTEGER NOT NULL, title TEXT NOT NULL, note_date TEXT NOT NULL, body TEXT NOT NULL, is_warning INTEGER NOT NULL DEFAULT 0, work_order_reference TEXT NOT NULL DEFAULT '', issue_status TEXT NOT NULL DEFAULT 'active', resolved_at TEXT, resolved_by_user_id INTEGER, resolved_by_name TEXT NOT NULL DEFAULT '', resolution_summary TEXT NOT NULL DEFAULT '', reopened_at TEXT, reopened_by_user_id INTEGER, reopened_by_name TEXT NOT NULL DEFAULT '', deleted INTEGER NOT NULL DEFAULT 0, deleted_at TEXT, deleted_by_user_id INTEGER, deleted_by_name TEXT NOT NULL DEFAULT '', delete_reason TEXT NOT NULL DEFAULT '', pdf_filename TEXT NOT NULL DEFAULT '', pdf_stored_reference TEXT NOT NULL DEFAULT '', created_by_user_id INTEGER, updated_by_user_id INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
 CREATE TABLE IF NOT EXISTS machine_asset_note_attachments (id INTEGER PRIMARY KEY AUTOINCREMENT, note_id INTEGER NOT NULL, original_filename TEXT NOT NULL, mime_type TEXT NOT NULL, file_size INTEGER NOT NULL, stored_file_reference TEXT NOT NULL, uploaded_by_user_id INTEGER, created_at TEXT NOT NULL, source_type TEXT NOT NULL DEFAULT 'supporting', sha256 TEXT NOT NULL DEFAULT '');
CREATE TABLE IF NOT EXISTS machine_document_folders (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_id INTEGER NOT NULL, parent_id INTEGER, name TEXT NOT NULL COLLATE NOCASE, description TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by_user_id INTEGER, updated_by_user_id INTEGER, FOREIGN KEY(asset_id) REFERENCES machine_assets(id) ON DELETE RESTRICT, FOREIGN KEY(parent_id) REFERENCES machine_document_folders(id) ON DELETE RESTRICT);
...
CREATE TABLE IF NOT EXISTS equipment_assets (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_number TEXT NOT NULL UNIQUE COLLATE NOCASE, equipment_name TEXT NOT NULL DEFAULT '', category TEXT NOT NULL DEFAULT '', equipment_type TEXT NOT NULL DEFAULT '', manufacturer TEXT NOT NULL DEFAULT '', model TEXT NOT NULL DEFAULT '', serial_number TEXT NOT NULL DEFAULT '', equipment_year TEXT NOT NULL DEFAULT '', location TEXT NOT NULL DEFAULT '', department TEXT NOT NULL DEFAULT '', status TEXT NOT NULL DEFAULT 'active', criticality TEXT NOT NULL DEFAULT '', power_type TEXT NOT NULL DEFAULT '', voltage TEXT NOT NULL DEFAULT '', phase TEXT NOT NULL DEFAULT '', amperage TEXT NOT NULL DEFAULT '', air_requirement TEXT NOT NULL DEFAULT '', water_requirement TEXT NOT NULL DEFAULT '', capacity_rating TEXT NOT NULL DEFAULT '', dimensions TEXT NOT NULL DEFAULT '', weight TEXT NOT NULL DEFAULT '', specification_notes TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by_user_id INTEGER, updated_by_user_id INTEGER, deleted INTEGER NOT NULL DEFAULT 0, deleted_at TEXT, deleted_by_user_id INTEGER);
 CREATE TABLE IF NOT EXISTS equipment_asset_notes (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_id INTEGER NOT NULL, title TEXT NOT NULL, note_date TEXT NOT NULL, body TEXT NOT NULL, is_warning INTEGER NOT NULL DEFAULT 0, work_order_reference TEXT NOT NULL DEFAULT '', issue_status TEXT NOT NULL DEFAULT 'active', resolved_at TEXT, resolved_by_user_id INTEGER, resolved_by_name TEXT NOT NULL DEFAULT '', resolution_summary TEXT NOT NULL DEFAULT '', reopened_at TEXT, reopened_by_user_id INTEGER, reopened_by_name TEXT NOT NULL DEFAULT '', deleted INTEGER NOT NULL DEFAULT 0, deleted_at TEXT, deleted_by_user_id INTEGER, deleted_by_name TEXT NOT NULL DEFAULT '', delete_reason TEXT NOT NULL DEFAULT '', pdf_filename TEXT NOT NULL DEFAULT '', pdf_stored_reference TEXT NOT NULL DEFAULT '', created_by_user_id INTEGER, updated_by_user_id INTEGER, created_at TEXT NOT NULL, updated_at TEXT NOT NULL);
 CREATE TABLE IF NOT EXISTS equipment_asset_note_attachments (id INTEGER PRIMARY KEY AUTOINCREMENT, note_id INTEGER NOT NULL, original_filename TEXT NOT NULL, mime_type TEXT NOT NULL, file_size INTEGER NOT NULL, stored_file_reference TEXT NOT NULL, uploaded_by_user_id INTEGER, created_at TEXT NOT NULL, source_type TEXT NOT NULL DEFAULT 'supporting', sha256 TEXT NOT NULL DEFAULT '');
 CREATE TABLE IF NOT EXISTS asset_note_updates (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, body TEXT NOT NULL, created_by_user_id INTEGER, created_by_name TEXT NOT NULL, created_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')));
 CREATE TABLE IF NOT EXISTS asset_note_update_attachments (id INTEGER PRIMARY KEY AUTOINCREMENT, update_id INTEGER NOT NULL, original_filename TEXT NOT NULL, mime_type TEXT NOT NULL, file_size INTEGER NOT NULL, stored_file_reference TEXT NOT NULL, uploaded_by_user_id INTEGER, uploaded_by_name TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY(update_id) REFERENCES asset_note_updates(id) ON DELETE RESTRICT);
 CREATE TABLE IF NOT EXISTS asset_note_lifecycle_events (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, event_type TEXT NOT NULL, actor_user_id INTEGER, actor_name TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '', old_value_json TEXT NOT NULL DEFAULT '{}', new_value_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')));
 CREATE TABLE IF NOT EXISTS asset_note_labor (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, user_id INTEGER, display_name TEXT NOT NULL, labor_hours REAL NOT NULL DEFAULT 0, is_primary INTEGER NOT NULL DEFAULT 0, technician_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE RESTRICT, UNIQUE(asset_library,note_id,technician_order));
...
CREATE INDEX IF NOT EXISTS idx_pm_history_asset ON pm_history (asset_id,completion_date DESC,id DESC);`);
  ...
  const pmTaskArchiveMigrations: Array<{name:string;definition:string}> = [
    ...
    {name:'deletion_reason',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'deleted_task_snapshot_json',definition:"TEXT NOT NULL DEFAULT ''"},
  ];
  for (const column of pmTaskArchiveMigrations) if (!pmTaskColumns.has(column.name)) run(`ALTER TABLE pm_tasks ADD COLUMN ${column.name} ${column.definition}`);
  ...
  const assetNoteLifecycleColumns: Array<{name:string;definition:string}> = [
    {name:'is_on_hold',definition:'INTEGER NOT NULL DEFAULT 0'},
    {name:'work_order_reference',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'issue_status',definition:"TEXT NOT NULL DEFAULT 'active'"},
    {name:'resolved_at',definition:'TEXT'},
    {name:'resolved_by_user_id',definition:'INTEGER'},
    {name:'resolved_by_name',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'resolution_summary',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'reopened_at',definition:'TEXT'},
    {name:'reopened_by_user_id',definition:'INTEGER'},
    {name:'reopened_by_name',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'deleted',definition:'INTEGER NOT NULL DEFAULT 0'},
    {name:'deleted_at',definition:'TEXT'},
    {name:'deleted_by_user_id',definition:'INTEGER'},
    {name:'deleted_by_name',definition:"TEXT NOT NULL DEFAULT ''"},
    {name:'delete_reason',definition:"TEXT NOT NULL DEFAULT ''"},
  ];
  for (const column of assetNoteLifecycleColumns) {
    if (!machineAssetNoteColumns.has(column.name)) run(`ALTER TABLE machine_asset_notes ADD COLUMN ${column.name} ${column.definition}`);
    if (!equipmentAssetNoteColumns.has(column.name)) run(`ALTER TABLE equipment_asset_notes ADD COLUMN ${column.name} ${column.definition}`);
  }
  const attachmentMetadataColumns: Array<{name:string;definition:string}> = [
    {name:'source_type',definition:"TEXT NOT NULL DEFAULT 'supporting'"},
    {name:'sha256',definition:"TEXT NOT NULL DEFAULT ''"},
  ];
  for (const table of ['machine_asset_note_attachments','equipment_asset_note_attachments']) {
    const columns=new Set(all<{name:string}>(`PRAGMA table_info(${table})`).map(column=>column.name));
    for(const column of attachmentMetadataColumns)if(!columns.has(column.name))run(`ALTER TABLE ${table} ADD COLUMN ${column.name} ${column.definition}`);
    run(`UPDATE ${table} SET source_type='work_order_photo' WHERE source_type='supporting' AND sha256='' AND lower(mime_type)='application/pdf' AND lower(original_filename) LIKE '%\\_photos.pdf' ESCAPE '\\'`);
  }
  db.exec(`CREATE TABLE IF NOT EXISTS asset_note_updates (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, body TEXT NOT NULL, created_by_user_id INTEGER, created_by_name TEXT NOT NULL, created_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')));
 CREATE TABLE IF NOT EXISTS asset_note_update_attachments (id INTEGER PRIMARY KEY AUTOINCREMENT, update_id INTEGER NOT NULL, original_filename TEXT NOT NULL, mime_type TEXT NOT NULL, file_size INTEGER NOT NULL, stored_file_reference TEXT NOT NULL, uploaded_by_user_id INTEGER, uploaded_by_name TEXT NOT NULL, created_at TEXT NOT NULL, FOREIGN KEY(update_id) REFERENCES asset_note_updates(id) ON DELETE RESTRICT);
 CREATE TABLE IF NOT EXISTS asset_note_lifecycle_events (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, event_type TEXT NOT NULL, actor_user_id INTEGER, actor_name TEXT NOT NULL, reason TEXT NOT NULL DEFAULT '', old_value_json TEXT NOT NULL DEFAULT '{}', new_value_json TEXT NOT NULL DEFAULT '{}', created_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')));
 CREATE TABLE IF NOT EXISTS asset_note_labor (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_library TEXT NOT NULL, note_id INTEGER NOT NULL, user_id INTEGER, display_name TEXT NOT NULL, labor_hours REAL NOT NULL DEFAULT 0, is_primary INTEGER NOT NULL DEFAULT 0, technician_order INTEGER NOT NULL DEFAULT 0, created_at TEXT NOT NULL, updated_at TEXT NOT NULL, CHECK(asset_library IN ('machine','equipment')), FOREIGN KEY(user_id) REFERENCES users(id) ON DELETE RESTRICT, UNIQUE(asset_library,note_id,technician_order));
 DROP INDEX IF EXISTS idx_machine_asset_notes_warning;
 DROP INDEX IF EXISTS idx_equipment_asset_notes_warning;
 CREATE INDEX IF NOT EXISTS idx_machine_asset_notes_warning ON machine_asset_notes (is_warning,issue_status,deleted,asset_id,note_date DESC,created_at DESC);
 CREATE INDEX IF NOT EXISTS idx_equipment_asset_notes_warning ON equipment_asset_notes (is_warning,issue_status,deleted,asset_id,note_date DESC,created_at DESC);
 CREATE INDEX IF NOT EXISTS idx_asset_note_updates_note ON asset_note_updates (asset_library,note_id,created_at,id);
...
CREATE INDEX IF NOT EXISTS idx_pm_history_participants_history ON pm_history_participants (pm_history_id,participant_order);
CREATE INDEX IF NOT EXISTS idx_pm_asset_meters_asset ON pm_asset_meters (asset_library,asset_id,meter_type);
CREATE INDEX IF NOT EXISTS idx_pm_meter_history_asset ON pm_meter_history (asset_library,asset_id,meter_type,created_at DESC,id DESC);
CREATE INDEX IF NOT EXISTS idx_pm_tasks_live_asset ON pm_tasks (asset_library,asset_id,deleted,active,updated_at DESC);`);
  const meterMigrationTimestamp=now();
  run(`INSERT OR IGNORE INTO pm_asset_meters (asset_id,asset_library,meter_type,current_reading,updated_by_user_id,created_at,updated_at)
    SELECT asset_id,asset_library,CASE interval_type WHEN 'hourly' THEN 'hours' ELSE 'cycles' END,MAX(current_meter),NULL,?,?
    FROM pm_tasks WHERE interval_type IN ('hourly','cycles') AND current_meter IS NOT NULL AND deleted=0 GROUP BY asset_library,asset_id,interval_type`,[meterMigrationTimestamp,meterMigrationTimestamp]);
  run(`INSERT INTO pm_meter_history (meter_id,asset_id,asset_library,meter_type,previous_reading,new_reading,change_type,reason,source,user_id,user_name,created_at)
    SELECT meter.id,meter.asset_id,meter.asset_library,meter.meter_type,NULL,meter.current_reading,'migration','Established from the highest existing schedule reading during the v1.5.3 additive migration.','migration',NULL,'System migration',?
    FROM pm_asset_meters meter WHERE NOT EXISTS (SELECT 1 FROM pm_meter_history history WHERE history.meter_id=meter.id AND history.source='migration')`,[meterMigrationTimestamp]);
  ...
  for(const hierarchy of [
    {folderTable:'machine_document_folders',documentTable:'machine_documents',assetTable:'machine_assets',index:'idx_machine_document_folders_parent'},
    {folderTable:'equipment_document_folders',documentTable:'equipment_documents',assetTable:'equipment_assets',index:'idx_equipment_document_folders_parent'},
  ]){
    const columns=new Set(all<{name:string}>(`PRAGMA table_info(${hierarchy.folderTable})`).map(column=>column.name));
    if(!columns.has('parent_id')){
      db.exec(`PRAGMA foreign_keys=OFF; PRAGMA legacy_alter_table=ON; BEGIN IMMEDIATE;
        ALTER TABLE ${hierarchy.folderTable} RENAME TO ${hierarchy.folderTable}_issue111_legacy;
        CREATE TABLE ${hierarchy.folderTable} (id INTEGER PRIMARY KEY AUTOINCREMENT, asset_id INTEGER NOT NULL, parent_id INTEGER, name TEXT NOT NULL COLLATE NOCASE, description TEXT NOT NULL DEFAULT '', created_at TEXT NOT NULL, updated_at TEXT NOT NULL, created_by_user_id INTEGER, updated_by_user_id INTEGER, FOREIGN KEY(asset_id) REFERENCES ${hierarchy.assetTable}(id) ON DELETE RESTRICT, FOREIGN KEY(parent_id) REFERENCES ${hierarchy.folderTable}(id) ON DELETE RESTRICT);
        INSERT INTO ${hierarchy.folderTable} (id,asset_id,parent_id,name,description,created_at,updated_at,created_by_user_id,updated_by_user_id) SELECT id,asset_id,NULL,name,description,created_at,updated_at,created_by_user_id,updated_by_user_id FROM ${hierarchy.folderTable}_issue111_legacy;
        DROP TABLE ${hierarchy.folderTable}_issue111_legacy;
        COMMIT; PRAGMA legacy_alter_table=OFF; PRAGMA foreign_keys=ON;`);
    }
    db.exec(`CREATE INDEX IF NOT EXISTS ${hierarchy.index} ON ${hierarchy.folderTable} (asset_id,parent_id,name COLLATE NOCASE);
      CREATE UNIQUE INDEX IF NOT EXISTS ${hierarchy.index}_unique_name ON ${hierarchy.folderTable} (asset_id,IFNULL(parent_id,0),name COLLATE NOCASE);`);
  }
  const hierarchyForeignKeyErrors=all<Record<string,unknown>>('PRAGMA foreign_key_check');
  if(hierarchyForeignKeyErrors.length)throw new Error('Asset library folder hierarchy migration failed foreign-key validation.');
...
const backupDataAreaDefinitions = [
  ...
  { key: 'preventiveMaintenanceRecords', label: 'PM', tables: ['pm_tasks','pm_history','pm_history_participants','pm_work_order_attachments','pm_asset_meters','pm_meter_history','preventive_maintenance'] },
...
function masterBackupRecordCounts() {
  return {
    ...
    preventiveMaintenanceRecords: tableGroupCount(['pm_tasks','pm_history','pm_history_participants','pm_work_order_attachments','pm_asset_meters','pm_meter_history','preventive_maintenance']),
  };
}
  ...
  try {
    ...
    for (const includedFolder of masterBackupFolderCandidates) {
      ...
      const filter=includedFolder==='pm-work-orders'
        ...
        : includedFolder==='uploads'
          ? (source:string)=>!['.incoming','.resumable'].includes(path.basename(source))
          : undefined;
    ...
    if (portable && portableIdentity) {
      ...
      try {
        ...
        if(archiveResult.sizeBytes>portableUploadMaxBytes){const oversizedPath=archiveResult.archivePath;archiveResult=null;fs.rmSync(oversizedPath,{force:true});throw new Error(`archive exceeds the ${Math.floor(portableUploadMaxBytes/1024/1024)} MB verified non-ZIP64 recovery ceiling`);}
...
function masterBackupStatus(actor?: User) {
  ...
  return {
    ...
    portableArchiveMaxBytes: portableUploadMaxBytes,
...
function importRowFromRecord(record: Record<string, string>, rowNumber: number): NativeImportRow {
  const has = (...headers: string[]) => headers.some(header => record[header] !== undefined || record[normalizeImportHeader(header)] !== undefined);
...
function obsoleteImportValue(value: string | null, rowNumber: number) {
  ...
  const normalized = value.trim().toLowerCase();
  if (!normalized || ['false','no','n','0','active','current'].includes(normalized)) return false;
  if (['true','yes','y','1','obsolete','discontinued'].includes(normalized)) return true;
  throw new Error(`Row ${rowNumber}: Obsolete must be Yes or No.`);
}
...
const resetTableAllowlists: Record<'machine_library' | 'equipment_library' | 'facility_info' | 'preventive_maintenance', string[]> = {
  ...
  preventive_maintenance: ['pm_history_participants','pm_work_order_attachments','pm_meter_history','pm_asset_meters','pm_history','pm_tasks','preventive_maintenance'],
};
...
type MachineDocumentFolderRow = { id:number; asset_id:number; parent_id:number|null; name:string; description:string; created_at:string; updated_at:string; created_by_user_id:number|null; updated_by_user_id:number|null; document_count?:number; child_count?:number; library_updated_at?:string; folder_path?:string };
...
type PmAssetMeterRow = { id:number; asset_id:number; asset_library:AssetLibrary; meter_type:PmMeterType; current_reading:number; updated_by_user_id:number|null; created_at:string; updated_at:string };
type PmMeterHistoryRow = { id:number; meter_id:number; asset_id:number; asset_library:AssetLibrary; meter_type:PmMeterType; previous_reading:number|null; new_reading:number; change_type:string; reason:string; source:string; user_id:number|null; user_name:string; created_at:string };
...
function machineComponentImageType(value: unknown) {
  const clean = String(value ?? '').trim().toLowerCase() as MachineComponentImageType;
  if (!machineComponentImageTypes.includes(clean)) throw new Error('Component image type is invalid.');
  return clean;
}
function machineComponentImageByAsset(assetId: number, componentType: MachineComponentImageType) {
  return one<MachineComponentImageRow>('SELECT * FROM machine_component_images WHERE asset_id=? AND component_type=?',[assetId,componentType]);
}
function publicMachineComponentImage(row: MachineComponentImageRow) {
  const baseUrl = `/api/machine-library/assets/${row.asset_id}/component-images/${encodeURIComponent(row.component_type)}/image`;
  const version = encodeURIComponent(row.uploaded_at);
  return { id:row.id,assetId:row.asset_id,componentType:row.component_type,filename:row.original_filename,mimeType:row.mime_type,fileSize:Number(row.file_size),uploadedAt:row.uploaded_at,contentUrl:`${baseUrl}?v=${version}`,downloadUrl:`${baseUrl}?download=true&v=${version}` };
}
function machineComponentImageFilePath(storedReference: string) {
  const relative = storedReference.replace(/\\/g,'/');
  if (!relative.startsWith('uploads/machine-component-images/')) throw new Error('Component image file reference is invalid.');
  const resolved = path.resolve(__dirname,'../../',relative);
  const root = path.resolve(machineComponentImagesDir);
  if (!resolved.startsWith(`${root}${path.sep}`)) throw new Error('Component image file reference is invalid.');
  return resolved;
}
function validatedMachineComponentImage(file: Express.Multer.File) {
  const extension = path.extname(file.originalname).toLowerCase();
  const allowedExtensions = new Set(['.jpg','.jpeg','.png','.webp']);
  if (!allowedExtensions.has(extension)) throw new Error('Component image must be JPG, JPEG, PNG, or WEBP.');
  const bytes = file.buffer;
  const detected = bytes.length >= 12 && bytes[0]===0x89 && bytes[1]===0x50 && bytes[2]===0x4e && bytes[3]===0x47
    ...
    : bytes.length >= 3 && bytes[0]===0xff && bytes[1]===0xd8 && bytes[2]===0xff
      ...
      : bytes.length >= 12 && bytes.subarray(0,4).toString('ascii')==='RIFF' && bytes.subarray(8,12).toString('ascii')==='WEBP'
        ...
        : null;
  if (!detected || !['image/jpeg','image/png','image/webp','image/jpg'].includes(file.mimetype.toLowerCase())) throw new Error('Component image must be a valid JPG, JPEG, PNG, or WEBP file.');
  if ((extension==='.png' && detected.mimeType!=='image/png') || (extension==='.webp' && detected.mimeType!=='image/webp') || (['.jpg','.jpeg'].includes(extension) && detected.mimeType!=='image/jpeg')) throw new Error('Component image file extension does not match its contents.');
  return detected;
}
function safeMachineComponentOriginalName(value: string, extension: string) {
  const base = path.basename(value,path.extname(value)).replace(/[\r\n"]/g,'').trim().slice(0,140) || 'component-image';
  return `${base}${extension}`;
}
function receiveMachineComponentImage(req: Request,res:Response,next:NextFunction) {
...
const machineAssetNoteAttachmentTypes = new Map([
  ['.pdf','application/pdf'],['.doc','application/msword'],['.docx','application/vnd.openxmlformats-officedocument.wordprocessingml.document'],
  ['.jpg','image/jpeg'],['.jpeg','image/jpeg'],['.png','image/png'],['.webp','image/webp'],
]);
function validMachineAssetNoteDate(value: unknown) {
  const clean = String(value ?? '').trim();
  if (!/^\d{4}-\d{2}-\d{2}$/.test(clean)) throw new Error('Enter a valid note date.');
  const parsed = new Date(`${clean}T12:00:00Z`);
...
function validatedMachineAssetNoteAttachment(file: Express.Multer.File) {
  ...
  if (!mimeType) throw new Error('Attachments must be PDF, Word, JPG, JPEG, PNG, or WEBP files.');
  const bytes = file.buffer;
  const matches = extension==='.pdf' ? bytes.subarray(0,5).toString('ascii')==='%PDF-'
    : extension==='.png' ? bytes.length>=4 && bytes[0]===0x89 && bytes[1]===0x50 && bytes[2]===0x4e && bytes[3]===0x47
      : ['.jpg','.jpeg'].includes(extension) ? bytes.length>=3 && bytes[0]===0xff && bytes[1]===0xd8 && bytes[2]===0xff
        : extension==='.webp' ? bytes.length>=12 && bytes.subarray(0,4).toString('ascii')==='RIFF' && bytes.subarray(8,12).toString('ascii')==='WEBP'
          : extension==='.docx' ? bytes.length>=4 && bytes[0]===0x50 && bytes[1]===0x4b && bytes[2]===0x03 && bytes[3]===0x04
            : bytes.length>=8 && bytes.subarray(0,8).equals(Buffer.from([0xd0,0xcf,0x11,0xe0,0xa1,0xb1,0x1a,0xe1]));
  if (!matches) throw new Error(`${path.basename(file.originalname)} does not match its file type.`);
  return { extension, mimeType };
}
function safeMachineAssetNoteOriginalName(value: string, extension: string) {
  const base = path.basename(value,path.extname(value)).replace(/[\r\n"]/g,'').trim().slice(0,140) || 'attachment';
  return `${base}${extension}`;
}
...
function machineAssetNoteById(noteId: number, includeDeleted=false) {
  return one<MachineAssetNoteRow>(`SELECT n.*,a.asset_number,a.asset_name,a.brand,a.model,a.serial_number,a.location,'' AS category,'machine' AS asset_library,COALESCE(u.full_name,'Unknown user') AS created_by_name
    ...
    WHERE n.id=? AND a.deleted=0 AND (?=1 OR n.deleted=0)`,[noteId,includeDeleted?1:0]);
}
function machineAssetNoteAttachments(noteId: number) {
  return all<MachineAssetNoteAttachmentRow>('SELECT * FROM machine_asset_note_attachments WHERE note_id=? ORDER BY created_at,id',[noteId]);
}
function assetNoteAttachmentSource(row:MachineAssetNoteAttachmentRow) {
  return row.source_type==='work_order_photo'?'work_order_photo':'supporting';
}
function ensureAssetNoteAttachmentHash(library:AssetLibrary,row:MachineAssetNoteAttachmentRow) {
  if(/^[a-f0-9]{64}$/i.test(row.sha256??''))return row.sha256.toLowerCase();
  try{
    const filePath=assetNoteStoredFilePath(library,row.stored_file_reference);const value=crypto.createHash('sha256').update(fs.readFileSync(filePath)).digest('hex');
    run(`UPDATE ${library==='machine'?'machine_asset_note_attachments':'equipment_asset_note_attachments'} SET sha256=? WHERE id=?`,[value,row.id]);row.sha256=value;return value;
  }catch{return '';}
}
function publicAssetNoteAttachment(library:AssetLibrary,row:MachineAssetNoteAttachmentRow) {
  const baseUrl = `/api/${library}-library/asset-note-attachments/${row.id}/file`;
  const version = encodeURIComponent(row.created_at);
  return { id:row.id,noteId:row.note_id,filename:row.original_filename,mimeType:row.mime_type,fileSize:Number(row.file_size),createdAt:row.created_at,sourceType:assetNoteAttachmentSource(row),sha256:ensureAssetNoteAttachmentHash(library,row),contentUrl:`${baseUrl}?v=${version}`,downloadUrl:`${baseUrl}?download=true&v=${version}` };
}
function publicMachineAssetNoteAttachment(row: MachineAssetNoteAttachmentRow) {
  return publicAssetNoteAttachment('machine',row);
}
function assetNoteManagePermission(library:AssetLibrary):PermissionKey { return `${library}.notes_manage` as PermissionKey; }
function assetNoteViewPermission(library:AssetLibrary):PermissionKey { return `${library}.view` as PermissionKey; }
function isWarningIssueManager(actor:User) { return Boolean(actor.is_owner_admin)||actor.role==='Manager'||actor.role==='Admin'; }
function canEditAssetNoteIssue(actor:User,library:AssetLibrary,note:MachineAssetNoteRow) {
  if (!hasPermission(actor,assetNoteManagePermission(library))) return false;
  return !note.is_warning||isWarningIssueManager(actor)||note.created_by_user_id===actor.id;
}
function canDeleteAssetNoteIssue(actor:User,library:AssetLibrary,note:MachineAssetNoteRow) {
  if (!hasPermission(actor,assetNoteManagePermission(library))) return false;
  return !note.is_warning||isWarningIssueManager(actor);
}
...
function validateAssetNoteWorkOrder(value:unknown,warning:boolean) {
  ...
  if (!warning&&clean) throw new Error('Work Order is available only for Warning / Needs Attention issues.');
  return warning?clean:'';
}
function requiredAssetNoteReason(value:unknown,label:string,maxLength=4000) {
  const clean=String(value??'').replace(/\r/g,'').trim().slice(0,maxLength);
  if (!clean) throw new Error(`${label} is required.`);
  return clean;
}
function assetNoteByLibraryId(library:AssetLibrary,noteId:number,includeDeleted=false) {
  return library==='machine'?machineAssetNoteById(noteId,includeDeleted):equipmentAssetNoteById(noteId,includeDeleted);
}
function assetNoteAttachments(library:AssetLibrary,noteId:number) {
  return library==='machine'?machineAssetNoteAttachments(noteId):equipmentAssetNoteAttachments(noteId);
}
function assetNoteUpdates(library:AssetLibrary,noteId:number) {
  return all<AssetNoteUpdateRow>('SELECT * FROM asset_note_updates WHERE asset_library=? AND note_id=? ORDER BY created_at,id',[library,noteId]);
}
function assetNoteUpdateAttachments(updateId:number) {
  return all<AssetNoteUpdateAttachmentRow>('SELECT * FROM asset_note_update_attachments WHERE update_id=? ORDER BY created_at,id',[updateId]);
}
function assetNoteLifecycleEvents(library:AssetLibrary,noteId:number) {
  return all<AssetNoteLifecycleEventRow>('SELECT * FROM asset_note_lifecycle_events WHERE asset_library=? AND note_id=? ORDER BY created_at,id',[library,noteId]);
}
function assetNoteLabor(library:AssetLibrary,noteId:number) {
...
function parseAssetNoteLabor(value:unknown,actor:User,required:boolean):AssetNoteLaborInput[]|null {
  ...
  let parsed:unknown;try{parsed=typeof value==='string'?JSON.parse(value):value;}catch{throw new Error('Labor entries must be valid JSON.');}
  if(!Array.isArray(parsed)||!parsed.length||parsed.length>20)throw new Error('Select a primary technician and no more than 20 technicians.');
  const seen=new Set<number>();let primaryCount=0;
  return parsed.map((raw,index)=>{
    if(!isRecord(raw))throw new Error('Each labor entry must identify an MCC technician.');
    const userId=Number(raw.userId);if(!Number.isInteger(userId)||userId<=0||seen.has(userId))throw new Error('Each labor technician must be a unique MCC user.');seen.add(userId);
    const user=one<User>('SELECT * FROM users WHERE id=? AND disabled=0 AND deleted=0',[userId]);if(!user)throw new Error('A selected labor technician is unavailable.');
    const hours=Number(raw.hours??0);if(!Number.isFinite(hours)||hours<0||hours>10000)throw new Error('Labor hours must be between 0 and 10,000.');
    const isPrimary=Boolean(raw.isPrimary);if(isPrimary)primaryCount+=1;
    return {userId:user.id,displayName:user.full_name,hours:Math.round(hours*100)/100,isPrimary,order:index};
  }).map((entry,index,entries)=>{if(index===entries.length-1&&primaryCount!==1)throw new Error('Exactly one Primary Technician is required.');return entry;});
}
function replaceAssetNoteLabor(library:AssetLibrary,noteId:number,entries:AssetNoteLaborInput[],timestamp:string) {
  run('DELETE FROM asset_note_labor WHERE asset_library=? AND note_id=?',[library,noteId]);
  for(const entry of entries)run('INSERT INTO asset_note_labor (asset_library,note_id,user_id,display_name,labor_hours,is_primary,technician_order,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?)',[library,noteId,entry.userId,entry.displayName,entry.hours,entry.isPrimary?1:0,entry.order,timestamp,timestamp]);
}
function assetNoteAttachmentSources(value:unknown,count:number) {
  let parsed:unknown=[];try{parsed=typeof value==='string'?JSON.parse(value):value;}catch{throw new Error('Attachment source metadata is invalid.');}
  const values=Array.isArray(parsed)?parsed:[];
  return Array.from({length:count},(_,index)=>values[index]==='work_order_photo'?'work_order_photo':'supporting');
}
function validatedAssetNoteAttachment(file:Express.Multer.File,sourceType:'supporting'|'work_order_photo') {
  const detected=validatedMachineAssetNoteAttachment(file);
  if(sourceType==='work_order_photo'&&detected.mimeType!=='application/pdf')throw new Error('Photographed work-order pages must be supplied as a PDF.');
  return detected;
}
function assetNoteFileHash(file:Express.Multer.File){return crypto.createHash('sha256').update(file.buffer).digest('hex');}
function assetNoteAttachmentExists(library:AssetLibrary,noteId:number,sha256:string){return assetNoteAttachments(library,noteId).some(row=>ensureAssetNoteAttachmentHash(library,row)===sha256);}
...
function publicAssetNoteLifecycleEvent(row:AssetNoteLifecycleEventRow) {
  return {id:row.id,type:row.event_type,actorUserId:row.actor_user_id,actor:row.actor_name,reason:row.reason,oldValue:safeJsonRecord(row.old_value_json),newValue:safeJsonRecord(row.new_value_json),createdAt:row.created_at};
}
function assetNoteHistorySnapshot(note:MachineAssetNoteRow) {
  const library=note.asset_library==='equipment'?'equipment':'machine';
  return {id:note.id,assetId:note.asset_id,title:note.title,noteDate:note.note_date,body:note.body,warning:Boolean(note.is_warning),hold:Boolean(note.is_warning&&note.is_on_hold),workOrder:note.work_order_reference,status:note.issue_status,resolvedAt:note.resolved_at,resolvedByUserId:note.resolved_by_user_id,resolutionSummary:note.resolution_summary,labor:publicAssetNoteLabor(library,note.id),updatedAt:note.updated_at};
}
function recordAssetNoteLifecycleEvent(input:{library:AssetLibrary;noteId:number;type:string;actor:User;reason?:string;oldValue?:Record<string,unknown>;newValue?:Record<string,unknown>;createdAt?:string}) {
  run('INSERT INTO asset_note_lifecycle_events (asset_library,note_id,event_type,actor_user_id,actor_name,reason,old_value_json,new_value_json,created_at) VALUES (?,?,?,?,?,?,?,?,?)',[input.library,input.noteId,input.type,input.actor.id,input.actor.full_name,input.reason??'',JSON.stringify(input.oldValue??{}),JSON.stringify(input.newValue??{}),input.createdAt??now()]);
}
function recordAssetNoteHoldChange(library:AssetLibrary,noteId:number,previous:boolean,hold:boolean,actor:User,createdAt:string) {
  if(previous===hold)return;
  recordAssetNoteLifecycleEvent({library,noteId,type:hold?'hold_enabled':'hold_disabled',actor,oldValue:{hold:previous},newValue:{hold},createdAt});
}
function assetNoteOriginalAttachments(library:AssetLibrary,noteId:number) {
...
function safeMachineAssetNotePdfText(value: unknown) {
  return String(value ?? '')
    .replace(/[\u2018\u2019]/g,"'").replace(/[\u201c\u201d]/g,'"').replace(/[\u2013\u2014]/g,'-').replace(/\u2026/g,'...').replace(/\u2022/g,'-')
    .normalize('NFKD').replace(/[\u0300-\u036f]/g,'').replace(/[^\x09\x0a\x0d\x20-\x7e]/g,'?');
}
function notePdfLines(text: string, font: PDFFont, size: number, maxWidth: number) {
  const lines: string[] = [];
  for (const paragraph of safeMachineAssetNotePdfText(text).replace(/\r/g,'').split('\n')) {
    const rawWords = paragraph.trim().split(/\s+/).filter(Boolean);
    const words:string[]=[];
    for (const rawWord of rawWords) {
      if (font.widthOfTextAtSize(rawWord,size)<=maxWidth) { words.push(rawWord); continue; }
      let segment='';
      for (const character of rawWord) {
        if (segment&&font.widthOfTextAtSize(`${segment}${character}`,size)>maxWidth) { words.push(segment); segment=character; }
...
async function buildMachineAssetNotePdf(note: MachineAssetNoteRow, attachments: MachineAssetNoteAttachmentRow[]) {
  const pdf = await PDFDocument.create();
  const regular = await pdf.embedFont(StandardFonts.Helvetica);
  const bold = await pdf.embedFont(StandardFonts.HelveticaBold);
  ...
  const metadata:string[][] = [
    ...
    ['Status',!note.is_warning?'Standard Record':note.issue_status==='resolved'?'Resolved':note.is_on_hold?'On Hold':'Tech Open'],['Location',note.location || '-'],['Opened / Note Date',note.note_date],['Created',new Date(note.created_at).toLocaleString('en-US')],
  ];
  ...
  for (const line of bodyLines) {
    ensureSpace(16);
    if (line) page.drawText(line,{x:margin,y,size:10.5,font:regular,color:ink});
    y-=15;
  }
  if(note.is_warning){
    const library=note.asset_library==='equipment'?'equipment':'machine';
  ...
  for(const attachment of workOrderPhotoPdfs){
    const identity=ensureAssetNoteAttachmentHash(library,attachment)||`id:${attachment.id}`;if(embedded.has(identity))continue;embedded.add(identity);
    try{const source=await PDFDocument.load(fs.readFileSync(assetNoteStoredFilePath(library,attachment.stored_file_reference)));const sourcePages=await pdf.copyPages(source,source.getPageIndices());for(const sourcePage of sourcePages)pdf.addPage(sourcePage);}catch{/* Preserve the maintenance record even if a legacy captured PDF is unreadable. */}
  }
  const pages=pdf.getPages();
  summaryPages.forEach((item,index)=>{
...
async function regenerateMachineAssetNotePdf(noteId:number) {
  const note=machineAssetNoteById(noteId);
  if (!note) throw new Error('Asset note not found.');
  const attachments=machineAssetNoteAttachments(note.id);
  const pdfBuffer=await buildMachineAssetNotePdf(note,attachments);
  const pdfFilename=`${safeFileToken(note.asset_number || 'Asset')}_Maintenance_Record_${safeFileToken(note.title)}_${note.note_date}.pdf`;
  const storedName=`asset-${note.asset_id}-note-${note.id}-${Date.now()}-${crypto.randomBytes(5).toString('hex')}.pdf`;
  const storedPath=path.join(machineAssetNotesDir,storedName);
  fs.writeFileSync(storedPath,pdfBuffer,{flag:'wx'});
  const oldReference=note.pdf_stored_reference;
  run('UPDATE machine_asset_notes SET pdf_filename=?,pdf_stored_reference=? WHERE id=?',[pdfFilename,`uploads/machine-asset-notes/${storedName}`,note.id]);
  if (oldReference) { const oldPath=machineAssetNoteFilePath(oldReference); if (oldPath!==storedPath&&fs.existsSync(oldPath)) fs.rmSync(oldPath,{force:true}); }
}
...
function safeStoredLibraryFilename(storedFilename:string,label:string){
  const filename=String(storedFilename??'');
  ...
  if(
    ...
    /[. ]$/.test(filename)
  )throw new Error(`${label} file reference is invalid.`);

  const extension=path.extname(filename);

  if(extension.length>=180)throw new Error(`${label} file reference is invalid.`);
...
async function validatedMachineDocument(file: Express.Multer.File) {
  const limit=configuredLibraryLimit(libraryUploadPolicyConfig,file.originalname);
...
function streamLibraryFile(req:Request,res:Response,filePath:string,mimeType:string,displayFilename:string,download:boolean,video:boolean){
  const size=fs.statSync(filePath).size;const safeName=displayFilename.replace(/[^\x20-\x7e]|["\\]/g,'_');
  res.setHeader('Content-Type',mimeType);res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Cache-Control','private, no-store');res.setHeader('Content-Disposition',`${download?'attachment':'inline'}; filename="${safeName}"`);
  if(video&&!download){
    res.setHeader('Accept-Ranges','bytes');const range=String(req.headers.range??'');
    if(range){const match=/^bytes=(\d*)-(\d*)$/.exec(range);if(!match)return res.status(416).setHeader('Content-Range',`bytes */${size}`).end();const start=match[1]?Number(match[1]):Math.max(0,size-Number(match[2]||0));const end=match[2]?Math.min(Number(match[2]),size-1):size-1;if(!Number.isInteger(start)||!Number.isInteger(end)||start<0||end<start||start>=size)return res.status(416).setHeader('Content-Range',`bytes */${size}`).end();res.status(206);res.setHeader('Content-Range',`bytes ${start}-${end}/${size}`);res.setHeader('Content-Length',String(end-start+1));fs.createReadStream(filePath,{start,end}).pipe(res);return;}
  }
  res.setHeader('Content-Length',String(size));fs.createReadStream(filePath).pipe(res);
}
function machineDocumentFolderById(assetId:number,folderId:number) {
  const folder=one<MachineDocumentFolderRow>('SELECT f.*,(SELECT COUNT(*) FROM machine_document_folders child WHERE child.parent_id=f.id) AS child_count FROM machine_document_folders f WHERE f.id=? AND f.asset_id=?',[folderId,assetId]);if(folder)folder.folder_path=assetLibraryFolderPath('machine_document_folders',assetId,folder.id);return folder;
}
function assetLibraryFolderPath(table:'machine_document_folders'|'equipment_document_folders',assetId:number,folderId:number){
  const folders=all<MachineDocumentFolderRow>(`SELECT * FROM ${table} WHERE asset_id=?`,[assetId]);const byId=new Map(folders.map(folder=>[folder.id,folder]));const names:string[]=[];const seen=new Set<number>();let current=byId.get(folderId);
  while(current){if(seen.has(current.id))throw new Error('Asset folder hierarchy contains a cycle.');seen.add(current.id);names.unshift(current.name);current=current.parent_id?byId.get(current.parent_id):undefined;}
  return names.join(' / ');
}
function validateAssetFolderParent(table:'machine_document_folders'|'equipment_document_folders',assetId:number,folderId:number,parentId:number|null){
  if(parentId===null)return;if(parentId===folderId)throw new Error('A folder cannot be its own parent.');let current=one<MachineDocumentFolderRow>(`SELECT * FROM ${table} WHERE id=? AND asset_id=?`,[parentId,assetId]);if(!current)throw new Error('Parent folder not found.');const seen=new Set<number>();while(current){if(current.id===folderId)throw new Error('A folder cannot be moved inside one of its subfolders.');if(seen.has(current.id))throw new Error('Asset folder hierarchy contains a cycle.');seen.add(current.id);current=current.parent_id?one<MachineDocumentFolderRow>(`SELECT * FROM ${table} WHERE id=? AND asset_id=?`,[current.parent_id,assetId]):undefined;}
}
function machineDocumentById(assetId:number,documentId:number) {
  const row=one<MachineDocumentRow>(`SELECT d.*,COALESCE(u.full_name,'Unknown user') AS uploaded_by_name,f.name AS folder_name FROM machine_documents d JOIN machine_document_folders f ON f.id=d.folder_id AND f.asset_id=d.asset_id LEFT JOIN users u ON u.id=d.uploaded_by_user_id WHERE d.id=? AND d.asset_id=?`,[documentId,assetId]);if(row)row.folder_path=assetLibraryFolderPath('machine_document_folders',assetId,row.folder_id);return row;
}
...
function machineDocumentFilePath(assetId:number,storedFilename:string) {
  const filename=safeStoredLibraryFilename(storedFilename,'Machine document');
  const root=path.resolve(machineDocumentAssetDirectory(assetId));
  const resolved=path.resolve(root,filename);

  if(path.dirname(resolved)!==root){
    throw new Error('Machine document file reference is invalid.');
  }
  ...
  folders: Array<{ id:number; parentId:number|null; name:string; path:string; description:string; documentCount:number; createdAt:string; updatedAt:string }>;
  documents: Array<{ id:number; folderId:number; folderName:string; folderPath:string; visibleFilename:string; originalFilename:string; storedFilename:string; extension:string; mimeType:string; sizeBytes:number; description:string; revision:string; uploadedAt:string; updatedAt:string; uploadedBy:string }>;
};
...
function appendAssetRecoveryArchive(archive:Archiver,manifest:MachineDocumentRecoveryManifest,prefix='') {
  ...
  for(const document of manifest.documents){
    ...
    const folder=(document.folderPath||document.folderName||'Unfiled').split(' / ').map(segment=>safeArchiveSegment(segment,'Folder')).join('/');
...
function publicMachineDocumentFolder(row:MachineDocumentFolderRow) {
  return {id:row.id,assetId:row.asset_id,parentId:row.parent_id??null,name:row.name,path:row.folder_path??row.name,description:row.description,documentCount:Number(row.document_count??0),childCount:Number(row.child_count??0),createdAt:row.created_at,updatedAt:row.library_updated_at??row.updated_at};
}
function publicMachineDocument(row:MachineDocumentRow) {
  const base=`/api/machine-library/assets/${row.asset_id}/documents/${row.id}`;
  const mediaType=libraryFileType(row.extension).mediaType;
  return {id:row.id,assetId:row.asset_id,folderId:row.folder_id,folderName:row.folder_name??'',folderPath:row.folder_path??row.folder_name??'',mediaType,originalFilename:row.original_filename,displayFilename:row.display_filename,extension:row.extension,mimeType:libraryFileType(row.extension).mimeType,sizeBytes:Number(row.size_bytes),description:row.description,revision:row.revision,uploadedAt:row.uploaded_at,updatedAt:row.updated_at,uploadedBy:row.uploaded_by_name??'Unknown user',openUrl:`${base}/open`,downloadUrl:`${base}/download`,canPrint:row.extension==='.pdf'||mediaType==='picture'};
}
function machineDocumentFolders(assetId:number) {
  return all<MachineDocumentFolderRow>(`SELECT f.*,COUNT(d.id) AS document_count,(SELECT COUNT(*) FROM machine_document_folders child WHERE child.parent_id=f.id) AS child_count,CASE WHEN MAX(d.updated_at)>f.updated_at THEN MAX(d.updated_at) ELSE f.updated_at END AS library_updated_at FROM machine_document_folders f LEFT JOIN machine_documents d ON d.folder_id=f.id AND d.asset_id=f.asset_id WHERE f.asset_id=? GROUP BY f.id ORDER BY f.name COLLATE NOCASE`,[assetId]).map(folder=>({...folder,folder_path:assetLibraryFolderPath('machine_document_folders',assetId,folder.id)}));
}
function machineDocuments(assetId:number,search:string,sort:string) {
  ...
  return all<MachineDocumentRow>(`SELECT d.*,COALESCE(u.full_name,'Unknown user') AS uploaded_by_name,f.name AS folder_name FROM machine_documents d JOIN machine_document_folders f ON f.id=d.folder_id AND f.asset_id=d.asset_id LEFT JOIN users u ON u.id=d.uploaded_by_user_id WHERE ${where} ORDER BY ${order}`,params).map(document=>({...document,folder_path:assetLibraryFolderPath('machine_document_folders',assetId,document.folder_id)}));
}
...
function pmTaskById(id:number,library:AssetLibrary='machine',includeDeleted=false) { return one<PmTaskRow>(`SELECT * FROM pm_tasks WHERE id=? AND asset_library=?${includeDeleted?'':' AND deleted=0'}`,[id,library]); }
function pmMeterTypeForInterval(intervalType:PmIntervalType):PmMeterType|null {return intervalType==='hourly'?'hours':intervalType==='cycles'?'cycles':null;}
function pmIntervalForMeterType(meterType:PmMeterType):PmIntervalType {return meterType==='hours'?'hourly':'cycles';}
function pmAssetMeter(assetId:number,library:AssetLibrary,meterType:PmMeterType){return one<PmAssetMeterRow>('SELECT * FROM pm_asset_meters WHERE asset_id=? AND asset_library=? AND meter_type=?',[assetId,library,meterType]);}
function pmAssetMeterHistory(assetId:number,library:AssetLibrary,meterType?:PmMeterType){return all<PmMeterHistoryRow>(`SELECT * FROM pm_meter_history WHERE asset_id=? AND asset_library=?${meterType?' AND meter_type=?':''} ORDER BY created_at DESC,id DESC LIMIT 100`,meterType?[assetId,library,meterType]:[assetId,library]);}
function publicPmAssetMeter(row:PmAssetMeterRow|null|undefined){return row?{id:row.id,meterType:row.meter_type,currentReading:Number(row.current_reading),updatedAt:row.updated_at}:null;}
function publicPmMeterHistory(row:PmMeterHistoryRow){return {id:row.id,meterType:row.meter_type,previousReading:row.previous_reading===null?null:Number(row.previous_reading),newReading:Number(row.new_reading),changeType:row.change_type,reason:row.reason,source:row.source,userName:row.user_name||'Unknown user',createdAt:row.created_at};}
function pmMeterOverrideFromInput(input:Record<string,unknown>,previousReading:number|null,newReading:number){
  if(previousReading===null||newReading>=previousReading)return null;
  const raw=isRecord(input.meterOverride)?input.meterOverride:{};const type=String(raw.type??input.type??input.meterOverrideType??input.overrideType??'').trim().toLowerCase();const reason=String(raw.reason??input.reason??input.meterOverrideReason??input.overrideReason??'').replace(/\r/g,'').trim().slice(0,1000);
  if(!['replacement','correction','override'].includes(type)||!meaningfulPmNote(reason))throw new Error(`A decreasing meter reading requires an authorized meter replacement, reset/correction, or override reason. Stored reading: ${previousReading}.`);
  return {type,reason};
}
function validatePmMeterReading(value:unknown,meterType:PmMeterType){const reading=optionalPmNumber(value,meterType==='hours'?'Hour Meter':'Cycle Counter');if(reading===null)throw new Error(`${meterType==='hours'?'Hour Meter':'Cycle Counter'} reading is required.`);if(meterType==='cycles'&&!Number.isInteger(reading))throw new Error('Cycle Counter must use a whole number.');return reading;}
function applyPmAssetMeter(input:{assetId:number;library:AssetLibrary;meterType:PmMeterType;newReading:number;actor:User;source:string;assetLabel:string;override?:{type:string;reason:string}|null;allowDecrease?:boolean;timestamp?:string}){
  const timestamp=input.timestamp??now();const previous=pmAssetMeter(input.assetId,input.library,input.meterType);const previousReading=previous===undefined?null:Number(previous.current_reading);const decreasing=previousReading!==null&&input.newReading<previousReading;const override=input.override??(input.allowDecrease?null:pmMeterOverrideFromInput({},previousReading,input.newReading));
  if(decreasing&&!override&&!input.allowDecrease)throw new Error(`A decreasing meter reading requires an authorized correction. Stored reading: ${previousReading}.`);
  run(`INSERT INTO pm_asset_meters (asset_id,asset_library,meter_type,current_reading,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?)
    ON CONFLICT(asset_library,asset_id,meter_type) DO UPDATE SET current_reading=excluded.current_reading,updated_by_user_id=excluded.updated_by_user_id,updated_at=excluded.updated_at`,[input.assetId,input.library,input.meterType,input.newReading,input.actor.id,timestamp,timestamp]);
  const meter=pmAssetMeter(input.assetId,input.library,input.meterType)!;const intervalType=pmIntervalForMeterType(input.meterType);
  run('UPDATE pm_tasks SET current_meter=?,updated_by_user_id=?,updated_at=? WHERE asset_id=? AND asset_library=? AND interval_type=? AND deleted=0',[input.newReading,input.actor.id,timestamp,input.assetId,input.library,intervalType]);
  if(previousReading===null||previousReading!==input.newReading){const workbookDecrease=decreasing&&input.allowDecrease&&!override;const changeType=override?.type??(workbookDecrease?'workbook_import':'normal');const reason=override?.reason??(workbookDecrease?'Confirmed PM Excel import applied the workbook meter value.':'');run('INSERT INTO pm_meter_history (meter_id,asset_id,asset_library,meter_type,previous_reading,new_reading,change_type,reason,source,user_id,user_name,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',[meter.id,input.assetId,input.library,input.meterType,previousReading,input.newReading,changeType,reason,input.source,input.actor.id,input.actor.full_name,timestamp]);recordHistoryLog({section:'preventive_maintenance',action:override?'pm_machine_meter_override':'pm_machine_meter_updated',entityType:'pm_asset_meter',entityId:meter.id,entityLabel:`${input.assetLabel} ${input.meterType==='hours'?'Hour Meter':'Cycle Counter'}`,assetId:String(input.assetId),machineName:input.library==='machine'?input.assetLabel:'',equipmentName:input.library==='equipment'?input.assetLabel:'',oldValue:{reading:previousReading,meterType:input.meterType},newValue:{reading:input.newReading,meterType:input.meterType,source:input.source,overrideType:override?.type??null},reasonNote:reason,actor:input.actor});}
  return {meter,previousReading,changed:previousReading===null||previousReading!==input.newReading,tasks:all<PmTaskRow>('SELECT * FROM pm_tasks WHERE asset_id=? AND asset_library=? AND interval_type=? AND deleted=0 ORDER BY id',[input.assetId,input.library,intervalType])};
}
...
function publicPmTask(row:PmTaskRow) {
  ...
  return { id:row.id,assetId:row.asset_id,title:row.title,instructions:row.instructions,intervalType:row.interval_type,intervalLabel:pmIntervalLabels[row.interval_type],intervalValue:Number(row.interval_value),lastCompletedDate:row.last_completed_date,lastCompletedMeter:row.last_completed_meter,currentMeter:row.current_meter,nextDueDate:row.next_due_date,nextDueMeter:row.next_due_meter,scheduleStatus,active:Boolean(row.active),notes:row.notes,status:state.status,countdown:state.countdown,historyCount,deleted:Boolean(row.deleted),deletedAt:row.deleted_at,deletionReason:row.deletion_reason,createdAt:row.created_at,updatedAt:row.updated_at };
}
...
function dashboardWarningNotes(user:User) {
  ...
  if (hasPermission(user,'machine.view')) rows.push(...all<MachineAssetNoteRow>(`SELECT n.*,a.asset_number,a.asset_name,a.brand,a.model,a.serial_number,a.location,'' AS category,'machine' AS asset_library,COALESCE(bs.color_hex,def.color_hex,?) AS accent_color,COALESCE(u.full_name,'Unknown user') AS created_by_name
    ...
    LEFT JOIN users u ON u.id=n.created_by_user_id WHERE n.is_warning=1 AND n.issue_status='active' AND n.deleted=0 AND a.deleted=0`,[machineDefaultBrandColors.Default]));
  if (hasPermission(user,'equipment.view')) rows.push(...all<MachineAssetNoteRow>(`SELECT n.*,a.asset_number,a.equipment_name AS asset_name,a.manufacturer AS brand,a.model,a.serial_number,a.location,a.category,'equipment' AS asset_library,'' AS accent_color,COALESCE(u.full_name,'Unknown user') AS created_by_name
    ...
    WHERE n.is_warning=1 AND n.issue_status='active' AND n.deleted=0 AND a.deleted=0`));
  return rows.sort((left,right)=>right.note_date.localeCompare(left.note_date)||right.created_at.localeCompare(left.created_at)||right.id-left.id).map(row=>{
    ...
    const pdfBaseUrl=`/api/${library}-library/asset-notes/${row.id}/pdf`;
    const version=encodeURIComponent(row.updated_at);
    return {id:row.id,assetId:row.asset_id,assetLibrary:library,assetNumber:row.asset_number??'',assetName:row.asset_name??'',brand:row.brand??'',model:row.model??'',serialNumber:row.serial_number??'',location:row.location??'',assetCategory:row.category??'',assetAccentColor:library==='machine'?safeHexColor(row.accent_color??'',machineDefaultBrandColors.Default):'',title:row.title,noteDate:row.note_date,body:row.body,warning:true,hold:Boolean(row.is_on_hold),workOrder:row.work_order_reference??'',status:'active',createdBy:row.created_by_name??'Unknown user',createdAt:row.created_at,updatedAt:row.updated_at,pdfFilename:row.pdf_filename,pdfUrl:`${pdfBaseUrl}?v=${version}`,pdfDownloadUrl:`${pdfBaseUrl}?download=true&v=${version}`};
  });
}
...
function publicPmHistoryParticipant(row:PmHistoryParticipantRow){return {userId:row.user_id,displayName:row.display_name,isPrimary:Boolean(row.is_primary),order:Number(row.participant_order)};}
function pmCompletionParticipants(input:Record<string,unknown>,actor:User){
  let raw:unknown=input.technicianIds??input.participantUserIds??[];
  if(typeof raw==='string'){try{raw=raw.trim()?JSON.parse(raw):[];}catch{throw new Error('Selected technicians are invalid.');}}
  if(!Array.isArray(raw))throw new Error('Selected technicians are invalid.');
  const ids:number[]=[];for(const value of raw){const id=Number(value);if(!Number.isInteger(id)||id<=0)throw new Error('Selected technicians are invalid.');if(id!==actor.id&&!ids.includes(id))ids.push(id);}
  if(ids.length>11)throw new Error('A PM completion can include up to 12 technicians.');
  const users=ids.map(id=>findUserById(id)).map((user,index)=>{if(!user||user.disabled||user.deleted)throw new Error(`Selected technician ${ids[index]} is unavailable.`);return user;});
  return [{userId:actor.id,displayName:actor.full_name,isPrimary:true},...users.map(user=>({userId:user.id,displayName:user.full_name,isPrimary:false}))];
}
...
type PmImportTrackerAction={kind:'addition'|'update'|'restoration';rowNumber:number;assetId:number;taskId:number|null;decreasingMeter:boolean;row:ParsedPmWorkbook['trackerRows'][number]};
type PmImportMeterAction={assetId:number;intervalType:'hourly'|'cycles';reading:number;conflictingWorkbookRows:boolean};
type PmImportHistoryAction={row:ParsedPmWorkbook['historyRows'][number];assetId:number;taskKey:string};
type PmImportStage={token:string;filename:string;sha256:string;buffer:Buffer;parsed:ParsedPmWorkbook;importedByUserId:number;previewedAt:string;trackerActions:PmImportTrackerAction[];trackerSyncActions:PmImportTrackerAction[];meterActions:PmImportMeterAction[];historyActions:PmImportHistoryAction[];preview:Record<string,unknown>};
...
function pmTrackerUpdate(task:PmTaskRow,assetNumber:string,currentDate=new Date().toISOString().slice(0,10),previousTask?:PmTaskRow,assetName?:string):TrackerWorkbookUpdate {
  ...
  return {assetNumber,assetName,taskTitle:task.title,intervalType,...(previousTask?{matchTaskTitle:previousTask.title,matchIntervalType:pmWorkbookInterval(previousTask.interval_type)}:{}),intervalValue:task.interval_value,lastCompletedDate:task.last_completed_date,lastCompletedMeter:task.last_completed_meter,currentDate:meter?null:currentDate,currentMeter:task.current_meter,remaining:calculation.remaining,status:calculation.status};
}
...
async function performPmWorkbookSyncUnlocked(input:{actor:User;sourcePath:string;originalFilename:string;trackerUpdates:TrackerWorkbookUpdate[];trackerRemovals?:TrackerWorkbookRemoval[];historyRows:HistoryWorkbookAppend[];operationId?:string}) {
  ...
  try {
    const assetChoices=all<Pick<MachineAssetRow,'asset_number'>>("SELECT asset_number FROM machine_assets WHERE deleted=0 AND status IN ('active','down') ORDER BY asset_number COLLATE NOCASE,id").map(asset=>asset.asset_number);const intervalChoices=pmIntervalTypes.map(value=>({value,label:pmIntervalLabels[value]}));
    const result=await synchronizePmWorkbook({sourcePath:input.sourcePath,destinationPath:pmExcelLatestPath,backupDir:pmExcelBackupDir,trackerUpdates:input.trackerUpdates,trackerRemovals:input.trackerRemovals,historyRows:input.historyRows,assetChoices,intervalChoices,onStage:input.operationId?(stage)=>updatePmExcelOperation(input.operationId!,stage):undefined});
...
function buildPmImportStage(parsed:ParsedPmWorkbook,buffer:Buffer,filename:string,importedByUserId:number,options:{token?:string;previewedAt?:string;store?:boolean}={}):PmImportStage {
  ...
  const additions:Array<Record<string,unknown>>=[];const updates:Array<Record<string,unknown>>=[];const conflicts:Array<Record<string,unknown>>=[];const warnings=[...parsed.warnings];const rejectedRows=[...parsed.rejectedRows];const ignoredRows:Array<Record<string,unknown>>=[];
  const trackerActions:PmImportTrackerAction[]=[];const trackerSyncActions:PmImportTrackerAction[]=[];const historyActions:PmImportHistoryAction[]=[];
  ...
  const dbTasks=all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' ORDER BY deleted,id");
  ...
  const sharedReadings=new Map<string,{assetId:number;assetNumber:string;intervalType:'hourly'|'cycles';rows:number[];values:Set<number>}>();
  for(const row of parsed.trackerRows){
    if((row.intervalType!=='hourly'&&row.intervalType!=='cycles')||row.currentMeter===null)continue;
    const resolution=trackerAssetResolutions.get(row.rowNumber);if(!resolution||resolution.status!=='resolved')continue;
    const key=`${resolution.asset.id}:${row.intervalType}`;const group=sharedReadings.get(key)??{assetId:resolution.asset.id,assetNumber:resolution.asset.asset_number,intervalType:row.intervalType,rows:[],values:new Set<number>()};
    group.rows.push(row.rowNumber);group.values.add(row.currentMeter);sharedReadings.set(key,group);
  }
  const effectiveCurrentMeters=new Map<number,number|null>();const sharedMeterConflictRows=new Set<number>();const meterActions:PmImportMeterAction[]=[];
  for(const group of sharedReadings.values()){
    const conflicting=group.values.size>1;let reading:number|null;
    if(conflicting){
      const meterType:PmMeterType=group.intervalType==='hourly'?'hours':'cycles';const canonical=pmAssetMeter(group.assetId,'machine',meterType);reading=canonical?Number(canonical.current_reading):null;
      for(const rowNumber of group.rows)sharedMeterConflictRows.add(rowNumber);
      conflicts.push({sheet:'Machine Pm Tracker',rowNumbers:group.rows,assetNumber:group.assetNumber,code:'INCONSISTENT_SHARED_METER',repairable:true,message:`${group.assetNumber} has conflicting ${group.intervalType==='hourly'?'hour':'cycle'} readings on rows ${group.rows.join(', ')}. One physical machine meter must use one Current value, so MCC will preserve the canonical machine meter and normalize every workbook row.`});
    }else reading=[...group.values][0]??null;
    for(const rowNumber of group.rows)effectiveCurrentMeters.set(rowNumber,reading);
    if(reading!==null)meterActions.push({assetId:group.assetId,intervalType:group.intervalType,reading,conflictingWorkbookRows:conflicting});
  }
  ...
  for (const row of parsed.trackerRows) {
    ...
    if (resolution.status!=='resolved') {
      ...
      else ignoredRows.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskTitle:row.taskTitle,reason:'Machine not in MCC; this workbook section is preserved.'});
      continue;
    }
    const asset=resolution.asset;const matchDetails=resolutionDetails(row.assetNumber,resolution,'Machine Pm Tracker',row.rowNumber);const titleMatches=dbTasks.filter(task=>task.asset_id===asset.id&&normalizedPmKey(task.title)===normalizedPmKey(row.taskTitle));const intervalMatches=titleMatches.filter(task=>task.interval_type===row.intervalType);const matching=intervalMatches.length?intervalMatches:titleMatches;const liveMatching=matching.filter(task=>!task.deleted);const deletedMatching=matching.filter(task=>Boolean(task.deleted));
    if (liveMatching.length>1||(!liveMatching.length&&deletedMatching.length>1)) {conflicts.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskTitle:row.taskTitle,...sectionDetails,code:'AMBIGUOUS_MCC_MATCH',message:'More than one MCC PM task matches this asset and task title.'});continue;}
    if (!intervalMatches.length&&titleMatches.length&&parsed.trackerRows.filter(candidate=>trackerAssetResolutions.get(candidate.rowNumber)?.status==='resolved'&&(trackerAssetResolutions.get(candidate.rowNumber) as {asset:{id:number}}).asset.id===asset.id&&normalizedPmKey(candidate.taskTitle)===normalizedPmKey(row.taskTitle)).length>1) {conflicts.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskTitle:row.taskTitle,code:'AMBIGUOUS_MCC_MATCH',message:'Multiple workbook intervals share this task title; an interval change cannot be matched safely.'});continue;}
    const effectiveRow=effectiveCurrentMeters.has(row.rowNumber)?{...row,currentMeter:effectiveCurrentMeters.get(row.rowNumber)??null}:row;
    if (!matching.length) {const item={sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,assetName:row.assetName,taskTitle:row.taskTitle,intervalType:row.intervalType,intervalValue:row.intervalValue,...sectionDetails,...matchDetails};additions.push(item);const action:PmImportTrackerAction={kind:'addition',rowNumber:row.rowNumber,assetId:asset.id,taskId:null,decreasingMeter:false,row:effectiveRow};trackerActions.push(action);trackerSyncActions.push(action);continue;}
    const task=liveMatching[0]??deletedMatching[0];const next=pmTaskValuesFromWorkbook(effectiveRow);const oldValue=pmHistoryValue(task);
    ...
    if(task.deleted){const action:PmImportTrackerAction={kind:'restoration',rowNumber:row.rowNumber,assetId:asset.id,taskId:task.id,decreasingMeter,row:effectiveRow};updates.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskTitle:row.taskTitle,...sectionDetails,...matchDetails,oldValue,newValue:next,decreasingMeter,restoration:true,message:'The deleted MCC PM schedule will be restored from the confirmed workbook.'});trackerActions.push(action);trackerSyncActions.push(action);}
    else if (changed) {const action:PmImportTrackerAction={kind:'update',rowNumber:row.rowNumber,assetId:asset.id,taskId:task.id,decreasingMeter,row:effectiveRow};updates.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskTitle:row.taskTitle,...sectionDetails,...matchDetails,oldValue,newValue:next,decreasingMeter});trackerActions.push(action);trackerSyncActions.push(action);}
    else {warnings.push({sheet:'Machine Pm Tracker',rowNumber:row.rowNumber,...matchDetails,message:sharedMeterConflictRows.has(row.rowNumber)?'Schedule data already matches MCC; the conflicting workbook meter will be normalized on apply.':'No MCC changes are required for this row.'});if(sharedMeterConflictRows.has(row.rowNumber))trackerSyncActions.push({kind:'update',rowNumber:row.rowNumber,assetId:asset.id,taskId:task.id,decreasingMeter:false,row:effectiveRow});}
  }
  ...
  for (const row of parsed.historyRows) {
    ...
    if (resolution.status!=='resolved') {if(resolution.status==='ambiguous')conflicts.push({sheet:'PMHistory',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskType:row.taskType,code:'AMBIGUOUS_ASSET_MATCH',assetMatchType:resolution.matchType,pressAlias:resolution.alias,candidateAssetNumbers:resolution.assets.map(item=>item.asset_number),message:'Asset Number is ambiguous in MCC.'});else ignoredRows.push({sheet:'PMHistory',rowNumber:row.rowNumber,assetNumber:row.assetNumber,taskType:row.taskType,reason:'Machine not in MCC; history row skipped.'});continue;}
    ...
    const unchangedMatches=dbTasks.filter(task=>!task.deleted&&task.asset_id===asset.id&&task.interval_type===row.intervalType&&normalizedPmKey(task.title)===normalizedPmKey(row.taskType)&&!trackerActions.some(action=>action.taskId===task.id));const plannedMatches=trackerActions.filter(action=>action.assetId===asset.id&&action.row.intervalType===row.intervalType&&normalizedPmKey(action.row.taskTitle)===normalizedPmKey(row.taskType));const taskExists=unchangedMatches.length+plannedMatches.length===1;
...
function pmTrackerRemovalsForDeletedTasks(){
  const liveKeys=new Set(all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=0 ORDER BY id").map(task=>pmImportTaskKey(String(task.asset_id),task.title,pmWorkbookInterval(task.interval_type))));
  const removals:TrackerWorkbookRemoval[]=[];const seen=new Set<string>();
  for(const task of all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=1 ORDER BY id")){
    const key=pmImportTaskKey(String(task.asset_id),task.title,pmWorkbookInterval(task.interval_type));if(liveKeys.has(key)||seen.has(key))continue;
    const asset=machineAssetById(task.asset_id,true);if(!asset)continue;seen.add(key);removals.push({assetNumber:asset.asset_number,taskTitle:task.title,intervalType:pmWorkbookInterval(task.interval_type)});
  }
  return removals;
}
async function retryPmWorkbookSync(actor:User,lockHeld=false) {
  ...
  const trackerUpdates:TrackerWorkbookUpdate[]=[];const historyRows:HistoryWorkbookAppend[]=[];const taskWorkbookAssets=new Map<number,string>();
  const trackerRemovals=pmTrackerRemovalsForDeletedTasks();
  for(const task of all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND deleted=0 ORDER BY id")){const asset=machineAssetById(task.asset_id);if(!asset)throw new Error(`Retry cannot resolve the MCC machine asset for PM task ${task.title}.`);trackerUpdates.push(pmTrackerUpdate(task,asset.asset_number,undefined,undefined,asset.asset_name));taskWorkbookAssets.set(task.id,asset.asset_number);}
...
function pmCompletionNote(input:Record<string,unknown>,intervalType:PmIntervalType,completedMeter:number|null) {
  ...
  const noteInterval:WorkbookPmInterval=intervalType;
...
function pmCompletionInput(task:PmTaskRow,body:Record<string,unknown>,upload:Express.Multer.File|undefined,actor:User) {
...
function commitPmCompletion(input:{task:PmTaskRow;assetDisplayName:string;actor:User;requestId:string;values:ReturnType<typeof pmCompletionInput>;beforeFinalize?:()=>void}) {
  ...
  try{
    ...
    const meterType=pmMeterTypeForInterval(task.interval_type);if(meterType&&values.completedMeter!==null)applyPmAssetMeter({assetId:task.asset_id,library:task.asset_library,meterType,newReading:values.completedMeter,actor,source:'pm_completion',assetLabel:input.assetDisplayName,override:values.override?{type:values.override.type,reason:values.override.reason}:null,timestamp});
    db.exec('COMMIT');
  }catch(error){
    db.exec('ROLLBACK');
...
async function syncSingleMachinePmTask(actor:User,task:PmTaskRow,asset:MachineAssetRow,currentDate?:string,previousTask?:PmTaskRow){if(!fs.existsSync(pmExcelLatestPath))return null;try{return await performPmWorkbookSync({actor,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:[pmTrackerUpdate(task,asset.asset_number,currentDate,previousTask,asset.asset_name)],historyRows:[]});}catch(error){const message=safeErrorMessage(error,[],'The PM change could not be synchronized to the approved workbook.');updatePmSyncState({status:'failed',actor,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',errorMessage:message});recordPmWorkbookAudit(actor,'pm_excel_sync_failed',{taskId:task.id,error:message},message);return{ok:false as const,error:message,status:pmSyncState()};}}
...
app.get('/api/machine-library/assets', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  ...
  const assets = assetRows.map(row=>({
    ...
    pmSummary:machineAssetPmCardSummary(all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND deleted=0 ORDER BY active DESC,hold ASC,updated_at DESC",[row.id])),
...
app.get('/api/machine-library/assets/:id/specification.pdf', requireAuth, requirePermission('machine.view'), async (req:AuthRequest,res)=>{
  try {
    ...
    const generatedAt = new Date();
    const tasks = all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND active=1 AND deleted=0 ORDER BY hold ASC,COALESCE(next_due_date,'9999-12-31'),COALESCE(next_due_meter,999999999999),title COLLATE NOCASE",[asset.id]).map(publicPmTask);
...
app.post('/api/pm-excel/confirm',requireAuth,requirePermission('machine.pm_manage'),async(req:AuthRequest,res)=>{
  ...
  try {
    ...
    updatePmExcelOperation(operationId,'importing');updatePmExcelOperation(operationId,'validating');const reparsed=await inspectPmWorkbook(previewStage.buffer);updatePmExcelOperation(operationId,'analyzing');const revalidated=buildPmImportStage(reparsed,previewStage.buffer,previewStage.filename,previewStage.importedByUserId,{token:previewStage.token,previewedAt:previewStage.previewedAt,store:false});const stage=revalidated;
    const canConfirm=Boolean((stage.preview.confirmEligibility as Record<string,unknown>)?.canConfirm);
    if(!canConfirm)return res.status(409).json({ok:false,error:'This workbook contains a machine or structure condition that MCC cannot safely repair.'});
    const sourcePath=writePmImportSource(stage.buffer,stage.sha256);const timestamp=now();const changedTaskIds:number[]=[];const insertedHistoryIds:Array<{id:number;workbookAssetNumber:string}>=[];let added=0;let updated=0;let restored=0;let historyAdded=0;
    ...
    try {
      for(const action of stage.trackerActions){const row=action.row;const values=pmTaskValuesFromWorkbook(row);let task:PmTaskRow;
        if(action.kind==='addition'){
          ...
          const existing=pmTaskById(action.taskId!,'machine',action.kind==='restoration');if(!existing)throw new Error(`PM task for workbook row ${row.rowNumber} changed after preview.`);const oldValue=pmHistoryValue(existing);
          if(action.kind==='restoration')run("UPDATE pm_tasks SET title=?,interval_type=?,interval_value=?,last_completed_date=?,last_completed_meter=?,current_meter=?,next_due_date=?,next_due_meter=?,active=1,hold=0,deleted=0,deleted_at=NULL,deleted_by_user_id=NULL,deletion_reason='',deleted_task_snapshot_json='',updated_by_user_id=?,updated_at=? WHERE id=? AND asset_library='machine'",[values.title,values.intervalType,values.intervalValue,values.lastCompletedDate,values.lastCompletedMeter,values.currentMeter,values.nextDueDate,values.nextDueMeter,req.user!.id,timestamp,existing.id]);
          ...
          task=pmTaskById(existing.id)!;const asset=machineAssetById(task.asset_id)!;recordPmAudit({action:action.kind==='restoration'?'pm_import_restored':'pm_import_updated',task,asset,actor:req.user!,oldValue,newValue:{...pmHistoryValue(task),workbookRow:row.rowNumber,importedByUserId:stage.importedByUserId,confirmedByUserId:req.user!.id},reasonNote:action.kind==='restoration'?'Confirmed PM workbook restored this previously deleted schedule.':''});updated+=1;if(action.kind==='restoration')restored+=1;
        }
        changedTaskIds.push(task.id);
      }
      for(const action of stage.meterActions){const meterType:PmMeterType=action.intervalType==='hourly'?'hours':'cycles';const asset=machineAssetById(action.assetId)!;const applied=applyPmAssetMeter({assetId:action.assetId,library:'machine',meterType,newReading:action.reading,actor:req.user!,source:action.conflictingWorkbookRows?'pm_excel_normalization':'pm_excel_import',assetLabel:asset.asset_number,allowDecrease:true,timestamp});for(const task of applied.tasks)if(!changedTaskIds.includes(task.id))changedTaskIds.push(task.id);}
      for(const action of stage.historyActions){const task=all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND deleted=0",[action.assetId]).filter(item=>item.interval_type===action.row.intervalType&&normalizedPmKey(item.title)===normalizedPmKey(action.row.taskType));if(task.length!==1)throw new Error(`PMHistory row ${action.row.rowNumber} no longer matches exactly one live MCC PM task and interval.`);const row=action.row;const workOrder=row.workOrderNumber||`MCC-IMPORT-${row.sourceRef.slice(0,12).toUpperCase()}`;const result=run(`INSERT OR IGNORE INTO pm_history (pm_task_id,asset_id,asset_library,work_order_number,task_status,start_date,completion_date,completed_meter,performed_by_user_id,performed_by_name,work_order_type,interval_type,task_type,completion_notes,no_issues_found,import_source_ref,previous_due_date,previous_due_meter,next_due_date,next_due_meter,created_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[task[0].id,action.assetId,'machine',workOrder,row.taskStatus,row.startDate,row.completionDate,null,null,row.performedBy,row.workOrderType,row.intervalType,row.taskType,row.taskNote,/no issues found/i.test(row.taskNote)?1:0,row.sourceRef,null,null,null,null,timestamp]);if(Number(result.changes)>0){insertedHistoryIds.push({id:Number(result.lastInsertRowid),workbookAssetNumber:row.assetNumber});historyAdded+=1;}}
      const summary={added,updated,restored,historyAdded,conflicts:(stage.preview.conflicts as unknown[]).length,rejectedRows:(stage.preview.rejectedRows as unknown[]).length,ignoredRows:(stage.preview.ignoredRows as unknown[]).length,warnings:(stage.preview.warnings as unknown[]).length,filename:stage.filename,sha256:stage.sha256,importedByUserId:stage.importedByUserId,confirmedByUserId:req.user!.id,confirmedAt:timestamp};
    ...
    const trackerRemovals=pmTrackerRemovalsForDeletedTasks();const sync=await performPmWorkbookSync({actor:req.user!,sourcePath,originalFilename:stage.filename,trackerUpdates,trackerRemovals,historyRows,operationId,lockHeld:true});scheduleAutoBackup('PM Excel import confirmed',req.user!);
...
const requirePmManage=(req:AuthRequest,res:Response,next:NextFunction)=>{if(!req.user)return res.status(401).json({error:'Authentication required.'});if(!hasPermission(req.user,'machine.pm_manage')&&!hasPermission(req.user,'equipment.pm_manage'))return res.status(403).json({error:'Permission denied.'});next();};
app.get('/api/preventive-maintenance/technicians',requireAuth,requirePmManage,(req:AuthRequest,res)=>{
  const query=String(req.query.q??'').trim().slice(0,120);const params:SqlParam[]=[];let where='deleted=0 AND disabled=0';if(query){where+=" AND (full_name LIKE ? ESCAPE '\\' COLLATE NOCASE OR email LIKE ? ESCAPE '\\' COLLATE NOCASE)";const like=`%${escapeLike(query)}%`;params.push(like,like);}const users=all<User>(`SELECT * FROM users WHERE ${where} ORDER BY CASE WHEN id=? THEN 0 ELSE 1 END,full_name COLLATE NOCASE,id LIMIT 20`,[...params,req.user!.id]);res.json({ok:true,technicians:users.map(user=>({id:user.id,fullName:user.full_name,email:user.email,role:user.role,isCurrentUser:user.id===req.user!.id}))});
});

function pmAssetDescriptor(library:AssetLibrary,assetId:number){if(library==='machine'){const asset=machineAssetById(assetId);return asset?{id:asset.id,label:asset.asset_number,displayName:asset.asset_name||asset.asset_number,brand:asset.brand}:null;}const asset=equipmentAssetById(assetId);return asset?{id:asset.id,label:asset.asset_number,displayName:asset.equipment_name||asset.asset_number,brand:asset.manufacturer}:null;}
function registerPmWorkflowIntegrityRoutes(library:AssetLibrary){
  const base=`/api/${library}-library`;const viewPermission=`${library}.view`;const managePermission=`${library}.pm_manage`;
  app.get(`${base}/assets/:assetId/preventive-maintenance/meters`,requireAuth,requirePermission(viewPermission),(req,res)=>{const asset=pmAssetDescriptor(library,Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:`${library==='machine'?'Machine':'Equipment'} asset not found.`});res.json({ok:true,meters:{hours:publicPmAssetMeter(pmAssetMeter(asset.id,library,'hours')),cycles:publicPmAssetMeter(pmAssetMeter(asset.id,library,'cycles'))},history:pmAssetMeterHistory(asset.id,library).map(publicPmMeterHistory)});});
  app.put(`${base}/assets/:assetId/preventive-maintenance/meters/:meterType`,requireAuth,requirePermission(managePermission),async(req:AuthRequest,res)=>{try{const asset=pmAssetDescriptor(library,Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:`${library==='machine'?'Machine':'Equipment'} asset not found.`});const meterType=String(req.params.meterType).toLowerCase() as PmMeterType;if(meterType!=='hours'&&meterType!=='cycles')throw new Error('Meter type must be hours or cycles.');const body=isRecord(req.body)?req.body:{};const reading=validatePmMeterReading(body.reading??body.currentReading,meterType);const previous=pmAssetMeter(asset.id,library,meterType);const override=pmMeterOverrideFromInput(body,previous?Number(previous.current_reading):null,reading);db.exec('BEGIN IMMEDIATE');let applied:ReturnType<typeof applyPmAssetMeter>;try{applied=applyPmAssetMeter({assetId:asset.id,library,meterType,newReading:reading,actor:req.user!,source:'machine_meter_entry',assetLabel:asset.label,override});db.exec('COMMIT');}catch(error){db.exec('ROLLBACK');throw error;}let sync:null|Awaited<ReturnType<typeof performPmWorkbookSync>>=null;if(library==='machine'&&fs.existsSync(pmExcelLatestPath)){sync=await performPmWorkbookSync({actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:applied.tasks.map(task=>pmTrackerUpdate(task,asset.label,undefined,undefined,asset.displayName)),historyRows:[]});}scheduleAutoBackup(`${library} ${meterType} meter updated`,req.user!);res.json({ok:true,meter:publicPmAssetMeter(applied.meter),updatedScheduleCount:applied.tasks.length,tasks:applied.tasks.map(publicPmTask),sync:sync?.status??pmSyncState(),syncError:sync&&!sync.ok?sync.error:null});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Machine meter could not be updated.')});}});
  app.get(`${base}/assets/:assetId/preventive-maintenance/history`,requireAuth,requirePermission(viewPermission),(req,res)=>{const asset=pmAssetDescriptor(library,Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:`${library==='machine'?'Machine':'Equipment'} asset not found.`});type AssetHistoryRow=PmHistoryRow&{schedule_title:string;schedule_deleted:number;schedule_deleted_at:string|null};const history=all<AssetHistoryRow>(`SELECT history.*,task.title AS schedule_title,task.deleted AS schedule_deleted,task.deleted_at AS schedule_deleted_at FROM pm_history history JOIN pm_tasks task ON task.id=history.pm_task_id AND task.asset_library=history.asset_library WHERE history.asset_id=? AND history.asset_library=? ORDER BY history.completion_date DESC,history.id DESC`,[asset.id,library]).map(row=>({...publicPmHistory(row),taskTitle:row.schedule_title,scheduleDeleted:Boolean(row.schedule_deleted),scheduleDeletedAt:row.schedule_deleted_at}));res.json({ok:true,asset,history});});
  app.delete(`${base}/preventive-maintenance/:pmId`,requireAuth,requirePermission(managePermission),async(req:AuthRequest,res)=>{try{const task=pmTaskById(Number(req.params.pmId),library);if(!task)return res.status(404).json({ok:false,error:'Preventive maintenance tracking not found.'});const asset=pmAssetDescriptor(library,task.asset_id);if(!asset)return res.status(404).json({ok:false,error:`${library==='machine'?'Machine':'Equipment'} asset not found.`});const reason=String(isRecord(req.body)?req.body.reason??req.body.deletionReason??'':'').replace(/\r/g,'').trim().slice(0,1000);if(!meaningfulPmNote(reason))throw new Error('A meaningful deletion reason is required.');const timestamp=now();const snapshot=pmHistoryValue(task);db.exec('BEGIN IMMEDIATE');try{run("UPDATE pm_tasks SET deleted=1,active=0,hold=0,deleted_at=?,deleted_by_user_id=?,deletion_reason=?,deleted_task_snapshot_json=?,updated_by_user_id=?,updated_at=? WHERE id=? AND asset_library=? AND deleted=0",[timestamp,req.user!.id,reason,JSON.stringify(snapshot),req.user!.id,timestamp,task.id,library]);const archived=pmTaskById(task.id,library,true)!;recordPmAudit({action:'pm_schedule_deleted',task:archived,asset:{id:asset.id,asset_number:asset.label,equipment_name:library==='equipment'?asset.displayName:undefined},actor:req.user!,oldValue:snapshot,newValue:{...pmHistoryValue(archived),historyPreserved:true,workOrderAttachmentsPreserved:true},reasonNote:reason});db.exec('COMMIT');}catch(error){db.exec('ROLLBACK');throw error;}let sync:null|Awaited<ReturnType<typeof performPmWorkbookSync>>=null;if(library==='machine'&&fs.existsSync(pmExcelLatestPath)){sync=await performPmWorkbookSync({actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:[],trackerRemovals:[{assetNumber:asset.label,taskTitle:task.title,intervalType:pmWorkbookInterval(task.interval_type)}],historyRows:[]});}scheduleAutoBackup(`${library} preventive maintenance schedule deleted`,req.user!);res.json({ok:true,historyPreserved:true,attachmentCount:one<{count:number}>('SELECT COUNT(*) AS count FROM pm_work_order_attachments attachment JOIN pm_history history ON history.id=attachment.pm_history_id WHERE history.pm_task_id=?',[task.id])?.count??0,historyCount:one<{count:number}>('SELECT COUNT(*) AS count FROM pm_history WHERE pm_task_id=?',[task.id])?.count??0,sync:sync?.status??pmSyncState(),syncError:sync&&!sync.ok?sync.error:null});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Preventive maintenance schedule could not be deleted.')});}});
}
registerPmWorkflowIntegrityRoutes('machine');
registerPmWorkflowIntegrityRoutes('equipment');
...
app.get('/api/machine-library/assets/:assetId/preventive-maintenance', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  ...
  if (!asset) return res.status(404).json({ok:false,error:'Machine asset not found.'});
  const tasks=all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND deleted=0 ORDER BY active DESC,hold ASC,COALESCE(next_due_date,'9999-12-31'),COALESCE(next_due_meter,999999999999),title COLLATE NOCASE",[asset.id]).map(publicPmTask);
...
app.post('/api/machine-library/assets/:assetId/preventive-maintenance', requireAuth, requirePermission('machine.pm_manage'), async(req:AuthRequest,res)=>{
  try {
    ...
    const input=validatePmTaskInput(req.body);const body=isRecord(req.body)?req.body:{};const meterType=pmMeterTypeForInterval(input.intervalType);const authoritative=meterType?pmAssetMeter(asset.id,'machine',meterType):undefined;let meterOverride:null|{type:string;reason:string}=null;
    if(meterType&&authoritative)input.currentMeter=Number(authoritative.current_reading);
    if(meterType&&input.currentMeter!==null&&!authoritative)meterOverride=pmMeterOverrideFromInput(body,null,input.currentMeter);
    const timestamp=now();
    ...
    let task=pmTaskById(Number(result.lastInsertRowid))!;
    if(meterType&&input.currentMeter!==null){applyPmAssetMeter({assetId:asset.id,library:'machine',meterType,newReading:input.currentMeter,actor:req.user!,source:'pm_schedule_create',assetLabel:asset.asset_number,override:meterOverride,timestamp});task=pmTaskById(task.id)!;}
    ...
    const sync=meterType&&fs.existsSync(pmExcelLatestPath)?await performPmWorkbookSync({actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND interval_type=? AND deleted=0 ORDER BY id",[asset.id,input.intervalType]).map(item=>pmTrackerUpdate(item,asset.asset_number,undefined,undefined,asset.asset_name)),historyRows:[]}):await syncSingleMachinePmTask(req.user!,task,asset);res.status(201).json({ok:true,task:publicPmTask(task),sync:sync?.status??pmSyncState(),syncError:sync&&!sync.ok?sync.error:null});
  } catch (error) {
...
app.put('/api/machine-library/preventive-maintenance/:pmId', requireAuth, requirePermission('machine.pm_manage'), async(req:AuthRequest,res)=>{
  try {
    ...
    if (!asset) return res.status(404).json({ok:false,error:'Machine asset not found.'});
    const input=validatePmTaskInput(req.body);const body=isRecord(req.body)?req.body:{};const meterType=pmMeterTypeForInterval(input.intervalType);const authoritative=meterType?pmAssetMeter(existing.asset_id,'machine',meterType):undefined;if(meterType&&authoritative&&input.currentMeter===null)input.currentMeter=Number(authoritative.current_reading);
    const override=meterType&&input.currentMeter!==null?pmMeterOverrideFromInput(body,authoritative?Number(authoritative.current_reading):null,input.currentMeter):null;
    ...
    let task=pmTaskById(existing.id)!;if(meterType&&input.currentMeter!==null){applyPmAssetMeter({assetId:asset.id,library:'machine',meterType,newReading:input.currentMeter,actor:req.user!,source:'pm_schedule_edit',assetLabel:asset.asset_number,override});task=pmTaskById(existing.id)!;}
    recordPmAudit({action:'pm_updated',task,asset,actor:req.user!,oldValue,newValue:pmHistoryValue(task)});
    if(override)recordPmAudit({action:'pm_meter_override',task,asset,actor:req.user!,oldValue:{currentMeter:authoritative?Number(authoritative.current_reading):existing.current_meter,lastCompletedMeter:existing.last_completed_meter},newValue:{currentMeter:input.currentMeter,lastCompletedMeter:task.last_completed_meter,overrideType:override.type,source:'pm_edit'},reasonNote:override.reason});
    scheduleAutoBackup('preventive maintenance updated',req.user!);
    const sync=pmMeterIntervals.has(task.interval_type)&&fs.existsSync(pmExcelLatestPath)?await performPmWorkbookSync({actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND interval_type=? AND deleted=0 ORDER BY id",[task.asset_id,task.interval_type]).map(item=>pmTrackerUpdate(item,asset.asset_number,undefined,item.id===task.id?existing:undefined,asset.asset_name)),historyRows:[]}):await syncSingleMachinePmTask(req.user!,task,asset,undefined,existing);res.json({ok:true,task:publicPmTask(task),sync:sync?.status??pmSyncState(),syncError:sync&&!sync.ok?sync.error:null});
  } catch (error) {
...
app.post('/api/machine-library/preventive-maintenance/:pmId/complete', requireAuth, requirePermission('machine.pm_manage'), receivePmWorkOrder, async(req:AuthRequest,res)=>{
  ...
  try {
    ...
    const body=isRecord(req.body)?req.body:{};const values=pmCompletionInput(task,body,req.file,req.user!);staged=values.staged;
    const oldValue=pmHistoryValue(task);
    ...
    if(fs.existsSync(pmExcelLatestPath)){try{const synchronizedTasks=pmMeterIntervals.has(updated.interval_type)?all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='machine' AND asset_id=? AND interval_type=? AND deleted=0 ORDER BY id",[updated.asset_id,updated.interval_type]):[updated];const result=await performPmWorkbookSync({actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',trackerUpdates:synchronizedTasks.map(item=>pmTrackerUpdate(item,asset.asset_number,values.completionDate,undefined,asset.asset_name)),historyRows:[pmHistoryWorkbookRow(history,updated,asset.asset_number)]});sync=result.ok?{ok:true,status:result.status}:{ok:false,error:result.error,status:result.status};}catch(error){const message=safeErrorMessage(error,[],'The completed PM uses an interval that cannot be synchronized to the approved workbook.');updatePmSyncState({status:'failed',actor:req.user!,sourcePath:pmExcelLatestPath,originalFilename:pmSyncState().originalFilename||'PM_report.xlsx',errorMessage:message});recordPmWorkbookAudit(req.user!,'pm_excel_sync_failed',{historyId,taskId:task.id,error:message},message);sync={ok:false,error:message,status:pmSyncState()};}}
...
app.get('/api/machine-library/preventive-maintenance/:pmId/history', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  const task=pmTaskById(Number(req.params.pmId),'machine',true);
...
app.get('/api/machine-library/assets/:assetId/document-folders', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  ...
  res.json({ok:true,assetId:asset.id,folders,summary:{folderCount:folders.length,documentCount},limits:publicLibraryUploadLimits(libraryUploadPolicyConfig)});
});
app.post('/api/machine-library/assets/:assetId/document-folders', requireAuth, requirePermission('machine.documents_manage'), (req:AuthRequest,res)=>{
  try {
    ...
    const parentId=isRecord(req.body)&&req.body.parentId?Number(req.body.parentId):null;validateAssetFolderParent('machine_document_folders',asset.id,0,parentId);
    if(one('SELECT id FROM machine_document_folders WHERE asset_id=? AND IFNULL(parent_id,0)=? AND lower(name)=lower(?)',[asset.id,parentId??0,name]))return res.status(409).json({ok:false,code:'DUPLICATE_FOLDER',error:'A folder with this name already exists in this location.'});
    const timestamp=now();const result=run('INSERT INTO machine_document_folders (asset_id,parent_id,name,description,created_at,updated_at,created_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?)',[asset.id,parentId,name,description,timestamp,timestamp,req.user!.id,req.user!.id]);
...
app.patch('/api/machine-library/assets/:assetId/document-folders/:folderId', requireAuth, requirePermission('machine.documents_manage'), (req:AuthRequest,res)=>{
  try {
    ...
    const body=isRecord(req.body)?req.body:{};const name=body.name===undefined?folder.name:validateMachineDocumentFolderName(body.name);const description=body.description===undefined?folder.description:validateMachineDocumentDescription(body.description);const parentId=body.parentId===undefined?folder.parent_id:body.parentId?Number(body.parentId):null;validateAssetFolderParent('machine_document_folders',asset.id,folder.id,parentId);
    if(one('SELECT id FROM machine_document_folders WHERE asset_id=? AND IFNULL(parent_id,0)=? AND lower(name)=lower(?) AND id<>?',[asset.id,parentId??0,name,folder.id]))return res.status(409).json({ok:false,code:'DUPLICATE_FOLDER',error:'A folder with this name already exists in this location.'});
    const timestamp=now();run('UPDATE machine_document_folders SET parent_id=?,name=?,description=?,updated_at=?,updated_by_user_id=? WHERE id=? AND asset_id=?',[parentId,name,description,timestamp,req.user!.id,folder.id,asset.id]);
...
app.delete('/api/machine-library/assets/:assetId/document-folders/:folderId', requireAuth, requirePermission('machine.documents_manage'), (req:AuthRequest,res)=>{
  const asset=machineAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Machine asset not found.'});
  const folder=machineDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});
  const count=one<{count:number}>('SELECT (SELECT COUNT(*) FROM machine_documents WHERE asset_id=? AND folder_id=?)+(SELECT COUNT(*) FROM machine_document_folders WHERE asset_id=? AND parent_id=?) AS count',[asset.id,folder.id,asset.id,folder.id])?.count??0;
  if(count)return res.status(409).json({ok:false,code:'FOLDER_NOT_EMPTY',error:'This folder contains files or child folders. Move or delete them before deleting the folder.'});
...
function machineUploadContext(req:Request){return{assetId:Number(req.params.assetId),folderId:Number(req.params.folderId)};}
app.post('/api/machine-library/assets/:assetId/document-folders/:folderId/documents/upload-sessions',requireAuth,requirePermission('machine.documents_upload'),(req:AuthRequest,res)=>{
  try{
    const context=machineUploadContext(req);const asset=machineAssetById(context.assetId);if(!asset)return res.status(404).json({ok:false,error:'Machine asset not found.'});
    const folder=machineDocumentFolderById(asset.id,context.folderId);if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});
    const body=isRecord(req.body)?req.body:{};const filename=safeMachineDocumentDisplayName(body.filename);const duplicateAction=String(body.duplicateAction??'').trim().toLowerCase();const duplicate=duplicateMachineDocument(asset.id,folder.id,filename);
    if(duplicate&&!['replace','keep_both'].includes(duplicateAction))return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:[filename]});
    const upload=machineDocumentResumableStore.create({ownerUserId:req.user!.id,originalName:filename,mimeType:body.mimeType,sizeBytes:body.sizeBytes,context,fields:{description:body.description,revision:body.revision,duplicateAction}});
    res.status(201).json({ok:true,upload});
  }catch(error){sendLibraryUploadError(res,error);}
});
app.get('/api/machine-library/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId',requireAuth,requirePermission('machine.documents_upload'),(req:AuthRequest,res)=>{try{res.json({ok:true,upload:machineDocumentResumableStore.status(req.params.uploadId,req.user!.id,machineUploadContext(req))});}catch(error){sendLibraryUploadError(res,error);}});
app.patch('/api/machine-library/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId',requireAuth,requirePermission('machine.documents_upload'),receiveLibraryChunk(libraryUploadPolicyConfig),(req:AuthRequest,res)=>{try{const upload=machineDocumentResumableStore.append(req.params.uploadId,req.user!.id,machineUploadContext(req),req.get('Upload-Offset'),Buffer.isBuffer(req.body)?req.body:Buffer.alloc(0));res.setHeader('Upload-Offset',String(upload.receivedBytes));res.json({ok:true,upload});}catch(error){sendLibraryUploadError(res,error);}});
app.delete('/api/machine-library/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId',requireAuth,requirePermission('machine.documents_upload'),(req:AuthRequest,res)=>{try{machineDocumentResumableStore.status(req.params.uploadId,req.user!.id,machineUploadContext(req));machineDocumentResumableStore.cancel(req.params.uploadId,req.user!.id);res.status(204).end();}catch(error){sendLibraryUploadError(res,error);}});
app.post('/api/machine-library/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId/complete',requireAuth,requirePermission('machine.documents_upload'),async(req:AuthRequest,res)=>{
  const context=machineUploadContext(req);let destination='';
  try{
    const asset=machineAssetById(context.assetId);if(!asset)return res.status(404).json({ok:false,error:'Machine asset not found.'});const folder=machineDocumentFolderById(asset.id,context.folderId);if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});
    const staged=machineDocumentResumableStore.stagedFile(req.params.uploadId,req.user!.id,context);const validated=await validatedMachineDocument(staged.file);const fields=staged.metadata.fields;const duplicateAction=String(fields.duplicateAction??'').toLowerCase();let displayFilename=validated.displayFilename;let existing=duplicateMachineDocument(asset.id,folder.id,displayFilename);
    if(existing&&!['replace','keep_both'].includes(duplicateAction)){machineDocumentResumableStore.cancel(req.params.uploadId,req.user!.id);return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:[displayFilename]});}
    ...
    const storedFilename=`${crypto.randomUUID()}${validated.extension}`;destination=machineDocumentFilePath(asset.id,storedFilename);fs.mkdirSync(machineDocumentAssetDirectory(asset.id),{recursive:true});await promoteStagedFile(staged.file.path,destination);
    const timestamp=now();let id=0;let replacedPath='';db.exec('BEGIN IMMEDIATE');try{
      if(existing){id=existing.id;replacedPath=machineDocumentFilePath(asset.id,existing.stored_filename);run('UPDATE machine_documents SET original_filename=?,display_filename=?,stored_filename=?,extension=?,mime_type=?,size_bytes=?,description=?,revision=?,uploaded_at=?,updated_at=?,uploaded_by_user_id=?,updated_by_user_id=? WHERE id=? AND asset_id=?',[validated.displayFilename,displayFilename,storedFilename,validated.extension,validated.mimeType,staged.file.size,validateMachineDocumentDescription(fields.description),validateMachineDocumentRevision(fields.revision),timestamp,timestamp,req.user!.id,req.user!.id,id,asset.id]);recordMachineDocumentHistory('document_replaced',req.user!,asset,{documentId:id,folderId:folder.id,folderName:folder.name,displayFilename});}
      else{const result=run('INSERT INTO machine_documents (asset_id,folder_id,original_filename,display_filename,stored_filename,extension,mime_type,size_bytes,description,revision,uploaded_at,updated_at,uploaded_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[asset.id,folder.id,validated.displayFilename,displayFilename,storedFilename,validated.extension,validated.mimeType,staged.file.size,validateMachineDocumentDescription(fields.description),validateMachineDocumentRevision(fields.revision),timestamp,timestamp,req.user!.id,req.user!.id]);id=Number(result.lastInsertRowid);recordMachineDocumentHistory('document_uploaded',req.user!,asset,{documentId:id,folderId:folder.id,folderName:folder.name,displayFilename});}
      run('UPDATE machine_document_folders SET updated_at=?,updated_by_user_id=? WHERE id=?',[timestamp,req.user!.id,folder.id]);db.exec('COMMIT');
    }catch(error){db.exec('ROLLBACK');throw error;}
    machineDocumentResumableStore.complete(req.params.uploadId);if(replacedPath&&replacedPath!==destination&&fs.existsSync(replacedPath))fs.rmSync(replacedPath,{force:true});refreshMachineDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('machine document uploaded',req.user!);res.status(201).json({ok:true,documents:[publicMachineDocument(machineDocumentById(asset.id,id)!)]});
  }catch(error){if(destination&&fs.existsSync(destination))fs.rmSync(destination,{force:true});try{machineDocumentResumableStore.cancel(req.params.uploadId,req.user?.id);}catch{}sendLibraryUploadError(res,error,'File could not be uploaded.');}
});
app.post('/api/machine-library/assets/:assetId/document-folders/:folderId/documents', requireAuth, requirePermission('machine.documents_upload'), receiveMachineDocuments, async(req:AuthRequest,res)=>{
  const newFiles:string[]=[];const replacedFiles:string[]=[];let databaseCommitted=false;
  try {
    const asset=machineAssetById(Number(req.params.assetId));if(!asset){cleanupStagedFiles(req.files as Express.Multer.File[]|undefined);return res.status(404).json({ok:false,error:'Machine asset not found.'});}
    const folder=machineDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder){cleanupStagedFiles(req.files as Express.Multer.File[]|undefined);return res.status(404).json({ok:false,error:'Document folder not found.'});}
    const files=(req.files as Express.Multer.File[]|undefined)??[];if(!files.length)return res.status(400).json({ok:false,error:'Select at least one file to upload.'});
    const validated=[];for(const file of files)validated.push({file,...await validatedMachineDocument(file)});const duplicateAction=String(req.body?.duplicateAction??'').trim().toLowerCase();const selectedNames=new Set<string>();
    ...
    if(duplicates.length&&!['replace','keep_both'].includes(duplicateAction)){cleanupStagedFiles(files);return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:[...new Set(duplicates)]});}
    ...
    try {
      ...
      for(const item of validated){
        ...
        if(existing&&duplicateAction==='keep_both'){displayFilename=uniqueMachineDocumentName(asset.id,folder.id,displayFilename);existing=undefined;}
        const storedFilename=`${crypto.randomUUID()}${item.extension}`;const storedPath=machineDocumentFilePath(asset.id,storedFilename);await promoteStagedFile(item.file.path,storedPath);newFiles.push(storedPath);
  ...
  }catch(error){cleanupStagedFiles((req.files as Express.Multer.File[]|undefined));if(!databaseCommitted)for(const filePath of newFiles)if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Files could not be uploaded.')});}
});
for(const mode of ['open','download'] as const)app.get(`/api/machine-library/assets/:assetId/documents/:documentId/${mode}`,requireAuth,requirePermission('machine.view'),(req:AuthRequest,res)=>{
  try{
    ...
    const fileType=libraryFileType(document.extension);const forceDownload=mode==='download'||fileType.mediaType==='file'||(fileType.mediaType==='document'&&String(document.extension).toLowerCase()!=='.pdf');streamLibraryFile(req,res,filePath,fileType.mimeType,document.display_filename,forceDownload,fileType.mediaType==='video');
...
app.get('/api/machine-library/assets/:assetId/document-folders/:folderId/export',requireAuth,requirePermission('machine.view'),(req:AuthRequest,res)=>{
  try{
    const asset=machineAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Machine asset not found.'});
    const folder=machineDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});
    const folders=machineDocumentFolders(asset.id);
    const documents=machineDocuments(asset.id,'','name');
    const prepared=prepareShareableFolderArchive(folder.id,folders.map(item=>({id:item.id,parentId:item.parent_id??null,name:item.name})),documents.map(document=>({folderId:document.folder_id,displayFilename:document.display_filename,sourcePath:machineDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes)})));
    const date=new Date().toISOString().slice(0,10);streamShareableFolderArchive(res,`${safeShareableSegment(folder.name,'Machine Folder')}_${date}.zip`,prepared);
  }catch(error){if(res.headersSent)res.destroy(error as Error);else res.status(500).json({ok:false,error:safeErrorMessage(error,[],'Machine folder export could not be created.')});}
});
app.get('/api/machine-library/assets/:assetId/document-folders/:folderId/export/plan',requireAuth,requirePermission('machine.view'),(req:AuthRequest,res)=>{
  try{const asset=machineAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Machine asset not found.'});const folder=machineDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});const folders=machineDocumentFolders(asset.id);const documents=machineDocuments(asset.id,'','name');const prepared=prepareShareableFolderArchive(folder.id,folders.map(item=>({id:item.id,parentId:item.parent_id??null,name:item.name})),documents.map(document=>({folderId:document.folder_id,displayFilename:document.display_filename,sourcePath:machineDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes)})));const urls=new Map(documents.map(document=>[path.resolve(machineDocumentFilePath(asset.id,document.stored_filename)),`/api/machine-library/assets/${asset.id}/documents/${document.id}/download`]));res.json({ok:true,plan:{rootName:prepared.rootName,directories:prepared.directories,files:prepared.files.map(file=>({archivePath:file.archivePath,sizeBytes:file.sizeBytes,downloadUrl:urls.get(path.resolve(file.sourcePath))})),summary:{fileCount:prepared.files.length,totalBytes:prepared.files.reduce((sum,file)=>sum+file.sizeBytes,0)}}});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Machine folder export could not be prepared.')});}
});
...
app.get('/api/machine-library/assets/:id/notes', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  ...
  res.json({ok:true,notes,currentUser:{id:req.user!.id,fullName:req.user!.full_name,email:req.user!.email,role:req.user!.role},permissions:{canCreate:hasPermission(req.user!,'machine.notes_manage'),canExportWorkOrderRecords:hasPermission(req.user!,'machine.import_export')}});
});
app.post('/api/machine-library/assets/:id/notes', requireAuth, requirePermission('machine.notes_manage'), receiveMachineAssetNote, async (req:AuthRequest,res)=>{
  ...
  try {
    ...
    const warning=assetNoteWarningValue(req.body?.warning);const hold=warning&&assetNoteWarningValue(req.body?.hold);
    const workOrder=validateAssetNoteWorkOrder(req.body?.workOrder??req.body?.workOrderReference,warning);
    if (!title) throw new Error('Note Title is required.');
    if (!body) throw new Error('Note Body is required.');
    const files=(req.files ?? []) as Express.Multer.File[];
    const sources=assetNoteAttachmentSources(req.body?.attachmentSources,files.length);const validated=files.map((file,index)=>({file,detected:validatedAssetNoteAttachment(file,sources[index]),sourceType:sources[index],sha256:assetNoteFileHash(file)}));
    const labor=parseAssetNoteLabor(req.body?.laborEntries,req.user!,true)!;
    const timestamp=now();
    const result=run('INSERT INTO machine_asset_notes (asset_id,title,note_date,body,is_warning,is_on_hold,work_order_reference,issue_status,pdf_filename,pdf_stored_reference,created_by_user_id,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[
      asset.id,title,noteDate,body,warning?1:0,hold?1:0,workOrder,'active','','',req.user!.id,req.user!.id,timestamp,timestamp,
    ]);
    noteId=Number(result.lastInsertRowid);
    replaceAssetNoteLabor('machine',noteId,labor,timestamp);const seenHashes=new Set<string>();
    for (const {file,detected,sourceType,sha256} of validated) {
      if(seenHashes.has(sha256))continue;seenHashes.add(sha256);
      const storedName=`asset-${asset.id}-note-${noteId}-attachment-${Date.now()}-${crypto.randomBytes(5).toString('hex')}${detected.extension}`;
      const storedPath=path.join(machineAssetNotesDir,storedName);
      fs.writeFileSync(storedPath,file.buffer,{flag:'wx'});
      storedFiles.push(storedPath);
      run('INSERT INTO machine_asset_note_attachments (note_id,original_filename,mime_type,file_size,stored_file_reference,uploaded_by_user_id,created_at,source_type,sha256) VALUES (?,?,?,?,?,?,?,?,?)',[
        noteId,safeMachineAssetNoteOriginalName(file.originalname,detected.extension),detected.mimeType,file.size,`uploads/machine-asset-notes/${storedName}`,req.user!.id,timestamp,sourceType,sha256,
      ]);
    }
    await regenerateMachineAssetNotePdf(noteId);
    if (warning) recordAssetNoteLifecycleEvent({library:'machine',noteId,type:'issue_created',actor:req.user!,newValue:assetNoteHistorySnapshot(machineAssetNoteById(noteId)!),createdAt:timestamp});
    scheduleAutoBackup('machine asset note created',req.user!);
    res.status(201).json({ok:true,note:publicMachineAssetNote(machineAssetNoteById(noteId)!,req.user)});
  } catch (error) {
    if (noteId) {
      const note=machineAssetNoteById(noteId);
      run('DELETE FROM machine_asset_note_attachments WHERE note_id=?',[noteId]);
      run("DELETE FROM asset_note_labor WHERE asset_library='machine' AND note_id=?",[noteId]);
      run("DELETE FROM asset_note_lifecycle_events WHERE asset_library='machine' AND note_id=?",[noteId]);
      run('DELETE FROM machine_asset_notes WHERE id=?',[noteId]);
      if (note?.pdf_stored_reference) { const pdfPath=machineAssetNoteFilePath(note.pdf_stored_reference); if (fs.existsSync(pdfPath)) fs.rmSync(pdfPath,{force:true}); }
    }
    ...
    res.status(/not found/i.test(message)?404:/required|valid|must be|match|50 MB|resolve the warning|work order/i.test(message)?400:500).json({ok:false,error:message});
  }
});
app.put('/api/machine-library/asset-notes/:noteId', requireAuth, requirePermission('machine.notes_manage'), receiveMachineAssetNote, async (req:AuthRequest,res)=>{
  ...
  let previousLabor:AssetNoteLaborInput[]=[];
  let previousSnapshot:ReturnType<typeof assetNoteHistorySnapshot>|null=null;
  try {
    const existing=machineAssetNoteById(Number(req.params.noteId));
    if (!existing) return res.status(404).json({ok:false,error:'Asset note not found.'});
    if (!canEditAssetNoteIssue(req.user!,'machine',existing)) return res.status(403).json({ok:false,error:'You may edit only warning issues that you created.',code:'ISSUE_OWNER_REQUIRED'});
    previous=existing;
    previousLabor=assetNoteLabor('machine',existing.id).map((row,index)=>({userId:row.user_id??req.user!.id,displayName:row.display_name,hours:Number(row.labor_hours),isPrimary:Boolean(row.is_primary),order:index}));
    previousSnapshot=assetNoteHistorySnapshot(existing);
    const title=String(req.body?.title ?? '').replace(/\s+/g,' ').trim().slice(0,180);
    const body=String(req.body?.body ?? '').replace(/\r/g,'').trim().slice(0,30000);
    const noteDate=validMachineAssetNoteDate(req.body?.noteDate);
    const warning=assetNoteWarningValue(req.body?.warning,Boolean(existing.is_warning));const hold=warning&&assetNoteWarningValue(req.body?.hold,Boolean(existing.is_on_hold));
    if (existing.is_warning&&!warning) throw new Error('Resolve the warning issue instead of clearing Warning / Needs Attention.');
    const workOrder=validateAssetNoteWorkOrder(req.body?.workOrder??req.body?.workOrderReference,warning);
    if (!title) throw new Error('Note Title is required.');
    if (!body) throw new Error('Note Body is required.');
    const files=(req.files ?? []) as Express.Multer.File[];
    const sources=assetNoteAttachmentSources(req.body?.attachmentSources,files.length);const validated=files.map((file,index)=>({file,detected:validatedAssetNoteAttachment(file,sources[index]),sourceType:sources[index],sha256:assetNoteFileHash(file)}));
    const labor=parseAssetNoteLabor(req.body?.laborEntries,req.user!,false);
    const timestamp=now();
    run('UPDATE machine_asset_notes SET title=?,note_date=?,body=?,is_warning=?,is_on_hold=?,work_order_reference=?,issue_status=?,updated_by_user_id=?,updated_at=? WHERE id=?',[title,noteDate,body,warning?1:0,hold?1:0,workOrder,existing.is_warning?existing.issue_status:'active',req.user!.id,timestamp,existing.id]);
    if(labor)replaceAssetNoteLabor('machine',existing.id,labor,timestamp);const seenHashes=new Set<string>();
    for (const {file,detected,sourceType,sha256} of validated) {
      if(seenHashes.has(sha256)||assetNoteAttachmentExists('machine',existing.id,sha256))continue;seenHashes.add(sha256);
      ...
      fs.writeFileSync(storedPath,file.buffer,{flag:'wx'});
      storedFiles.push(storedPath);
      const result=run('INSERT INTO machine_asset_note_attachments (note_id,original_filename,mime_type,file_size,stored_file_reference,uploaded_by_user_id,created_at,source_type,sha256) VALUES (?,?,?,?,?,?,?,?,?)',[
        existing.id,safeMachineAssetNoteOriginalName(file.originalname,detected.extension),detected.mimeType,file.size,`uploads/machine-asset-notes/${storedName}`,req.user!.id,timestamp,sourceType,sha256,
      ]);
      newAttachmentIds.push(Number(result.lastInsertRowid));
    }
    await regenerateMachineAssetNotePdf(existing.id);
    recordAssetNoteHoldChange('machine',existing.id,Boolean(existing.is_on_hold),hold,req.user!,timestamp);
    if (warning) recordAssetNoteLifecycleEvent({library:'machine',noteId:existing.id,type:existing.is_warning?'issue_edited':'warning_enabled',actor:req.user!,oldValue:previousSnapshot??assetNoteHistorySnapshot(existing),newValue:assetNoteHistorySnapshot(machineAssetNoteById(existing.id)!),createdAt:timestamp});
    scheduleAutoBackup('machine asset note updated',req.user!);
    res.json({ok:true,note:publicMachineAssetNote(machineAssetNoteById(existing.id)!,req.user)});
  } catch (error) {
    for (const attachmentId of newAttachmentIds) run('DELETE FROM machine_asset_note_attachments WHERE id=?',[attachmentId]);
    for (const filePath of storedFiles) if (fs.existsSync(filePath)) fs.rmSync(filePath,{force:true});
    if (previous) {run('UPDATE machine_asset_notes SET title=?,note_date=?,body=?,is_warning=?,is_on_hold=?,work_order_reference=?,issue_status=?,updated_by_user_id=?,updated_at=? WHERE id=?',[previous.title,previous.note_date,previous.body,previous.is_warning,previous.is_on_hold,previous.work_order_reference,previous.issue_status,previous.updated_by_user_id,previous.updated_at,previous.id]);replaceAssetNoteLabor('machine',previous.id,previousLabor,previous.updated_at);}
...
app.delete('/api/machine-library/asset-notes/:noteId', requireAuth, requirePermission('machine.notes_manage'), (req:AuthRequest,res)=>{
  ...
  if (!canDeleteAssetNoteIssue(req.user!,'machine',note)) return res.status(403).json({ok:false,error:'Only a Manager or Admin may delete a warning issue.',code:'ISSUE_MANAGER_REQUIRED'});
  if (note.is_warning) {
    try {
      const reason=requiredAssetNoteReason(isRecord(req.body)?req.body.deleteReason??req.body.reason:'','Delete Reason',2000);
      const timestamp=now();
      db.exec('BEGIN IMMEDIATE');
      try {
        run("UPDATE machine_asset_notes SET deleted=1,deleted_at=?,deleted_by_user_id=?,deleted_by_name=?,delete_reason=?,updated_by_user_id=?,updated_at=? WHERE id=? AND deleted=0",[timestamp,req.user!.id,req.user!.full_name,reason,req.user!.id,timestamp,note.id]);
        recordAssetNoteLifecycleEvent({library:'machine',noteId:note.id,type:'issue_deleted',actor:req.user!,reason,oldValue:assetNoteHistorySnapshot(note),newValue:{deleted:true,historyPreserved:true,attachmentsPreserved:true},createdAt:timestamp});
        db.exec('COMMIT');
      } catch (error) { db.exec('ROLLBACK');throw error; }
      audit(req,'warning issue delete','machine_asset_note',note.id,{library:'machine',reason,historyPreserved:true,attachmentsPreserved:true});
      scheduleAutoBackup('machine warning issue deleted',req.user!);
      return res.json({ok:true,historyPreserved:true});
    } catch (error) {
      return res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Warning issue could not be deleted.')});
    }
  }
  const attachments=machineAssetNoteAttachments(note.id);
  run('DELETE FROM machine_asset_note_attachments WHERE note_id=?',[note.id]);
  run("DELETE FROM asset_note_labor WHERE asset_library='machine' AND note_id=?",[note.id]);
  run('DELETE FROM machine_asset_notes WHERE id=?',[note.id]);
  for (const item of attachments) { const filePath=machineAssetNoteFilePath(item.stored_file_reference); if (fs.existsSync(filePath)) fs.rmSync(filePath,{force:true}); }
  if (note.pdf_stored_reference) { const pdfPath=machineAssetNoteFilePath(note.pdf_stored_reference); if (fs.existsSync(pdfPath)) fs.rmSync(pdfPath,{force:true}); }
...
app.delete('/api/machine-library/asset-note-attachments/:attachmentId', requireAuth, requirePermission('machine.notes_manage'), async (req:AuthRequest,res)=>{
  try {
    ...
    const note=attachment?machineAssetNoteById(attachment.note_id):undefined;
    if (!attachment || !note) return res.status(404).json({ok:false,error:'Asset note attachment not found.'});
    if (!canEditAssetNoteIssue(req.user!,'machine',note)) return res.status(403).json({ok:false,error:'You may remove attachments only from warning issues that you may edit.',code:'ISSUE_OWNER_REQUIRED'});
    if (note.is_warning&&note.issue_status==='resolved') return res.status(409).json({ok:false,error:'Resolved issue attachments are preserved. Reopen the issue before changing attachments.',code:'ISSUE_RESOLVED'});
    const filePath=machineAssetNoteFilePath(attachment.stored_file_reference);
    run('DELETE FROM machine_asset_note_attachments WHERE id=?',[attachment.id]);
    await regenerateMachineAssetNotePdf(attachment.note_id);
    if (fs.existsSync(filePath)) fs.rmSync(filePath,{force:true});
    if(note.is_warning){recordAssetNoteLifecycleEvent({library:'machine',noteId:note.id,type:'attachment_deleted',actor:req.user!,newValue:{attachmentId:attachment.id,filename:attachment.original_filename}});audit(req,'warning issue attachment delete','machine_asset_note',note.id,{attachmentId:attachment.id,filename:attachment.original_filename});}
    scheduleAutoBackup('machine asset note attachment deleted',req.user!);
    res.json({ok:true,note:publicMachineAssetNote(machineAssetNoteById(attachment.note_id)!,req.user)});
  } catch (error) {
    res.status(500).json({ok:false,error:safeErrorMessage(error,[],'Attachment could not be deleted.')});
  }
});
app.get('/api/machine-library/asset-notes/:noteId/pdf', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  try {
    const note=machineAssetNoteById(Number(req.params.noteId));
    if (!note || !note.pdf_stored_reference) return res.status(404).json({ok:false,error:'Asset note PDF not found.'});
    const filePath=machineAssetNoteFilePath(note.pdf_stored_reference);
    if (!fs.existsSync(filePath)) return res.status(404).json({ok:false,error:'Stored asset note PDF is missing.'});
    res.setHeader('Content-Type','application/pdf');
    res.setHeader('Content-Length',String(fs.statSync(filePath).size));
    res.setHeader('X-Content-Type-Options','nosniff');
    res.setHeader('Cache-Control','private, no-store');
    res.setHeader('Content-Disposition',`${String(req.query.download)==='true'?'attachment':'inline'}; filename="${safeMachineAssetNoteOriginalName(note.pdf_filename,'.pdf')}"`);
    res.sendFile(filePath);
  } catch (error) {
    res.status(500).json({ok:false,error:safeErrorMessage(error,[],'Asset note PDF could not be opened.')});
  }
});
app.get('/api/machine-library/asset-note-attachments/:attachmentId/file', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  try {
    const attachment=one<MachineAssetNoteAttachmentRow>('SELECT * FROM machine_asset_note_attachments WHERE id=?',[Number(req.params.attachmentId)]);
    ...
    const filePath=machineAssetNoteFilePath(attachment.stored_file_reference);
    if (!fs.existsSync(filePath)) return res.status(404).json({ok:false,error:'Stored attachment is missing.'});
    const extension=path.extname(attachment.original_filename).toLowerCase();
    res.setHeader('Content-Type',attachment.mime_type || 'application/octet-stream');
    res.setHeader('Content-Length',String(fs.statSync(filePath).size));
    res.setHeader('X-Content-Type-Options','nosniff');
    res.setHeader('Cache-Control','private, no-store');
    res.setHeader('Content-Disposition',`${String(req.query.download)==='true'||['.doc','.docx'].includes(extension)?'attachment':'inline'}; filename="${safeMachineAssetNoteOriginalName(attachment.original_filename,extension)}"`);
    res.sendFile(filePath);
  } catch (error) {
    res.status(500).json({ok:false,error:safeErrorMessage(error,[],'Attachment could not be opened.')});
  }
});
app.get('/api/machine-library/assets/:id/component-images', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  const asset = machineAssetById(Number(req.params.id));
  if (!asset) return res.status(404).json({ok:false,error:'Machine asset not found.'});
  const images = all<MachineComponentImageRow>('SELECT * FROM machine_component_images WHERE asset_id=? ORDER BY component_type',[asset.id]).map(publicMachineComponentImage);
  res.json({ok:true,images});
});
app.get('/api/machine-library/assets/:id/component-images/:componentType/image', requireAuth, requirePermission('machine.view'), (req:AuthRequest,res)=>{
  try {
    const asset = machineAssetById(Number(req.params.id));
    if (!asset) return res.status(404).json({ok:false,error:'Machine asset not found.'});
    const componentType = machineComponentImageType(req.params.componentType);
    const image = machineComponentImageByAsset(asset.id,componentType);
    if (!image) return res.status(404).json({ok:false,error:'Component image not found.'});
    const filePath = machineComponentImageFilePath(image.stored_file_reference);
    if (!fs.existsSync(filePath)) return res.status(404).json({ok:false,error:'Stored component image is missing.'});
    const originalExtension = path.extname(image.original_filename).toLowerCase();
    const extension = ['.jpg','.jpeg','.png','.webp'].includes(originalExtension) ? originalExtension : image.mime_type==='image/png'?'.png':image.mime_type==='image/webp'?'.webp':'.jpg';
    const fileName = `${safeFileToken(path.basename(image.original_filename,path.extname(image.original_filename)))}${extension}`;
    res.setHeader('Content-Type',image.mime_type);
    res.setHeader('Content-Length',String(fs.statSync(filePath).size));
    res.setHeader('Cache-Control','private, no-store');
    res.setHeader('Content-Disposition',`${String(req.query.download)==='true'?'attachment':'inline'}; filename="${fileName}"`);
    res.sendFile(filePath);
  } catch (error) {
    const message = safeErrorMessage(error,[],'Component image could not be opened.');
...
app.put('/api/machine-library/assets/:id/component-images/:componentType', requireAuth, requirePermission('machine.documents_manage'), receiveMachineComponentImage, (req:AuthRequest,res)=>{
  ...
  try {
    ...
    if (!asset) return res.status(404).json({ok:false,error:'Machine asset not found.'});
    const componentType = machineComponentImageType(req.params.componentType);
    if (!req.file) throw new Error('Choose a component image.');
    const detected = validatedMachineComponentImage(req.file);
    const timestamp = now();
    const storedFileName = `asset-${asset.id}-${componentType}-${Date.now()}-${crypto.randomBytes(6).toString('hex')}${detected.extension}`;
    newFilePath = path.join(machineComponentImagesDir,storedFileName);
    fs.writeFileSync(newFilePath,req.file.buffer,{flag:'wx'});
    const storedReference = `uploads/machine-component-images/${storedFileName}`;
    const originalFileName = safeMachineComponentOriginalName(req.file.originalname,path.extname(req.file.originalname).toLowerCase());
    const previous = machineComponentImageByAsset(asset.id,componentType);
    db.exec('BEGIN IMMEDIATE');
    try {
      run(`INSERT INTO machine_component_images (asset_id,component_type,original_filename,mime_type,file_size,stored_file_reference,uploaded_at,uploaded_by_user_id) VALUES (?,?,?,?,?,?,?,?)
        ON CONFLICT(asset_id,component_type) DO UPDATE SET original_filename=excluded.original_filename,mime_type=excluded.mime_type,file_size=excluded.file_size,stored_file_reference=excluded.stored_file_reference,uploaded_at=excluded.uploaded_at,uploaded_by_user_id=excluded.uploaded_by_user_id`,[
        asset.id,componentType,originalFileName,detected.mimeType,req.file.size,storedReference,timestamp,actor.id,
      ]);
      db.exec('COMMIT');
    ...
    if (previous?.stored_file_reference && previous.stored_file_reference!==storedReference) {
      const previousPath = machineComponentImageFilePath(previous.stored_file_reference);
      if (fs.existsSync(previousPath)) fs.rmSync(previousPath,{force:true});
    }
    const saved = machineComponentImageByAsset(asset.id,componentType)!;
    scheduleAutoBackup('machine component image updated',actor);
    res.status(previous?200:201).json({ok:true,image:publicMachineComponentImage(saved),replaced:Boolean(previous)});
  } catch (error) {
    if (!databaseCommitted && newFilePath && fs.existsSync(newFilePath)) fs.rmSync(newFilePath,{force:true});
    const message = safeErrorMessage(error,[],'Component image upload failed.');
...
app.get('/api/equipment-library/assets',requireAuth,requirePermission('equipment.view'),(req:AuthRequest,res)=>{
  ...
  const assets=all<EquipmentAssetRow>(`SELECT a.* FROM equipment_assets a WHERE ${where.join(' AND ')} ORDER BY a.asset_number COLLATE NOCASE`,params).map(row=>{
    ...
    return {...publicEquipmentAsset(row),pmSummary:machineAssetPmCardSummary(all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='equipment' AND asset_id=? AND deleted=0 ORDER BY active DESC,hold ASC,updated_at DESC",[row.id])),latestHistory:latest?publicHistoryRecord(latest):null};
  });
...
app.get('/api/equipment-library/assets/:assetId/preventive-maintenance',requireAuth,requirePermission('equipment.view'),(req,res)=>{
  ...
  res.json(all<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='equipment' AND asset_id=? AND deleted=0 ORDER BY active DESC,hold ASC,COALESCE(next_due_date,'9999-12-31'),COALESCE(next_due_meter,999999999999),title COLLATE NOCASE",[asset.id]).map(publicPmTask));
});
app.post('/api/equipment-library/assets/:assetId/preventive-maintenance',requireAuth,requirePermission('equipment.pm_manage'),(req:AuthRequest,res)=>{
  try{
    const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});
    const clientRequestId=String(req.get('Idempotency-Key')??'').trim();if(clientRequestId&&!/^[A-Za-z0-9._:-]{8,128}$/.test(clientRequestId))return res.status(400).json({ok:false,error:'Invalid PM save request identifier.'});
    const existingRequest=clientRequestId?one<PmTaskRow>("SELECT * FROM pm_tasks WHERE asset_library='equipment' AND client_request_id=?",[clientRequestId]):undefined;if(existingRequest){if(existingRequest.asset_id!==asset.id)return res.status(409).json({ok:false,error:'PM save request identifier is already in use.'});return res.json({ok:true,task:publicPmTask(existingRequest),duplicatePrevented:true});}
    const input=validatePmTaskInput(req.body);const body=isRecord(req.body)?req.body:{};const meterType=pmMeterTypeForInterval(input.intervalType);const authoritative=meterType?pmAssetMeter(asset.id,'equipment',meterType):undefined;
    if(meterType&&authoritative)input.currentMeter=Number(authoritative.current_reading);
    const override=meterType&&input.currentMeter!==null&&!authoritative?pmMeterOverrideFromInput(body,null,input.currentMeter):null;const timestamp=now();
    const result=run(`INSERT INTO pm_tasks (asset_id,asset_library,client_request_id,title,instructions,interval_type,interval_value,last_completed_date,last_completed_meter,current_meter,next_due_date,next_due_meter,active,hold,notes,created_by_user_id,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?)`,[asset.id,'equipment',clientRequestId||null,input.title,input.instructions,input.intervalType,input.intervalValue,input.lastCompletedDate,input.lastCompletedMeter,input.currentMeter,input.nextDueDate,input.nextDueMeter,input.active?1:0,input.hold?1:0,input.notes,req.user!.id,req.user!.id,timestamp,timestamp]);let task=pmTaskById(Number(result.lastInsertRowid),'equipment')!;
    if(meterType&&input.currentMeter!==null){applyPmAssetMeter({assetId:asset.id,library:'equipment',meterType,newReading:input.currentMeter,actor:req.user!,source:'pm_schedule_create',assetLabel:asset.asset_number,override,timestamp});task=pmTaskById(task.id,'equipment')!;}
    recordPmAudit({action:'pm_created',task,asset,actor:req.user!,newValue:pmHistoryValue(task)});scheduleAutoBackup('equipment preventive maintenance created',req.user!);res.status(201).json({ok:true,task:publicPmTask(task)});
  }catch(error){const message=safeErrorMessage(error,[],'Preventive maintenance tracking could not be created.');res.status(/not found/i.test(message)?404:400).json({ok:false,error:message});}
});
app.put('/api/equipment-library/preventive-maintenance/:pmId',requireAuth,requirePermission('equipment.pm_manage'),(req:AuthRequest,res)=>{
  try{
    const existing=pmTaskById(Number(req.params.pmId),'equipment');if(!existing)return res.status(404).json({ok:false,error:'Preventive maintenance tracking not found.'});const asset=equipmentAssetById(existing.asset_id);if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});
    const input=validatePmTaskInput(req.body);const body=isRecord(req.body)?req.body:{};const meterType=pmMeterTypeForInterval(input.intervalType);const authoritative=meterType?pmAssetMeter(existing.asset_id,'equipment',meterType):undefined;if(meterType&&authoritative&&input.currentMeter===null)input.currentMeter=Number(authoritative.current_reading);const override=meterType&&input.currentMeter!==null?pmMeterOverrideFromInput(body,authoritative?Number(authoritative.current_reading):null,input.currentMeter):null;const oldValue=pmHistoryValue(existing);
    run('UPDATE pm_tasks SET title=?,instructions=?,interval_type=?,interval_value=?,last_completed_date=?,last_completed_meter=?,current_meter=?,next_due_date=?,next_due_meter=?,active=?,hold=?,notes=?,updated_by_user_id=?,updated_at=? WHERE id=? AND asset_library=?',[input.title,input.instructions,input.intervalType,input.intervalValue,input.lastCompletedDate,input.lastCompletedMeter,input.currentMeter,input.nextDueDate,input.nextDueMeter,input.active?1:0,input.hold?1:0,input.notes,req.user!.id,now(),existing.id,'equipment']);let task=pmTaskById(existing.id,'equipment')!;
    if(meterType&&input.currentMeter!==null){applyPmAssetMeter({assetId:asset.id,library:'equipment',meterType,newReading:input.currentMeter,actor:req.user!,source:'pm_schedule_edit',assetLabel:asset.asset_number,override});task=pmTaskById(existing.id,'equipment')!;}
    recordPmAudit({action:'pm_updated',task,asset,actor:req.user!,oldValue,newValue:pmHistoryValue(task)});if(override)recordPmAudit({action:'pm_meter_override',task,asset,actor:req.user!,oldValue:{currentMeter:authoritative?Number(authoritative.current_reading):existing.current_meter,lastCompletedMeter:existing.last_completed_meter},newValue:{currentMeter:task.current_meter,lastCompletedMeter:task.last_completed_meter,overrideType:override.type,source:'pm_edit'},reasonNote:override.reason});scheduleAutoBackup('equipment preventive maintenance updated',req.user!);res.json({ok:true,task:publicPmTask(task)});
  }catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Preventive maintenance tracking could not be updated.')});}
});
...
app.post('/api/equipment-library/preventive-maintenance/:pmId/complete',requireAuth,requirePermission('equipment.pm_manage'),receivePmWorkOrder,(req:AuthRequest,res)=>{
  ...
  try{
    ...
    const body=isRecord(req.body)?req.body:{};const values=pmCompletionInput(task,body,req.file,req.user!);staged=values.staged;const oldValue=pmHistoryValue(task);
    const failureInjection=process.env.NODE_ENV==='test'&&String(req.get('X-MCC-Test-PM-Finalize-Failure')??'')==='1'?()=>{throw new Error('Simulated work-order PDF finalization failure.');}:undefined;
...
function equipmentDocumentFilePath(assetId:number,storedFilename:string){
  const filename=safeStoredLibraryFilename(storedFilename,'Equipment document');
  const root=path.resolve(equipmentDocumentAssetDirectory(assetId));
  const resolved=path.resolve(root,filename);

  if(path.dirname(resolved)!==root){
    throw new Error('Equipment document path is invalid.');
  }

  return resolved;
}
function equipmentDocumentFolderById(assetId:number,folderId:number){const folder=one<MachineDocumentFolderRow>('SELECT f.*,(SELECT COUNT(*) FROM equipment_documents d WHERE d.folder_id=f.id AND d.asset_id=f.asset_id) AS document_count,(SELECT COUNT(*) FROM equipment_document_folders child WHERE child.parent_id=f.id) AS child_count FROM equipment_document_folders f WHERE f.id=? AND f.asset_id=?',[folderId,assetId]);if(folder)folder.folder_path=assetLibraryFolderPath('equipment_document_folders',assetId,folder.id);return folder;}
function equipmentDocumentById(assetId:number,documentId:number){const document=one<MachineDocumentRow>(`SELECT d.*,f.name AS folder_name,COALESCE(u.full_name,'Unknown user') AS uploaded_by_name FROM equipment_documents d JOIN equipment_document_folders f ON f.id=d.folder_id LEFT JOIN users u ON u.id=d.uploaded_by_user_id WHERE d.id=? AND d.asset_id=?`,[documentId,assetId]);if(document)document.folder_path=assetLibraryFolderPath('equipment_document_folders',assetId,document.folder_id);return document;}
...
function registerEquipmentDocumentLibraryRoutes(){
  ...
  app.get(`${base}/assets/:assetId/document-folders`,requireAuth,requirePermission('equipment.view'),(req,res)=>{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folders=all<MachineDocumentFolderRow>('SELECT f.*,(SELECT COUNT(*) FROM equipment_documents d WHERE d.folder_id=f.id AND d.asset_id=f.asset_id) AS document_count,(SELECT COUNT(*) FROM equipment_document_folders child WHERE child.parent_id=f.id) AS child_count FROM equipment_document_folders f WHERE f.asset_id=? ORDER BY f.name COLLATE NOCASE',[asset.id]).map(folder=>publicMachineDocumentFolder({...folder,folder_path:assetLibraryFolderPath('equipment_document_folders',asset.id,folder.id)}));const documentCount=one<{count:number}>('SELECT COUNT(*) AS count FROM equipment_documents WHERE asset_id=?',[asset.id])?.count??0;res.json({ok:true,folders,summary:{folderCount:folders.length,documentCount},limits:publicLibraryUploadLimits(libraryUploadPolicyConfig)});});
  app.post(`${base}/assets/:assetId/document-folders`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{try{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const body=isRecord(req.body)?req.body:{};const name=validateMachineDocumentFolderName(body.name);const description=validateMachineDocumentDescription(body.description);const parentId=body.parentId?Number(body.parentId):null;validateAssetFolderParent('equipment_document_folders',asset.id,0,parentId);if(one('SELECT id FROM equipment_document_folders WHERE asset_id=? AND IFNULL(parent_id,0)=? AND lower(name)=lower(?)',[asset.id,parentId??0,name]))return res.status(409).json({ok:false,code:'DUPLICATE_FOLDER',error:'A folder with this name already exists in this location.'});const timestamp=now();const result=run('INSERT INTO equipment_document_folders (asset_id,parent_id,name,description,created_at,updated_at,created_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?)',[asset.id,parentId,name,description,timestamp,timestamp,req.user!.id,req.user!.id]);const folder=equipmentDocumentFolderById(asset.id,Number(result.lastInsertRowid))!;recordEquipmentDocumentHistory('document_folder_created',req.user!,asset,{folderId:folder.id,parentId,folderName:name,description});refreshEquipmentDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('equipment document folder created',req.user!);res.status(201).json({ok:true,folder:publicMachineDocumentFolder(folder)});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Folder could not be created.')});}});
  app.patch(`${base}/assets/:assetId/document-folders/:folderId`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{try{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folder=equipmentDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});const body=isRecord(req.body)?req.body:{};const name=body.name===undefined?folder.name:validateMachineDocumentFolderName(body.name);const description=body.description===undefined?folder.description:validateMachineDocumentDescription(body.description);const parentId=body.parentId===undefined?folder.parent_id:body.parentId?Number(body.parentId):null;validateAssetFolderParent('equipment_document_folders',asset.id,folder.id,parentId);if(one('SELECT id FROM equipment_document_folders WHERE asset_id=? AND IFNULL(parent_id,0)=? AND lower(name)=lower(?) AND id<>?',[asset.id,parentId??0,name,folder.id]))return res.status(409).json({ok:false,code:'DUPLICATE_FOLDER',error:'A folder with this name already exists in this location.'});run('UPDATE equipment_document_folders SET parent_id=?,name=?,description=?,updated_at=?,updated_by_user_id=? WHERE id=? AND asset_id=?',[parentId,name,description,now(),req.user!.id,folder.id,asset.id]);recordEquipmentDocumentHistory(name!==folder.name?'document_folder_renamed':'document_folder_edited',req.user!,asset,{folderId:folder.id,parentId,previousName:folder.name,folderName:name,description});refreshEquipmentDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('equipment document folder updated',req.user!);res.json({ok:true,folder:publicMachineDocumentFolder(equipmentDocumentFolderById(asset.id,folder.id)!)});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Folder could not be updated.')});}});
  app.delete(`${base}/assets/:assetId/document-folders/:folderId`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folder=equipmentDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});if(folder.document_count||folder.child_count)return res.status(409).json({ok:false,code:'FOLDER_NOT_EMPTY',error:'This folder contains files or child folders. Move or delete them before deleting the folder.'});run('DELETE FROM equipment_document_folders WHERE id=? AND asset_id=?',[folder.id,asset.id]);recordEquipmentDocumentHistory('document_folder_deleted',req.user!,asset,{folderId:folder.id,folderName:folder.name});refreshEquipmentDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('equipment document folder deleted',req.user!);res.json({ok:true});});
  app.get(`${base}/assets/:assetId/documents`,requireAuth,requirePermission('equipment.view'),(req,res)=>{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const documents=all<MachineDocumentRow>(`SELECT d.*,f.name AS folder_name,COALESCE(u.full_name,'Unknown user') AS uploaded_by_name FROM equipment_documents d JOIN equipment_document_folders f ON f.id=d.folder_id LEFT JOIN users u ON u.id=d.uploaded_by_user_id WHERE d.asset_id=? ORDER BY d.display_filename COLLATE NOCASE`,[asset.id]).map(document=>publicEquipmentDocument({...document,folder_path:assetLibraryFolderPath('equipment_document_folders',asset.id,document.folder_id)}));res.json({ok:true,documents});});
  const uploadContext=(req:Request)=>({assetId:Number(req.params.assetId),folderId:Number(req.params.folderId)});
  app.post(`${base}/assets/:assetId/document-folders/:folderId/documents/upload-sessions`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{try{const context=uploadContext(req);const asset=equipmentAssetById(context.assetId);if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folder=equipmentDocumentFolderById(asset.id,context.folderId);if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});const body=isRecord(req.body)?req.body:{};const filename=safeMachineDocumentDisplayName(body.filename);const duplicateAction=String(body.duplicateAction??'').trim().toLowerCase();if(equipmentDuplicateDocument(asset.id,folder.id,filename)&&!['replace','keep_both'].includes(duplicateAction))return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:[filename]});const upload=equipmentDocumentResumableStore.create({ownerUserId:req.user!.id,originalName:filename,mimeType:body.mimeType,sizeBytes:body.sizeBytes,context,fields:{description:body.description,revision:body.revision,duplicateAction}});res.status(201).json({ok:true,upload});}catch(error){sendLibraryUploadError(res,error);}});
  app.get(`${base}/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{try{res.json({ok:true,upload:equipmentDocumentResumableStore.status(req.params.uploadId,req.user!.id,uploadContext(req))});}catch(error){sendLibraryUploadError(res,error);}});
  app.patch(`${base}/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId`,requireAuth,requirePermission('equipment.write'),receiveLibraryChunk(libraryUploadPolicyConfig),(req:AuthRequest,res)=>{try{const upload=equipmentDocumentResumableStore.append(req.params.uploadId,req.user!.id,uploadContext(req),req.get('Upload-Offset'),Buffer.isBuffer(req.body)?req.body:Buffer.alloc(0));res.setHeader('Upload-Offset',String(upload.receivedBytes));res.json({ok:true,upload});}catch(error){sendLibraryUploadError(res,error);}});
  app.delete(`${base}/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId`,requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{try{equipmentDocumentResumableStore.status(req.params.uploadId,req.user!.id,uploadContext(req));equipmentDocumentResumableStore.cancel(req.params.uploadId,req.user!.id);res.status(204).end();}catch(error){sendLibraryUploadError(res,error);}});
  app.post(`${base}/assets/:assetId/document-folders/:folderId/documents/upload-sessions/:uploadId/complete`,requireAuth,requirePermission('equipment.write'),async(req:AuthRequest,res)=>{const context=uploadContext(req);let destination='';try{const asset=equipmentAssetById(context.assetId);if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folder=equipmentDocumentFolderById(asset.id,context.folderId);if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});const staged=equipmentDocumentResumableStore.stagedFile(req.params.uploadId,req.user!.id,context);const validated=await validatedMachineDocument(staged.file);const fields=staged.metadata.fields;const duplicateAction=String(fields.duplicateAction??'').toLowerCase();let displayFilename=validated.displayFilename;let existing=equipmentDuplicateDocument(asset.id,folder.id,displayFilename);if(existing&&!['replace','keep_both'].includes(duplicateAction)){equipmentDocumentResumableStore.cancel(req.params.uploadId,req.user!.id);return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:[displayFilename]});}if(existing&&duplicateAction==='keep_both'){displayFilename=uniqueEquipmentDocumentName(asset.id,folder.id,displayFilename);existing=undefined;}const storedFilename=`${crypto.randomUUID()}${validated.extension}`;destination=equipmentDocumentFilePath(asset.id,storedFilename);fs.mkdirSync(equipmentDocumentAssetDirectory(asset.id),{recursive:true});await promoteStagedFile(staged.file.path,destination);const timestamp=now();let id=0;let replacedPath='';db.exec('BEGIN IMMEDIATE');try{if(existing){id=existing.id;replacedPath=equipmentDocumentFilePath(asset.id,existing.stored_filename);run('UPDATE equipment_documents SET original_filename=?,display_filename=?,stored_filename=?,extension=?,mime_type=?,size_bytes=?,description=?,revision=?,uploaded_at=?,updated_at=?,uploaded_by_user_id=?,updated_by_user_id=? WHERE id=? AND asset_id=?',[validated.displayFilename,displayFilename,storedFilename,validated.extension,validated.mimeType,staged.file.size,validateMachineDocumentDescription(fields.description),validateMachineDocumentRevision(fields.revision),timestamp,timestamp,req.user!.id,req.user!.id,id,asset.id]);recordEquipmentDocumentHistory('document_replaced',req.user!,asset,{documentId:id,folderId:folder.id,folderName:folder.name,displayFilename});}else{const result=run('INSERT INTO equipment_documents (asset_id,folder_id,original_filename,display_filename,stored_filename,extension,mime_type,size_bytes,description,revision,uploaded_at,updated_at,uploaded_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[asset.id,folder.id,validated.displayFilename,displayFilename,storedFilename,validated.extension,validated.mimeType,staged.file.size,validateMachineDocumentDescription(fields.description),validateMachineDocumentRevision(fields.revision),timestamp,timestamp,req.user!.id,req.user!.id]);id=Number(result.lastInsertRowid);recordEquipmentDocumentHistory('document_uploaded',req.user!,asset,{documentId:id,folderId:folder.id,folderName:folder.name,displayFilename});}run('UPDATE equipment_document_folders SET updated_at=?,updated_by_user_id=? WHERE id=?',[timestamp,req.user!.id,folder.id]);db.exec('COMMIT');}catch(error){db.exec('ROLLBACK');throw error;}equipmentDocumentResumableStore.complete(req.params.uploadId);if(replacedPath&&replacedPath!==destination&&fs.existsSync(replacedPath))fs.rmSync(replacedPath,{force:true});refreshEquipmentDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('equipment document uploaded',req.user!);res.status(201).json({ok:true,documents:[publicEquipmentDocument(equipmentDocumentById(asset.id,id)!)]});}catch(error){if(destination&&fs.existsSync(destination))fs.rmSync(destination,{force:true});try{equipmentDocumentResumableStore.cancel(req.params.uploadId,req.user?.id);}catch{}sendLibraryUploadError(res,error,'File could not be uploaded.');}});
  app.post(`${base}/assets/:assetId/document-folders/:folderId/documents`,requireAuth,requirePermission('equipment.write'),receiveEquipmentDocuments,async(req:AuthRequest,res)=>{const written:string[]=[];const oldFiles:string[]=[];let committed=false;try{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset){cleanupStagedFiles(req.files as Express.Multer.File[]|undefined);return res.status(404).json({ok:false,error:'Equipment asset not found.'});}const folder=equipmentDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder){cleanupStagedFiles(req.files as Express.Multer.File[]|undefined);return res.status(404).json({ok:false,error:'Document folder not found.'});}const files=(req.files as Express.Multer.File[]|undefined)??[];if(!files.length)return res.status(400).json({ok:false,error:'Select at least one file to upload.'});const validated=[];for(const file of files)validated.push({file,...await validatedMachineDocument(file)});const duplicateAction=String(req.body?.duplicateAction??'').toLowerCase();const names=new Set<string>();const duplicates=validated.filter(item=>{const key=item.displayFilename.toLowerCase();const duplicate=names.has(key)||Boolean(equipmentDuplicateDocument(asset.id,folder.id,item.displayFilename));names.add(key);return duplicate;});if(duplicates.length&&!['replace','keep_both'].includes(duplicateAction)){cleanupStagedFiles(files);return res.status(409).json({ok:false,code:'DOCUMENT_DUPLICATE',error:'A file with this name already exists.',duplicates:duplicates.map(item=>item.displayFilename)});}const description=validateMachineDocumentDescription(req.body?.description);const revision=validateMachineDocumentRevision(req.body?.revision);const timestamp=now();const ids:number[]=[];db.exec('BEGIN IMMEDIATE');try{fs.mkdirSync(equipmentDocumentAssetDirectory(asset.id),{recursive:true});for(const item of validated){let displayFilename=item.displayFilename;let existing=equipmentDuplicateDocument(asset.id,folder.id,displayFilename);if(existing&&duplicateAction==='keep_both'){displayFilename=uniqueEquipmentDocumentName(asset.id,folder.id,displayFilename);existing=undefined;}const storedFilename=`${crypto.randomUUID()}${item.extension}`;const filePath=equipmentDocumentFilePath(asset.id,storedFilename);await promoteStagedFile(item.file.path,filePath);written.push(filePath);if(existing&&duplicateAction==='replace'){oldFiles.push(equipmentDocumentFilePath(asset.id,existing.stored_filename));run('UPDATE equipment_documents SET original_filename=?,display_filename=?,stored_filename=?,extension=?,mime_type=?,size_bytes=?,description=?,revision=?,uploaded_at=?,updated_at=?,uploaded_by_user_id=?,updated_by_user_id=? WHERE id=? AND asset_id=?',[item.displayFilename,displayFilename,storedFilename,item.extension,item.mimeType,item.file.size,description,revision,timestamp,timestamp,req.user!.id,req.user!.id,existing.id,asset.id]);ids.push(existing.id);recordEquipmentDocumentHistory('document_replaced',req.user!,asset,{documentId:existing.id,folderId:folder.id,folderName:folder.name,displayFilename});}else{const result=run('INSERT INTO equipment_documents (asset_id,folder_id,original_filename,display_filename,stored_filename,extension,mime_type,size_bytes,description,revision,uploaded_at,updated_at,uploaded_by_user_id,updated_by_user_id) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)',[asset.id,folder.id,item.displayFilename,displayFilename,storedFilename,item.extension,item.mimeType,item.file.size,description,revision,timestamp,timestamp,req.user!.id,req.user!.id]);const id=Number(result.lastInsertRowid);ids.push(id);recordEquipmentDocumentHistory('document_uploaded',req.user!,asset,{documentId:id,folderId:folder.id,folderName:folder.name,displayFilename});}}run('UPDATE equipment_document_folders SET updated_at=?,updated_by_user_id=? WHERE id=?',[timestamp,req.user!.id,folder.id]);db.exec('COMMIT');committed=true;}catch(error){db.exec('ROLLBACK');throw error;}oldFiles.forEach(filePath=>{if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});});refreshEquipmentDocumentRecoveryMetadata(asset.id);scheduleAutoBackup('equipment files uploaded',req.user!);res.status(201).json({ok:true,documents:ids.map(id=>publicEquipmentDocument(equipmentDocumentById(asset.id,id)!))});}catch(error){cleanupStagedFiles(req.files as Express.Multer.File[]|undefined);if(!committed)written.forEach(filePath=>{if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});});res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Files could not be uploaded.')});}});
  for(const mode of ['open','download'] as const)app.get(`${base}/assets/:assetId/documents/:documentId/${mode}`,requireAuth,requirePermission('equipment.view'),(req,res)=>{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const document=equipmentDocumentById(asset.id,Number(req.params.documentId));if(!document)return res.status(404).json({ok:false,error:'File not found.'});const filePath=equipmentDocumentFilePath(asset.id,document.stored_filename);if(!fs.existsSync(filePath))return res.status(404).json({ok:false,error:'Stored file is missing.'});const fileType=libraryFileType(document.extension);const forceDownload=mode==='download'||fileType.mediaType==='file'||(fileType.mediaType==='document'&&String(document.extension).toLowerCase()!=='.pdf');streamLibraryFile(req,res,filePath,fileType.mimeType,document.display_filename,forceDownload,fileType.mediaType==='video');});
  ...
  app.get(`${base}/assets/:assetId/document-folders/:folderId/export`,requireAuth,requirePermission('equipment.view'),(req,res)=>{
    try{
      const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});
      const folder=equipmentDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});
      const folders=all<MachineDocumentFolderRow>('SELECT * FROM equipment_document_folders WHERE asset_id=? ORDER BY id',[asset.id]);
      const documents=all<MachineDocumentRow>('SELECT * FROM equipment_documents WHERE asset_id=? ORDER BY id',[asset.id]);
      const prepared=prepareShareableFolderArchive(folder.id,folders.map(item=>({id:item.id,parentId:item.parent_id??null,name:item.name})),documents.map(document=>({folderId:document.folder_id,displayFilename:document.display_filename,sourcePath:equipmentDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes)})));
      const date=new Date().toISOString().slice(0,10);streamShareableFolderArchive(res,`${safeShareableSegment(folder.name,'Equipment Folder')}_${date}.zip`,prepared);
    }catch(error){if(res.headersSent)res.destroy(error as Error);else res.status(500).json({ok:false,error:safeErrorMessage(error,[],'Equipment folder export could not be created.')});}
  });
  app.get(`${base}/assets/:assetId/document-folders/:folderId/export/plan`,requireAuth,requirePermission('equipment.view'),(req,res)=>{try{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const folder=equipmentDocumentFolderById(asset.id,Number(req.params.folderId));if(!folder)return res.status(404).json({ok:false,error:'Document folder not found.'});const folders=all<MachineDocumentFolderRow>('SELECT * FROM equipment_document_folders WHERE asset_id=? ORDER BY id',[asset.id]);const documents=all<MachineDocumentRow>('SELECT * FROM equipment_documents WHERE asset_id=? ORDER BY id',[asset.id]);const prepared=prepareShareableFolderArchive(folder.id,folders.map(item=>({id:item.id,parentId:item.parent_id??null,name:item.name})),documents.map(document=>({folderId:document.folder_id,displayFilename:document.display_filename,sourcePath:equipmentDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes)})));const urls=new Map(documents.map(document=>[path.resolve(equipmentDocumentFilePath(asset.id,document.stored_filename)),`/api/equipment-library/assets/${asset.id}/documents/${document.id}/download`]));res.json({ok:true,plan:{rootName:prepared.rootName,directories:prepared.directories,files:prepared.files.map(file=>({archivePath:file.archivePath,sizeBytes:file.sizeBytes,downloadUrl:urls.get(path.resolve(file.sourcePath))})),summary:{fileCount:prepared.files.length,totalBytes:prepared.files.reduce((sum,file)=>sum+file.sizeBytes,0)}}});}catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Equipment folder export could not be prepared.')});}});
  app.get(`${base}/assets/:assetId/documents/export`,requireAuth,requirePermission('equipment.view'),(req,res)=>{const asset=equipmentAssetById(Number(req.params.assetId));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});const recovery=refreshEquipmentDocumentRecoveryMetadata(asset.id);const manifest=recovery.manifests[0];const date=new Date().toISOString().slice(0,10);streamRecoveryArchive(res,`Equipment-${safeArchiveSegment(asset.asset_number,String(asset.id))}_Documents_${date}.zip`,archive=>{archive.append(`${JSON.stringify(manifest,null,2)}\n`,{name:'asset-info.json'});for(const document of manifest.documents){const source=equipmentDocumentFilePath(asset.id,String(document.storedFilename));if(!fs.existsSync(source))throw new Error(`Stored file is missing: ${document.visibleFilename}`);const folder=String(document.folderPath||document.folderName||'Unfiled').split(' / ').map(segment=>safeArchiveSegment(segment,'Folder')).join('/');archive.file(source,{name:`${folder}/${safeArchiveSegment(String(document.visibleFilename),'file')}`});}});});
}
registerEquipmentDocumentLibraryRoutes();
app.get('/api/equipment-library/documents/recovery-export',requireAuth,requirePermission('equipment.import_export'),(req:AuthRequest,res)=>{const recovery=refreshEquipmentDocumentRecoveryMetadata();const date=new Date().toISOString().slice(0,10);streamRecoveryArchive(res,`MCC_Equipment_Document_Recovery_${date}.zip`,archive=>{archive.append(`${JSON.stringify(recovery.index,null,2)}\n`,{name:'equipment-library-index.json'});archive.append('MCC Equipment Asset File Library - Full Recovery Export\n\nFiles are grouped by visible Equipment Asset Number. asset-info.json retains stable internal IDs, hierarchy, and physical filenames for recovery.\n',{name:'README.txt'});for(const manifest of recovery.manifests){const prefix=`Equipment-${safeArchiveSegment(manifest.asset.assetNumber,String(manifest.asset.internalAssetId))} - ${safeArchiveSegment(manifest.asset.equipmentName,'Unnamed Equipment')}`;archive.append(`${JSON.stringify(manifest,null,2)}\n`,{name:`${prefix}/asset-info.json`});for(const document of manifest.documents){const source=equipmentDocumentFilePath(manifest.asset.internalAssetId,String(document.storedFilename));const folder=String(document.folderPath||document.folderName||'Unfiled').split(' / ').map(segment=>safeArchiveSegment(segment,'Folder')).join('/');if(fs.existsSync(source))archive.file(source,{name:`${prefix}/${folder}/${safeArchiveSegment(String(document.visibleFilename),'file')}`});}}});});

function assetLibraryMasterExportSources(){
  const directories:string[]=[];const files:MasterExportSource[]=[];
  for(const asset of all<{id:number;asset_number:string;asset_name:string}>('SELECT id,asset_number,asset_name FROM machine_assets WHERE deleted=0 ORDER BY asset_number COLLATE NOCASE')){
    const assetPath=`Machines/${safeShareableSegment(asset.asset_number,String(asset.id))} - ${safeShareableSegment(asset.asset_name,'Unnamed Machine')} (${asset.id})`;directories.push(assetPath);
    for(const folder of machineDocumentFolders(asset.id))directories.push(`${assetPath}/${assetLibraryFolderPath('machine_document_folders',asset.id,folder.id).split(' / ').map(segment=>safeShareableSegment(segment,'Folder')).join('/')}`);
    for(const document of machineDocuments(asset.id,'','name'))files.push({archivePath:`${assetPath}/${String(document.folder_path||document.folder_name||'Unfiled').split(' / ').map(segment=>safeShareableSegment(segment,'Folder')).join('/')}/${safeShareableSegment(document.display_filename,`document-${document.id}`)}`,sourcePath:machineDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes),downloadUrl:`/api/machine-library/assets/${asset.id}/documents/${document.id}/download`});
  }
  for(const asset of all<{id:number;asset_number:string;equipment_name:string}>('SELECT id,asset_number,equipment_name FROM equipment_assets WHERE deleted=0 ORDER BY asset_number COLLATE NOCASE')){
    const assetPath=`Equipment/${safeShareableSegment(asset.asset_number,String(asset.id))} - ${safeShareableSegment(asset.equipment_name,'Unnamed Equipment')} (${asset.id})`;directories.push(assetPath);
    const folders=all<MachineDocumentFolderRow>('SELECT * FROM equipment_document_folders WHERE asset_id=? ORDER BY id',[asset.id]);for(const folder of folders)directories.push(`${assetPath}/${assetLibraryFolderPath('equipment_document_folders',asset.id,folder.id).split(' / ').map(segment=>safeShareableSegment(segment,'Folder')).join('/')}`);
    const documents=all<MachineDocumentRow>('SELECT * FROM equipment_documents WHERE asset_id=? ORDER BY id',[asset.id]);for(const document of documents)files.push({archivePath:`${assetPath}/${assetLibraryFolderPath('equipment_document_folders',asset.id,document.folder_id).split(' / ').map(segment=>safeShareableSegment(segment,'Folder')).join('/')}/${safeShareableSegment(document.display_filename,`document-${document.id}`)}`,sourcePath:equipmentDocumentFilePath(asset.id,document.stored_filename),sizeBytes:Number(document.size_bytes),downloadUrl:`/api/equipment-library/assets/${asset.id}/documents/${document.id}/download`});
  }
  const facility=facilityInfoService?.masterExportSources();if(facility){directories.push(...facility.directories);files.push(...facility.files);}
  return{directories,files};
}
async function preparedAssetLibraryMasterExport(req:Request){const sources=assetLibraryMasterExportSources();const controller=new AbortController();req.once('aborted',()=>controller.abort());return prepareMasterExport({rootName:'MCC_Asset_Library_Export',appVersion:applicationVersion??'0.0.0',...sources,signal:controller.signal});}
app.get('/api/asset-libraries/master-export/plan',requireAuth,requirePermission('machine.view'),requirePermission('equipment.view'),requirePermission('facility.view'),async(req,res)=>{try{const plan=await preparedAssetLibraryMasterExport(req);res.json({ok:true,plan:publicMasterExportPlan(plan)});}catch(error){if(!res.headersSent)res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Master Export could not be prepared.')});}});
app.get('/api/asset-libraries/master-export',requireAuth,requirePermission('machine.view'),requirePermission('equipment.view'),requirePermission('facility.view'),async(req,res)=>{try{const plan=await preparedAssetLibraryMasterExport(req);streamMasterExport(res,`MCC_Asset_Library_Master_Export_${new Date().toISOString().slice(0,10)}.zip`,plan);}catch(error){if(res.headersSent)res.destroy(error as Error);else res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Master Export could not be created.')});}});
...
function publicEquipmentAssetNoteAttachment(row:MachineAssetNoteAttachmentRow){return publicAssetNoteAttachment('equipment',row);}
function publicEquipmentAssetNote(row:MachineAssetNoteRow,actor?:User){return publicAssetNote(row,'equipment',actor);}
async function regenerateEquipmentAssetNotePdf(noteId:number){const note=equipmentAssetNoteById(noteId);if(!note)throw new Error('Equipment asset note not found.');const buffer=await buildMachineAssetNotePdf(note,equipmentAssetNoteAttachments(note.id));const filename=`${safeFileToken(note.asset_number??`Equipment_${note.asset_id}`)}_Maintenance_Record_${safeFileToken(note.title)}_${note.note_date}.pdf`;const storedName=`${crypto.randomUUID()}.pdf`;const storedPath=path.join(equipmentAssetNotesDir,storedName);fs.writeFileSync(storedPath,buffer,{flag:'wx'});const previous=note.pdf_stored_reference;run('UPDATE equipment_asset_notes SET pdf_filename=?,pdf_stored_reference=?,updated_at=? WHERE id=?',[filename,`uploads/equipment-asset-notes/${storedName}`,now(),note.id]);if(previous){const previousPath=equipmentAssetNoteFilePath(previous);if(fs.existsSync(previousPath))fs.rmSync(previousPath,{force:true});}}
app.get('/api/equipment-library/assets/:id/notes',requireAuth,requirePermission('equipment.view'),(req:AuthRequest,res)=>{
  ...
  res.json({ok:true,notes,currentUser:{id:req.user!.id,fullName:req.user!.full_name,email:req.user!.email,role:req.user!.role},permissions:{canCreate:hasPermission(req.user!,'equipment.notes_manage'),canExportWorkOrderRecords:hasPermission(req.user!,'equipment.import_export')}});
});
app.post('/api/equipment-library/assets/:id/notes',requireAuth,requirePermission('equipment.write'),receiveMachineAssetNote,async(req:AuthRequest,res)=>{
  ...
  try{
    const asset=equipmentAssetById(Number(req.params.id));if(!asset)return res.status(404).json({ok:false,error:'Equipment asset not found.'});
    const title=String(req.body?.title??'').replace(/\s+/g,' ').trim().slice(0,180);const body=String(req.body?.body??'').replace(/\r/g,'').trim().slice(0,30000);if(!title||!body)throw new Error('Note Title and Note Body are required.');
    const noteDate=validMachineAssetNoteDate(req.body?.noteDate);const warning=assetNoteWarningValue(req.body?.warning);const hold=warning&&assetNoteWarningValue(req.body?.hold);const workOrder=validateAssetNoteWorkOrder(req.body?.workOrder??req.body?.workOrderReference,warning);
    const files=(req.files as Express.Multer.File[]|undefined)??[];const sources=assetNoteAttachmentSources(req.body?.attachmentSources,files.length);const validated=files.map((file,index)=>({file,detected:validatedAssetNoteAttachment(file,sources[index]),sourceType:sources[index],sha256:assetNoteFileHash(file)}));const labor=parseAssetNoteLabor(req.body?.laborEntries,req.user!,true)!;const timestamp=now();
    const result=run('INSERT INTO equipment_asset_notes (asset_id,title,note_date,body,is_warning,is_on_hold,work_order_reference,issue_status,created_by_user_id,updated_by_user_id,created_at,updated_at) VALUES (?,?,?,?,?,?,?,?,?,?,?,?)',[asset.id,title,noteDate,body,warning?1:0,hold?1:0,workOrder,'active',req.user!.id,req.user!.id,timestamp,timestamp]);noteId=Number(result.lastInsertRowid);
    replaceAssetNoteLabor('equipment',noteId,labor,timestamp);const seenHashes=new Set<string>();for(const {file,detected,sourceType,sha256} of validated){if(seenHashes.has(sha256))continue;seenHashes.add(sha256);const storedName=`${crypto.randomUUID()}${detected.extension}`;const storedPath=path.join(equipmentAssetNotesDir,storedName);fs.writeFileSync(storedPath,file.buffer,{flag:'wx'});stored.push(storedPath);run('INSERT INTO equipment_asset_note_attachments (note_id,original_filename,mime_type,file_size,stored_file_reference,uploaded_by_user_id,created_at,source_type,sha256) VALUES (?,?,?,?,?,?,?,?,?)',[noteId,safeMachineAssetNoteOriginalName(file.originalname,detected.extension),detected.mimeType,file.size,`uploads/equipment-asset-notes/${storedName}`,req.user!.id,timestamp,sourceType,sha256]);}
    await regenerateEquipmentAssetNotePdf(noteId);
    if(warning)recordAssetNoteLifecycleEvent({library:'equipment',noteId,type:'issue_created',actor:req.user!,newValue:assetNoteHistorySnapshot(equipmentAssetNoteById(noteId)!),createdAt:timestamp});
    recordEquipmentHistory({action:'note_created',actor:req.user!,row:asset,newValue:{noteId,title,noteDate,warning,workOrder,attachmentCount:files.length}});scheduleAutoBackup('equipment note created',req.user!);res.status(201).json({ok:true,note:publicEquipmentAssetNote(equipmentAssetNoteById(noteId)!,req.user)});
  }catch(error){if(noteId){run('DELETE FROM equipment_asset_note_attachments WHERE note_id=?',[noteId]);run("DELETE FROM asset_note_labor WHERE asset_library='equipment' AND note_id=?",[noteId]);run("DELETE FROM asset_note_lifecycle_events WHERE asset_library='equipment' AND note_id=?",[noteId]);run('DELETE FROM equipment_asset_notes WHERE id=?',[noteId]);}stored.forEach(filePath=>{if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});});res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Equipment note could not be created.')});}
});
app.put('/api/equipment-library/asset-notes/:noteId',requireAuth,requirePermission('equipment.write'),receiveMachineAssetNote,async(req:AuthRequest,res)=>{
  const stored:string[]=[];const attachmentIds:number[]=[];let previous:MachineAssetNoteRow|undefined;let previousLabor:AssetNoteLaborInput[]=[];let previousSnapshot:ReturnType<typeof assetNoteHistorySnapshot>|null=null;
  try{
    const note=equipmentAssetNoteById(Number(req.params.noteId));if(!note)return res.status(404).json({ok:false,error:'Equipment asset note not found.'});
    if(!canEditAssetNoteIssue(req.user!,'equipment',note))return res.status(403).json({ok:false,error:'You may edit only warning issues that you created.',code:'ISSUE_OWNER_REQUIRED'});previous=note;previousLabor=assetNoteLabor('equipment',note.id).map((row,index)=>({userId:row.user_id??req.user!.id,displayName:row.display_name,hours:Number(row.labor_hours),isPrimary:Boolean(row.is_primary),order:index}));previousSnapshot=assetNoteHistorySnapshot(note);
    const asset=equipmentAssetById(note.asset_id)!;const title=String(req.body?.title??'').replace(/\s+/g,' ').trim().slice(0,180);const body=String(req.body?.body??'').replace(/\r/g,'').trim().slice(0,30000);if(!title||!body)throw new Error('Note Title and Note Body are required.');
    const noteDate=validMachineAssetNoteDate(req.body?.noteDate);const warning=assetNoteWarningValue(req.body?.warning,Boolean(note.is_warning));const hold=warning&&assetNoteWarningValue(req.body?.hold,Boolean(note.is_on_hold));if(note.is_warning&&!warning)throw new Error('Resolve the warning issue instead of clearing Warning / Needs Attention.');const workOrder=validateAssetNoteWorkOrder(req.body?.workOrder??req.body?.workOrderReference,warning);const labor=parseAssetNoteLabor(req.body?.laborEntries,req.user!,false);const timestamp=now();
    run('UPDATE equipment_asset_notes SET title=?,note_date=?,body=?,is_warning=?,is_on_hold=?,work_order_reference=?,issue_status=?,updated_by_user_id=?,updated_at=? WHERE id=?',[title,noteDate,body,warning?1:0,hold?1:0,workOrder,note.is_warning?note.issue_status:'active',req.user!.id,timestamp,note.id]);
    if(labor)replaceAssetNoteLabor('equipment',note.id,labor,timestamp);const files=(req.files as Express.Multer.File[]|undefined)??[];const sources=assetNoteAttachmentSources(req.body?.attachmentSources,files.length);const seenHashes=new Set<string>();for(let index=0;index<files.length;index+=1){const file=files[index];const detected=validatedAssetNoteAttachment(file,sources[index]);const sha256=assetNoteFileHash(file);if(seenHashes.has(sha256)||assetNoteAttachmentExists('equipment',note.id,sha256))continue;seenHashes.add(sha256);const storedName=`${crypto.randomUUID()}${detected.extension}`;const storedPath=path.join(equipmentAssetNotesDir,storedName);fs.writeFileSync(storedPath,file.buffer,{flag:'wx'});stored.push(storedPath);const result=run('INSERT INTO equipment_asset_note_attachments (note_id,original_filename,mime_type,file_size,stored_file_reference,uploaded_by_user_id,created_at,source_type,sha256) VALUES (?,?,?,?,?,?,?,?,?)',[note.id,safeMachineAssetNoteOriginalName(file.originalname,detected.extension),detected.mimeType,file.size,`uploads/equipment-asset-notes/${storedName}`,req.user!.id,timestamp,sources[index],sha256]);attachmentIds.push(Number(result.lastInsertRowid));}
    await regenerateEquipmentAssetNotePdf(note.id);
    recordAssetNoteHoldChange('equipment',note.id,Boolean(note.is_on_hold),hold,req.user!,timestamp);
    if(warning)recordAssetNoteLifecycleEvent({library:'equipment',noteId:note.id,type:note.is_warning?'issue_edited':'warning_enabled',actor:req.user!,oldValue:previousSnapshot??assetNoteHistorySnapshot(note),newValue:assetNoteHistorySnapshot(equipmentAssetNoteById(note.id)!),createdAt:timestamp});
    recordEquipmentHistory({action:'note_edited',actor:req.user!,row:asset,oldValue:{noteId:note.id,title:note.title,noteDate:note.note_date,warning:Boolean(note.is_warning),hold:Boolean(note.is_warning&&note.is_on_hold),workOrder:note.work_order_reference},newValue:{noteId:note.id,title,noteDate,warning,hold,workOrder}});scheduleAutoBackup('equipment note edited',req.user!);res.json({ok:true,note:publicEquipmentAssetNote(equipmentAssetNoteById(note.id)!,req.user)});
  }catch(error){for(const id of attachmentIds)run('DELETE FROM equipment_asset_note_attachments WHERE id=?',[id]);stored.forEach(filePath=>{if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});});if(previous){run('UPDATE equipment_asset_notes SET title=?,note_date=?,body=?,is_warning=?,is_on_hold=?,work_order_reference=?,issue_status=?,updated_by_user_id=?,updated_at=? WHERE id=?',[previous.title,previous.note_date,previous.body,previous.is_warning,previous.is_on_hold,previous.work_order_reference,previous.issue_status,previous.updated_by_user_id,previous.updated_at,previous.id]);replaceAssetNoteLabor('equipment',previous.id,previousLabor,previous.updated_at);}res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Equipment note could not be updated.')});}
});
app.delete('/api/equipment-library/asset-notes/:noteId',requireAuth,requirePermission('equipment.write'),(req:AuthRequest,res)=>{
  const note=equipmentAssetNoteById(Number(req.params.noteId));if(!note)return res.status(404).json({ok:false,error:'Equipment asset note not found.'});
  if(!canDeleteAssetNoteIssue(req.user!,'equipment',note))return res.status(403).json({ok:false,error:'Only a Manager or Admin may delete a warning issue.',code:'ISSUE_MANAGER_REQUIRED'});
  const asset=equipmentAssetById(note.asset_id)!;
  if(note.is_warning){try{const reason=requiredAssetNoteReason(isRecord(req.body)?req.body.deleteReason??req.body.reason:'','Delete Reason',2000);const timestamp=now();db.exec('BEGIN IMMEDIATE');try{run('UPDATE equipment_asset_notes SET deleted=1,deleted_at=?,deleted_by_user_id=?,deleted_by_name=?,delete_reason=?,updated_by_user_id=?,updated_at=? WHERE id=? AND deleted=0',[timestamp,req.user!.id,req.user!.full_name,reason,req.user!.id,timestamp,note.id]);recordAssetNoteLifecycleEvent({library:'equipment',noteId:note.id,type:'issue_deleted',actor:req.user!,reason,oldValue:assetNoteHistorySnapshot(note),newValue:{deleted:true,historyPreserved:true,attachmentsPreserved:true},createdAt:timestamp});db.exec('COMMIT');}catch(error){db.exec('ROLLBACK');throw error;}recordEquipmentHistory({action:'warning_issue_deleted',actor:req.user!,row:asset,oldValue:assetNoteHistorySnapshot(note),newValue:{historyPreserved:true},reasonNote:reason});audit(req,'warning issue delete','equipment_asset_note',note.id,{library:'equipment',reason,historyPreserved:true,attachmentsPreserved:true});scheduleAutoBackup('equipment warning issue deleted',req.user!);return res.json({ok:true,historyPreserved:true});}catch(error){return res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Warning issue could not be deleted.')});}}
  const attachments=equipmentAssetNoteAttachments(note.id);run('DELETE FROM equipment_asset_note_attachments WHERE note_id=?',[note.id]);run("DELETE FROM asset_note_labor WHERE asset_library='equipment' AND note_id=?",[note.id]);run('DELETE FROM equipment_asset_notes WHERE id=?',[note.id]);for(const item of attachments){const filePath=equipmentAssetNoteFilePath(item.stored_file_reference);if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});}if(note.pdf_stored_reference){const pdfPath=equipmentAssetNoteFilePath(note.pdf_stored_reference);if(fs.existsSync(pdfPath))fs.rmSync(pdfPath,{force:true});}recordEquipmentHistory({action:'note_deleted',actor:req.user!,row:asset,oldValue:{noteId:note.id,title:note.title}});scheduleAutoBackup('equipment note deleted',req.user!);res.json({ok:true});
});
app.delete('/api/equipment-library/asset-note-attachments/:attachmentId',requireAuth,requirePermission('equipment.write'),async(req:AuthRequest,res)=>{
  const attachment=one<MachineAssetNoteAttachmentRow>('SELECT * FROM equipment_asset_note_attachments WHERE id=?',[Number(req.params.attachmentId)]);if(!attachment)return res.status(404).json({ok:false,error:'Equipment note attachment not found.'});const note=equipmentAssetNoteById(attachment.note_id);if(!note)return res.status(404).json({ok:false,error:'Equipment asset note not found.'});if(!canEditAssetNoteIssue(req.user!,'equipment',note))return res.status(403).json({ok:false,error:'You may remove attachments only from warning issues that you may edit.',code:'ISSUE_OWNER_REQUIRED'});if(note.is_warning&&note.issue_status==='resolved')return res.status(409).json({ok:false,error:'Resolved issue attachments are preserved. Reopen the issue before changing attachments.',code:'ISSUE_RESOLVED'});const asset=equipmentAssetById(note.asset_id)!;run('DELETE FROM equipment_asset_note_attachments WHERE id=?',[attachment.id]);const filePath=equipmentAssetNoteFilePath(attachment.stored_file_reference);if(fs.existsSync(filePath))fs.rmSync(filePath,{force:true});await regenerateEquipmentAssetNotePdf(note.id);if(note.is_warning){recordAssetNoteLifecycleEvent({library:'equipment',noteId:note.id,type:'attachment_deleted',actor:req.user!,newValue:{attachmentId:attachment.id,filename:attachment.original_filename}});audit(req,'warning issue attachment delete','equipment_asset_note',note.id,{attachmentId:attachment.id,filename:attachment.original_filename});}recordEquipmentHistory({action:'note_attachment_deleted',actor:req.user!,row:asset,oldValue:{attachmentId:attachment.id,filename:attachment.original_filename}});scheduleAutoBackup('equipment note attachment deleted',req.user!);res.json({ok:true,note:publicEquipmentAssetNote(equipmentAssetNoteById(note.id)!,req.user)});
});
...
function assetNoteStoredFilePath(library:AssetLibrary,storedReference:string) {
  return library==='machine'?machineAssetNoteFilePath(storedReference):equipmentAssetNoteFilePath(storedReference);
}
...
function workOrderRecordSource(library:WorkOrderRecordLibrary,row:MachineAssetNoteRow):WorkOrderRecordSource {
  const updates=assetNoteUpdates(library,row.id);const labor=publicAssetNoteLabor(library,row.id);let pdfPath='';let pdfError='';if(row.pdf_stored_reference){try{pdfPath=assetNoteStoredFilePath(library,row.pdf_stored_reference);}catch(error){pdfError=safeErrorMessage(error,[],'Generated note PDF reference is invalid.');}}let pdfSize=0;try{pdfSize=pdfPath?fs.statSync(pdfPath).size:0;}catch{}
  return {
    library,
...
function warningIssueRows(library:AssetLibrary) {
...
for(const library of ['machine','equipment'] as const){
  ...
  const viewPermission=assetNoteViewPermission(library);
  app.get(`${base}/asset-note-technicians`,requireAuth,requirePermission(assetNoteManagePermission(library)),(req:AuthRequest,res)=>{
    const query=String(req.query.q??'').trim().slice(0,120).toLocaleLowerCase();const technicians=all<User>('SELECT * FROM users WHERE disabled=0 AND deleted=0 ORDER BY full_name COLLATE NOCASE,id').filter(user=>!query||`${user.full_name}\n${user.email}\n${user.role}`.toLocaleLowerCase().includes(query)).slice(0,50).map(user=>({id:user.id,fullName:user.full_name,email:user.email,role:user.role,isCurrentUser:user.id===req.user!.id}));res.json({ok:true,technicians});
  });
  app.get(`${base}/warning-issues`,requireAuth,requirePermission(viewPermission),(req:AuthRequest,res)=>{
    const query=String(req.query.q??req.query.search??'').trim().slice(0,200).toLocaleLowerCase();
    const status=String(req.query.status??'all').toLowerCase();
    const year=String(req.query.year??'').trim();
    if(!['all','active','hold','resolved'].includes(status))return res.status(400).json({ok:false,error:'Status must be active, hold, resolved, or all.'});
    if(year&&!/^\d{4}$/.test(year))return res.status(400).json({ok:false,error:'Resolved year must use four digits.'});
    const issues=warningIssueRows(library).map(row=>publicAssetNote(row,library,req.user)).filter(issue=>(status==='all'||(status==='hold'?issue.status==='active'&&issue.hold:status==='active'?issue.status==='active'&&!issue.hold:issue.status===status))&&(!year||issue.resolvedYear===year)&&(!query||assetNoteSearchText(issue).includes(query)));
  ...
  app.post(`${base}/asset-notes/:noteId/resolve`,requireAuth,requirePermission(viewPermission),async(req:AuthRequest,res)=>{
    try{
      ...
      const summary=requiredAssetNoteReason(isRecord(req.body)?req.body.resolutionSummary??req.body.reason:'','Resolution Summary',4000);const timestamp=now();const table=library==='machine'?'machine_asset_notes':'equipment_asset_notes';
      db.exec('BEGIN IMMEDIATE');try{run(`UPDATE ${table} SET issue_status='resolved',resolved_at=?,resolved_by_user_id=?,resolved_by_name=?,resolution_summary=?,updated_by_user_id=?,updated_at=? WHERE id=? AND deleted=0`,[timestamp,req.user!.id,req.user!.full_name,summary,req.user!.id,timestamp,note.id]);const updated=assetNoteByLibraryId(library,note.id)!;recordAssetNoteLifecycleEvent({library,noteId:note.id,type:'issue_resolved',actor:req.user!,reason:summary,oldValue:assetNoteHistorySnapshot(note),newValue:assetNoteHistorySnapshot(updated),createdAt:timestamp});db.exec('COMMIT');}catch(error){db.exec('ROLLBACK');throw error;}
      await regenerateAssetNotePdf(library,note.id);audit(req,'warning issue resolve',`${library}_asset_note`,note.id,{resolutionSummary:summary});scheduleAutoBackup(`${library} warning issue resolved`,req.user!);res.json({ok:true,note:publicAssetNote(assetNoteByLibraryId(library,note.id)!,library,req.user)});
    }catch(error){res.status(400).json({ok:false,error:safeErrorMessage(error,[],'Warning issue could not be resolved.')});}
  });
  ...
  app.get(`${base}/asset-note-update-attachments/:attachmentId/file`,requireAuth,requirePermission(viewPermission),(req:AuthRequest,res)=>{
    try{
      const attachment=one<AssetNoteUpdateAttachmentRow>('SELECT * FROM asset_note_update_attachments WHERE id=?',[Number(req.params.attachmentId)]);if(!attachment)return res.status(404).json({ok:false,error:'Maintenance update attachment not found.'});const update=one<AssetNoteUpdateRow>('SELECT * FROM asset_note_updates WHERE id=? AND asset_library=?',[attachment.update_id,library]);if(!update||!assetNoteByLibraryId(library,update.note_id))return res.status(404).json({ok:false,error:'Maintenance update attachment not found.'});const filePath=assetNoteStoredFilePath(library,attachment.stored_file_reference);if(!fs.existsSync(filePath))return res.status(404).json({ok:false,error:'Stored maintenance update attachment is missing.'});const extension=path.extname(attachment.original_filename).toLowerCase();const disposition=String(req.query.download)==='true'||['.doc','.docx'].includes(extension)?'attachment':'inline';res.setHeader('Content-Type',attachment.mime_type||'application/octet-stream');res.setHeader('Content-Length',String(fs.statSync(filePath).size));res.setHeader('X-Content-Type-Options','nosniff');res.setHeader('Cache-Control','private, no-store');res.setHeader('Content-Disposition',`${disposition}; filename="${safeMachineAssetNoteOriginalName(attachment.original_filename,extension)}"`);res.sendFile(filePath);
...
app.get('/api/settings/network-links', requireAuth, requirePermission('settings.view'), (_req,res)=>{
  if (canonicalHttps) {
    res.json({
      accessMode: 'https',
      canonicalUrl: canonicalHttps.url,
      certificateMode: canonicalHttps.certificateMode,
    });
    return;
  }
...
app.patch('/api/inventory/native/parts/:id', requireAuth, requirePermission('inventory.edit'), (req,res)=>{
  ...
  try {
    ...
    try {
      ...
      const quantityBefore = Number(existing.quantity ?? 0);
      const quantityAfter = Number(input.quantity ?? 0);
      inventoryAudit(req,'native part edit','part',partId,{partNumber:input.partNumber,obsolete:Boolean(obsolete),locationAutoCreated:location.created,vendorAutoCreated:vendor.created});
      audit(req,'inventory native part edit','inventory',partId,{partNumber:input.partNumber,obsolete:Boolean(obsolete)});
      const obsoleteChanged = Boolean(existing.obsolete ?? 0) !== Boolean(obsolete);
      recordInventoryPartHistory({
        action: obsoleteChanged ? (obsolete ? 'marked_obsolete' : 'obsolete_cleared') : quantityBefore !== quantityAfter ? 'quantity_changed' : 'updated',
        actor,
        partId,
        row: updatedRow,
        oldValue: nativePartHistoryValue(existing),
