    ...
    for (const update of input.trackerUpdates) {
      ...
      if(!row){
        ...
        if(section){
          ...
          const canonicalAsset=canonicalTrackerAsset(update.assetNumber,input.assetChoices);const sections=trackerMachineSections(trackerView,trackerHeader);const template=trackerSectionTemplate(sections,update.intervalType);const oldLastRow=trackerView.rowCount;const lastTaskRow=Math.max(...sections.flatMap(section=>section.taskRows.map(task=>task.rowNumber)));const headingRow=Math.max(oldLastRow+1,lastTaskRow+4);const headerRow=headingRow+1;row=headerRow+1;templateRow=template.task.rowNumber;sectionAssetNumber=canonicalAsset;inheritedAssetNumber=template.task.assetNumberInherited;const formulaTemplates=await worksheetFormulaTemplates(zip,trackerPart,trackerView,templateRow);
