...
type TrackerMachineSection = {assetNumber:string;headingRow:number;headerRow:number|null;endRow:number;taskRows:ParsedTrackerRow[];malformedReason:string|null};
...
function rowSliceForSqref(value:string,templateRow:number,targetRow:number){
  const match=/^(\$?[A-Z]{1,3})\$?(\d+)(?::(\$?[A-Z]{1,3})\$?(\d+))?$/i.exec(value);if(!match||templateRow<Number(match[2])||templateRow>Number(match[4]??match[2]))return null;return match[3]?`${match[1]}${targetRow}:${match[3]}${targetRow}`:`${match[1]}${targetRow}`;
}
function clonedRowSqref(value:string,templateRow:number,targetRow:number){return value.split(/\s+/).filter(Boolean).map(item=>rowSliceForSqref(item,templateRow,targetRow)).filter((item):item is string=>Boolean(item)).join(' ');}
function translateXmlFormulaNodes(entry:string,delta:number){return entry.replace(/<(formula[12]|formula)\b([^>]*)>([\s\S]*?)<\/\1>/gi,(_match,name,attributes,formula)=>`<${name}${attributes}>${formulaXmlEscape(shiftFormulaRows(decodeXml(formula),delta))}</${name}>`);}
function cloneWorksheetRowMetadata(xml:string,templateRow:number,targetRow:number){
  const delta=targetRow-templateRow;
  const validationContainer=/<dataValidations\b[^>]*>[\s\S]*?<\/dataValidations>/.exec(xml)?.[0];
  if(validationContainer){const entries=[...validationContainer.matchAll(/<dataValidation\b[^>]*?(?:\/>|>[\s\S]*?<\/dataValidation>)/g)].map(match=>match[0]);const clones=entries.map(entry=>{const opening=entry.match(/^<dataValidation\b[^>]*>/)?.[0]??entry;const sqref=decodeXml(xmlAttribute(opening,'sqref'));const target=clonedRowSqref(sqref,templateRow,targetRow);return target?translateXmlFormulaNodes(entry.replace(/\bsqref="[^"]*"/,`sqref="${target}"`),delta):'';}).filter(Boolean);if(clones.length)xml=xml.replace(validationContainer,validationContainer.replace(/(<dataValidations\b[^>]*\bcount=")\d+("[^>]*>)/,(_match,before,after)=>`${before}${entries.length+clones.length}${after}`).replace('</dataValidations>',`${clones.join('')}</dataValidations>`));}
  xml=xml.replace(/<conditionalFormatting\b[^>]*>[\s\S]*?<\/conditionalFormatting>/g,entry=>{const opening=entry.match(/^<conditionalFormatting\b[^>]*>/)?.[0]??entry;const target=clonedRowSqref(decodeXml(xmlAttribute(opening,'sqref')),templateRow,targetRow);return target?`${entry}${translateXmlFormulaNodes(entry.replace(/\bsqref="[^"]*"/,`sqref="${target}"`),delta)}`:entry;});
  xml=xml.replace(/(<ignoredError\b[^>]*\bsqref=")([^"]+)("[^>]*\/?>)/g,(entry,before,sqref,after)=>{const target=clonedRowSqref(decodeXml(sqref),templateRow,targetRow);return target?`${before}${sqref} ${target}${after}`:entry;});
  return xml;
}
function cloneSameRowMerges(xml:string,templateRow:number,targetRow:number){
  const refs:string[]=[];for(const match of xml.matchAll(/<mergeCell\b[^>]*\bref="([^"]+)"[^>]*\/>/g)){const range=/^(\$?[A-Z]{1,3})\$?(\d+):(\$?[A-Z]{1,3})\$?(\d+)$/i.exec(match[1]);if(range&&Number(range[2])===templateRow&&Number(range[4])===templateRow)refs.push(`${range[1]}${targetRow}:${range[3]}${targetRow}`);}if(!refs.length)return xml;
  if(/<mergeCells\b/.test(xml))return xml.replace(/<mergeCells\b([^>]*)\bcount="(\d+)"([^>]*)>/,(_match,before,count,after)=>`<mergeCells${before}count="${Number(count)+refs.length}"${after}>`).replace('</mergeCells>',`${refs.map(ref=>`<mergeCell ref="${ref}"/>`).join('')}</mergeCells>`);
  const insertion=xml.search(/<(?:conditionalFormatting|dataValidations|hyperlinks|printOptions|pageMargins|pageSetup|headerFooter|drawing|legacyDrawing|tableParts|extLst)\b|<\/worksheet>/);if(insertion<0)throw new Error('Machine Pm Tracker merge location is unavailable.');return `${xml.slice(0,insertion)}<mergeCells count="${refs.length}">${refs.map(ref=>`<mergeCell ref="${ref}"/>`).join('')}</mergeCells>${xml.slice(insertion)}`;
}
function appendWorksheetRow(xml:string,targetRow:number,templateRow:number,formulaTemplates:Map<number,string>,cloneMetadata=false){
  if(rowXmlPattern(targetRow).test(xml))throw new Error(`Machine Pm Tracker target row ${targetRow} is already occupied.`);const template=rowXmlPattern(templateRow).exec(xml)?.[0];if(!template)throw new Error(`Machine Pm Tracker formatting row ${templateRow} is unavailable.`);
  for(const match of xml.matchAll(/<mergeCell\b[^>]*\bref="([^"]+)"[^>]*\/>/g)){const range=/^(?:\$?[A-Z]{1,3})\$?(\d+):(?:\$?[A-Z]{1,3})\$?(\d+)$/i.exec(match[1]);if(range&&Number(range[1])!==Number(range[2])&&Number(range[1])<=templateRow&&Number(range[2])>=templateRow)throw new Error('Machine Pm Tracker formatting template uses a vertical merged range that cannot be copied safely.');}
  const cloned=blankTemplateRowValues(shiftedTemplateRow(template,templateRow,targetRow));const sheetDataEnd=xml.indexOf('</sheetData>');if(sheetDataEnd<0)throw new Error('Machine Pm Tracker worksheet data is unreadable.');xml=`${xml.slice(0,sheetDataEnd)}${cloned}${xml.slice(sheetDataEnd)}`;xml=extendWorksheetDimension(xml,targetRow);xml=cloneSameRowMerges(xml,templateRow,targetRow);if(cloneMetadata)xml=cloneWorksheetRowMetadata(xml,templateRow,targetRow);
  for(const [column,formula] of formulaTemplates)xml=setWorksheetStandaloneFormula(xml,`${cellColumnLetters(column)}${targetRow}`,shiftFormulaRows(formula,targetRow-templateRow),`${cellColumnLetters(column)}${templateRow}`);return xml;
}
function extendWorksheetDimension(xml:string,rowNumber:number){return xml.replace(/<dimension\b([^>]*)\bref="([^"]+)"([^>]*)\/>/i,(tag,before,ref,after)=>{const range=/^([A-Z]+)(\d+)(?::([A-Z]+)(\d+))?$/i.exec(ref);if(!range)return tag;const endColumn=range[3]??range[1];const endRow=Number(range[4]??range[2]);return rowNumber>endRow?`<dimension${before}ref="${range[1]}${range[2]}:${endColumn}${rowNumber}"${after}/>`:tag;});}

function trackerHeadingTemplateValues(sheet:ExcelJS.Worksheet,rowNumber:number,header:HeaderMap<TrackerField>,canonicalAsset:string,assetName?:string):Array<[number,string]>{
  const labels=new Set(['press','pressnumber','pressno','machine','machinenumber','machineno','asset','assetnumber','assetno']);const task=cleanText(cellRawValue(sheet.getCell(rowNumber,header.columns.taskTitle)));const interval=cleanText(cellRawValue(sheet.getCell(rowNumber,header.columns.intervalType)));const explicitColumn=header.columns.assetNumber;const explicit=explicitColumn?cleanText(cellRawValue(sheet.getCell(rowNumber,explicitColumn))):'';
  const values:Array<{column:number;text:string}>=[];for(let column=1;column<=Math.min(Math.max(sheet.columnCount,1),100);column+=1){const text=cleanText(cellRawValue(sheet.getCell(rowNumber,column)));if(text)values.push({column,text});}
  let result:Array<[number,string]>|null=explicit&&!task&&!interval&&!labels.has(normalizedHeader(explicit))?[[explicitColumn,canonicalAsset]]:null;
  if(!result)for(const item of values){const combined=/^((?:press|machine|asset)(?:\s*(?:number|no|#))?\s*:)\s*(.+)$/i.exec(item.text);if(combined){result=[[item.column,`${combined[1]} ${canonicalAsset}`]];break;}if(!labels.has(normalizedHeader(item.text)))continue;const next=values.find(candidate=>candidate.column===item.column+1&&!labels.has(normalizedHeader(candidate.text)));if(next){result=[[item.column,item.text],[next.column,canonicalAsset]];break;}}
  if(result&&assetName){const used=new Set(result.map(([column])=>column));const nameColumn=header.columns.assetName&&!used.has(header.columns.assetName)?header.columns.assetName:values.find(item=>!used.has(item.column)&&!labels.has(normalizedHeader(item.text)))?.column;if(nameColumn)result.push([nameColumn,assetName]);}
  if(result)return result;
  throw new Error('A safe machine/press heading template could not be populated; the workbook was not changed.');
}
function extendDefinedNameRanges(formula:string,oldLastRow:number,targetLastRow:number){return formula.replace(/(\$?[A-Z]{1,3}\$?\d+):(\$?[A-Z]{1,3})(\$?)(\d+)/gi,(entry,start,endColumn,absolute,endRow)=>Number(endRow)===oldLastRow?`${start}:${endColumn}${absolute}${targetLastRow}`:entry);}
async function extendTrackerDefinedNamesForAppend(zip:JSZip,sheetName:string,sheetIndex:number,oldLastRow:number,targetLastRow:number){const file=zip.file('xl/workbook.xml');if(!file)return;let xml=await file.async('string');const escapedName=sheetName.replace(/'/g,"''");xml=xml.replace(/<definedName\b([^>]*)>([\s\S]*?)<\/definedName>/g,(entry,attributes,formula)=>{const local=/\blocalSheetId="(\d+)"/.exec(attributes);const qualified=new RegExp(`(?:'${regexEscape(escapedName)}'|${regexEscape(sheetName)})!`,'i').test(decodeXml(formula));if((local&&Number(local[1])===sheetIndex)||qualified)return `<definedName${attributes}>${extendDefinedNameRanges(formula,oldLastRow,targetLastRow)}</definedName>`;return entry;});zip.file('xl/workbook.xml',xml);}

function setXmlAttribute(tag:string,name:string,value:string){const encoded=xmlEscape(value);return new RegExp(`\\b${regexEscape(name)}="[^"]*"`).test(tag)?tag.replace(new RegExp(`\\b${regexEscape(name)}="[^"]*"`),`${name}="${encoded}"`):tag.replace(/\/?\s*>$/,match=>` ${name}="${encoded}"${match}`);}
async function cloneTrackerSectionTable(zip:JSZip,sheetPart:string,worksheetXml:string,templateTaskRow:number,targetHeaderRow:number,targetTaskRow:number){
  const relPart=relationshipPart(sheetPart);let relationships=await zip.file(relPart)?.async('string');if(!relationships)return worksheetXml;let source:{part:string;xml:string;ref:string;startColumn:string;endColumn:string}|null=null;const tableFiles=new Map<string,string>();
  for(const match of relationships.matchAll(/<Relationship\b[^>]*\/?\s*>/g)){const tag=match[0];if(!/\/table"?$/i.test(xmlAttribute(tag,'Type')))continue;const target=xmlAttribute(tag,'Target');if(!target)continue;const part=relationshipTarget(sheetPart,decodeXml(target));const file=zip.file(part);if(!file)continue;const xml=await file.async('string');tableFiles.set(part,xml);const tableTag=/<table\b[^>]*>/.exec(xml)?.[0]??'';const ref=xmlAttribute(tableTag,'ref');const range=/^\$?([A-Z]{1,3})\$?(\d+):\$?([A-Z]{1,3})\$?(\d+)$/i.exec(ref);if(range&&templateTaskRow>Number(range[2])&&templateTaskRow<=Number(range[4]))source={part,xml,ref,startColumn:range[1],endColumn:range[3]};}
  if(!source)return worksheetXml;for(const name of Object.keys(zip.files).filter(name=>/^xl\/tables\/table\d+\.xml$/i.test(name))){if(tableFiles.has(name))continue;const file=zip.file(name);if(file)tableFiles.set(name,await file.async('string'));}
  const ids=[...tableFiles.values()].map(xml=>Number(xmlAttribute(/<table\b[^>]*>/.exec(xml)?.[0]??'','id'))).filter(Number.isFinite);const nextId=Math.max(0,...ids)+1;let partNumber=1;while(zip.file(`xl/tables/table${partNumber}.xml`))partNumber+=1;const newPart=`xl/tables/table${partNumber}.xml`;const names=new Set([...tableFiles.values()].flatMap(xml=>{const tag=/<table\b[^>]*>/.exec(xml)?.[0]??'';return [xmlAttribute(tag,'name'),xmlAttribute(tag,'displayName')].filter(Boolean);}));let tableName=`MCCPMSection${nextId}`;let suffix=2;while(names.has(tableName)){tableName=`MCCPMSection${nextId}_${suffix}`;suffix+=1;}const targetRef=`${source.startColumn}${targetHeaderRow}:${source.endColumn}${targetTaskRow}`;
  let cloned=source.xml.replace(/<table\b[^>]*>/,tag=>setXmlAttribute(setXmlAttribute(setXmlAttribute(setXmlAttribute(tag,'id',String(nextId)),'name',tableName),'displayName',tableName),'ref',targetRef));cloned=cloned.replace(new RegExp(`ref="${regexEscape(source.ref)}"`,'g'),`ref="${targetRef}"`).replace(/<calculatedColumnFormula>([\s\S]*?)<\/calculatedColumnFormula>/g,(_entry,formula)=>`<calculatedColumnFormula>${formulaXmlEscape(shiftFormulaRows(decodeXml(formula),targetTaskRow-templateTaskRow))}</calculatedColumnFormula>`);zip.file(newPart,cloned);
  const relationshipIds=new Set([...relationships.matchAll(/<Relationship\b[^>]*\bId="([^"]+)"[^>]*\/?\s*>/g)].map(match=>match[1]));let relationshipNumber=1;while(relationshipIds.has(`rId${relationshipNumber}`))relationshipNumber+=1;const relationshipId=`rId${relationshipNumber}`;relationships=relationships.replace('</Relationships>',`<Relationship Id="${relationshipId}" Type="http://schemas.openxmlformats.org/officeDocument/2006/relationships/table" Target="${xmlEscape(path.posix.relative(path.posix.dirname(sheetPart),newPart))}"/></Relationships>`);zip.file(relPart,relationships);
  const contentTypes=zip.file('[Content_Types].xml');if(contentTypes){let xml=await contentTypes.async('string');xml=xml.replace('</Types>',`<Override PartName="/${newPart}" ContentType="application/vnd.openxmlformats-officedocument.spreadsheetml.table+xml"/></Types>`);zip.file('[Content_Types].xml',xml);}
  if(!/\bxmlns:r="http:\/\/schemas\.openxmlformats\.org\/officeDocument\/2006\/relationships"/.test(worksheetXml))worksheetXml=worksheetXml.replace(/<worksheet\b/,`<worksheet xmlns:r="http://schemas.openxmlformats.org/officeDocument/2006/relationships"`);const tablePart=`<tablePart r:id="${relationshipId}"/>`;if(/<tableParts\b/.test(worksheetXml))worksheetXml=worksheetXml.replace(/<tableParts\b([^>]*)\bcount="(\d+)"([^>]*)>/,(_match,before,count,after)=>`<tableParts${before}count="${Number(count)+1}"${after}>`).replace('</tableParts>',`${tablePart}</tableParts>`);else{const insertion=worksheetXml.search(/<extLst\b|<\/worksheet>/);if(insertion<0)throw new Error('Machine Pm Tracker table location is unavailable.');worksheetXml=`${worksheetXml.slice(0,insertion)}<tableParts count="1">${tablePart}</tableParts>${worksheetXml.slice(insertion)}`;}return worksheetXml;
}
...
function trackerIdentityMatches(rowsByKey:ReturnType<typeof trackerRowsByKey>,assetNumber:string,taskTitle:string,intervalType:WorkbookPmInterval) {
  const taskKey=[normalizedPmKey(taskTitle),intervalType].join('\u001f');const exact=rowsByKey.exact.get([normalizedPmKey(assetNumber),taskKey].join('\u001f'))??[];
  if(exact.length)return {rows:exact,matchType:'exact' as const};
  const alias=strictPressNumberAlias(assetNumber);const aliases=alias?(rowsByKey.aliases.get([alias,taskKey].join('\u001f'))??[]):[];
  return {rows:aliases,matchType:alias?'press alias' as const:'none' as const};
}

function trackerRowNumbers(rows:readonly number[]){return rows.length?rows.join(', '):'none';}
function ambiguousTrackerMatchError(assetNumber:string,taskTitle:string,match:{rows:number[];matchType:'exact'|'press alias'|'none'}){
  return new Error(`Ambiguous Machine Pm Tracker match for ${assetNumber} / ${taskTitle}: match count ${match.rows.length} using ${match.matchType}; row numbers ${trackerRowNumbers(match.rows)}. The workbook was not changed.`);
}

function synchronizedTrackerMatches(parsed:ParsedPmWorkbook,assetNumber:string,taskTitle:string,intervalType:WorkbookPmInterval){
  const candidates=parsed.trackerRows.filter(row=>row.intervalType===intervalType&&normalizedPmKey(row.taskTitle)===normalizedPmKey(taskTitle));const exact=candidates.filter(row=>normalizedPmKey(row.assetNumber)===normalizedPmKey(assetNumber));const alias=strictPressNumberAlias(assetNumber);const aliases=alias?candidates.filter(row=>strictPressNumberAlias(row.assetNumber)===alias):[];const resolved=exact.length?exact:aliases;return {candidates,exact,aliases,resolved,matchType:exact.length?'exact':alias?'press alias':'none'};
}

export function validateSynchronizedTrackerUpdates(parsed:ParsedPmWorkbook,updates:readonly TrackerWorkbookUpdate[]){
  for(const update of updates){
    const match=synchronizedTrackerMatches(parsed,update.assetNumber,update.taskTitle,update.intervalType);if(match.resolved.length===1)continue;
    const rejected=parsed.rejectedRows.filter(row=>row.sheet===PM_TRACKER_SHEET).map(row=>`${row.rowNumber} (${row.reason})`);throw new Error(`Workbook validation failed for ${update.assetNumber} / ${update.taskTitle}: synchronized tracker match count ${match.resolved.length} using ${match.matchType} (expected 1); matching row numbers ${trackerRowNumbers(match.resolved.map(row=>row.rowNumber))}; task/interval candidate rows ${trackerRowNumbers(match.candidates.map(row=>row.rowNumber))}; rejected tracker rows ${rejected.length?rejected.join('; '):'none'}.`);
  }
}
...
function trackerMachineSections(sheet:ExcelJS.Worksheet,header:HeaderMap<TrackerField>) {
  ...
  for(let index=0;index<headings.length;index+=1){
    ...
    sections.push({assetNumber:heading.assetNumber,headingRow:heading.rowNumber,headerRow:repeatedHeaders[0]??null,endRow,taskRows,malformedReason});
  }
  return sections;
}

function findTrackerMachineSection(sheet:ExcelJS.Worksheet,header:HeaderMap<TrackerField>,assetNumber:string) {
  ...
  if(matches.length>1)throw new Error(`Machine/press section is ambiguous for ${assetNumber}; matching heading rows ${trackerRowNumbers(matches.map(section=>section.headingRow))}. The workbook was not changed.`);
...
function canonicalTrackerAsset(assetNumber:string,assetChoices:readonly string[]|undefined){
  if(!assetChoices)throw new Error(`Machine/press section could not be resolved for ${assetNumber}; current MCC asset validation is unavailable, so the workbook was not changed.`);
  const exact=assetChoices.filter(choice=>normalizedPmKey(choice)===normalizedPmKey(assetNumber));if(exact.length===1)return exact[0];if(exact.length>1)throw new Error(`Current MCC asset identifier is ambiguous for ${assetNumber}; the workbook was not changed.`);
  const alias=strictPressNumberAlias(assetNumber);const aliases=alias?assetChoices.filter(choice=>strictPressNumberAlias(choice)===alias):[];if(aliases.length===1)return aliases[0];if(aliases.length>1)throw new Error(`Current MCC press alias is ambiguous for ${assetNumber}; the workbook was not changed.`);
  throw new Error(`Machine/press section could not be resolved for ${assetNumber}, and it is not a unique current MCC asset; the workbook was not changed.`);
}

function trackerSectionTemplate(sections:readonly TrackerMachineSection[],intervalType:WorkbookPmInterval){
  const safe=sections.filter(section=>!section.malformedReason&&section.headerRow!==null&&section.taskRows.length);if(!safe.length)throw new Error('A safe Machine Pm Tracker section template is unavailable; the workbook was not changed.');const meter=intervalType==='hourly'||intervalType==='cycles';const exact=safe.filter(section=>section.taskRows.some(row=>row.intervalType===intervalType));const category=safe.filter(section=>section.taskRows.some(row=>(row.intervalType==='hourly'||row.intervalType==='cycles')===meter));const pool=exact.length?exact:category.length?category:safe;const section=pool[pool.length-1];const exactRows=section.taskRows.filter(row=>row.intervalType===intervalType);const categoryRows=section.taskRows.filter(row=>(row.intervalType==='hourly'||row.intervalType==='cycles')===meter);const rows=exactRows.length?exactRows:categoryRows.length?categoryRows:section.taskRows;return {section,task:rows[rows.length-1]};
}
...
export async function synchronizePmWorkbook(input:{sourcePath:string;destinationPath:string;backupDir:string;trackerUpdates:TrackerWorkbookUpdate[];trackerRemovals?:TrackerWorkbookRemoval[];historyRows:HistoryWorkbookAppend[];assetChoices?:string[];intervalChoices?:WorkbookCatalogChoice[];beforeReplace?:()=>void|Promise<void>;onStage?:(stage:'backing_up'|'replacing_active_workbook'|'verifying')=>void|Promise<void>}) {
  ...
  try {
    ...
    for (const update of input.trackerUpdates) {
      ...
      const meter=update.intervalType==='hourly'||update.intervalType==='cycles';const match=trackerIdentityMatches(rowsByKey,update.assetNumber,update.matchTaskTitle??update.taskTitle,update.matchIntervalType??update.intervalType);
      if(match.rows.length>1)throw ambiguousTrackerMatchError(update.assetNumber,update.taskTitle,match);
      let row=match.rows[0]??0;let inserted=false;let templateRow=0;let sectionAssetNumber='';let inheritedAssetNumber=true;
      if(!row){
        const section=findTrackerMachineSection(trackerView,trackerHeader,update.assetNumber);
        if(section){
          const lastTask=section.taskRows.reduce((latest,item)=>item.rowNumber>latest.rowNumber?item:latest);const exactTemplates=section.taskRows.filter(item=>item.intervalType===update.intervalType);const categoryTemplates=section.taskRows.filter(item=>(item.intervalType==='hourly'||item.intervalType==='cycles')===meter);const template=(exactTemplates.length?exactTemplates:categoryTemplates.length?categoryTemplates:section.taskRows).reduce((latest,item)=>item.rowNumber>latest.rowNumber?item:latest);
          ...
          const canonicalAsset=canonicalTrackerAsset(update.assetNumber,input.assetChoices);const sections=trackerMachineSections(trackerView,trackerHeader);const template=trackerSectionTemplate(sections,update.intervalType);const oldLastRow=trackerView.rowCount;const ordered=[...sections].sort((left,right)=>left.headingRow-right.headingRow);const templateIndex=ordered.findIndex(item=>item.headingRow===template.section.headingRow);let spacerRows=1;if(templateIndex>0){const previous=ordered[templateIndex-1];const previousTask=Math.max(...previous.taskRows.map(item=>item.rowNumber));const observed=template.section.headingRow-previousTask-1;if(observed>=1)spacerRows=Math.min(observed,6);}const headingRow=oldLastRow+spacerRows+1;const headerRow=headingRow+1;row=headerRow+1;templateRow=template.task.rowNumber;sectionAssetNumber=canonicalAsset;inheritedAssetNumber=template.task.assetNumberInherited;const formulaTemplates=await worksheetFormulaTemplates(zip,trackerPart,trackerView,templateRow);
          trackerXml=appendWorksheetRow(trackerXml,headingRow,template.section.headingRow,new Map());for(const [column,value] of trackerHeadingTemplateValues(trackerView,template.section.headingRow,trackerHeader,canonicalAsset,update.assetName)){const patched=setWorksheetCell(trackerXml,`${cellColumnLetters(column)}${headingRow}`,value,date1904,trackerView.getCell(template.section.headingRow,column).address);trackerXml=patched.xml;if(patched.changed)changedCells+=1;}
          trackerXml=appendWorksheetRow(trackerXml,headerRow,template.section.headerRow!,new Map());for(let column=1;column<=Math.min(Math.max(trackerView.columnCount,1),100);column+=1){const value=cellRawValue(trackerView.getCell(template.section.headerRow!,column));if(cleanText(value)==='')continue;const patched=setWorksheetCell(trackerXml,`${cellColumnLetters(column)}${headerRow}`,value,date1904,trackerView.getCell(template.section.headerRow!,column).address);trackerXml=patched.xml;if(patched.changed)changedCells+=1;}
          trackerXml=appendWorksheetRow(trackerXml,row,templateRow,formulaTemplates,true);for(const [column,formula] of formulaTemplates){if(meter&&(column===trackerHeader.columns.lastCompleted||column===trackerHeader.columns.current))continue;formulaExpectations.push({sheetName:PM_TRACKER_SHEET,address:`${cellColumnLetters(column)}${row}`,formula:!meter&&column===trackerHeader.columns.current?'TODAY()':shiftFormulaRows(formula,row-templateRow)});}trackerXml=await cloneTrackerSectionTable(zip,trackerPart,trackerXml,templateRow,headerRow,row);await extendTrackerDefinedNamesForAppend(zip,PM_TRACKER_SHEET,workbook.worksheets.findIndex(sheet=>sheet.name===PM_TRACKER_SHEET),oldLastRow,row);inserted=true;
        }
      }
    ...
    const generated=await zip.generateAsync({type:'nodebuffer',compression:'DEFLATE',compressionOptions:{level:6}});await validatePmWorkbookOoxml(generated,formulaExpectations);const parsedValidation=await inspectPmWorkbook(generated);validateSynchronizedTrackerUpdates(parsedValidation,input.trackerUpdates);for(const removal of input.trackerRemovals??[]){const match=synchronizedTrackerMatches(parsedValidation,removal.assetNumber,removal.taskTitle,removal.intervalType);if(match.resolved.length)throw new Error(`Workbook validation failed for ${removal.assetNumber} / ${removal.taskTitle}: deleted tracker row match count ${match.resolved.length}; row numbers ${trackerRowNumbers(match.resolved.map(row=>row.rowNumber))}.`);}fs.writeFileSync(temporaryPath,generated,{flag:'wx'});
