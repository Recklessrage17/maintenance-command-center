...
The synchronized tracker supports `Hourly`, `Cycle` / `Cycles`, `Days`, and `Annual` only. Hour and cycle rows use the physical machine's shared canonical meter. Days and Annual use real Excel dates. The production `Annual` value of `365` is retained as 365 days; legacy MCC Annual rows configured as `12` retain their existing 12-month behavior.
...
3. **Apply Workbook** requires an explicit preview token and idempotency key. The backend reparses the server-held workbook and rebuilds the action plan against current MCC data before opening the transaction. Structurally readable workbooks remain applyable when the database plan contains zero additions, updates, or history rows, including same-workbook re-imports and repairable shared-meter conflicts. Unknown or ambiguous machine assets, unreadable XLSX packages, and missing required structure remain hard stops.
4. Valid changes are committed to SQLite in one transaction and audited with both the previewing importer and confirming user IDs. A workbook row that exactly matches a deleted MCC PM schedule restores the original task and records a restoration audit instead of inserting a duplicate.
...
For an Hourly or Cycles meter, agreement across every workbook row is authoritative even when the agreed value is lower than MCC's current reading. If workbook rows disagree, MCC never chooses one duplicated reading: it preserves the canonical MCC physical meter, still applies unambiguous schedule/task data, and regenerates every affected tracker row with the canonical value. Truly deleted schedules are removed before generated-workbook validation. Reapplying the generated workbook therefore converges without recreating the same shared-meter conflict or deleted-row failure.
