...
export type MccFolderIconSize = 'compact' | 'standard';

export function MccFolderIcon({
  open = false,
  size = 'compact',
  className = '',
}: {
  open?: boolean;
  size?: MccFolderIconSize;
  className?: string;
}) {
  const gradientId = `mcc-folder-gradient-${useId().replace(/:/g, '')}`;
  const shadeId = `mcc-folder-shade-${useId().replace(/:/g, '')}`;

  return <span
    className={`mcc-file-type-icon mcc-file-type-icon--folder mcc-folder-icon mcc-folder-icon--${size}${open ? ' is-open' : ''}${className ? ` ${className}` : ''}`}
