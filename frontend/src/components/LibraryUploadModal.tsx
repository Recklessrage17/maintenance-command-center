...
export type LibraryUploadLimits={documentsMb:number|null;picturesMb:number|null;videosMb:number|null;resumable?:boolean;chunkBytes?:number;storageReserveBytes?:number};
...
export function LibraryUploadModal({eyebrow,title,destination,files,limits,busy,error='',progress=null,completedCount=0,currentFile='',transferredBytes=0,totalTransferBytes=0,speedBytesPerSecond=null,etaSeconds=null,allowLooseFiles=true,metadata,conflictActions,onPickFiles,onPickFolder,onRemove,onUpload,onCancel,onClose}:Props){
...
function fileCategory(value:string){const type=extension(value);return ['jpg','jpeg','png','webp'].includes(type)?'Picture':['mp4','webm','mov'].includes(type)?'Video':['pdf','doc','docx','xls','xlsx','txt'].includes(type)?'Document':type.toUpperCase()||'File';}
...
function limitLabel(value:number|null){return value===null?'limited only by available server storage':`up to ${value} MB`;}
function formatDuration(value:number){const seconds=Math.max(1,Math.round(value));if(seconds<60)return `${seconds}s`;const minutes=Math.ceil(seconds/60);if(minutes<60)return `${minutes}m`;return `${Math.floor(minutes/60)}h ${minutes%60}m`;}
