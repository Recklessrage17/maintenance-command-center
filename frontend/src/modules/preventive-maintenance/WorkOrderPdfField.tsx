...
export function WorkOrderPdfField({asset,taskTitle,workOrderNumber,value,disabled,onChange,onMaxBytesChange,onProcessingChange}:{asset:AssetIdentity;taskTitle:string;workOrderNumber:string;value:File|null;disabled:boolean;onChange:(file:File|null)=>void;onMaxBytesChange:(maxBytes:number)=>void;onProcessingChange:(processing:boolean)=>void}){
  ...
  useEffect(()=>{setPreviewingGeneratedPdf(false);if(!value||attachment?.kind!=='photos'){setGeneratedPreviewUrl('');return;}const url=URL.createObjectURL(value);setGeneratedPreviewUrl(url);return()=>URL.revokeObjectURL(url);},[attachment?.kind,value]);
  useEffect(()=>{if(!previewingGeneratedPdf)return;requestAnimationFrame(()=>generatedPreviewRef.current?.focus());const close=(event:KeyboardEvent)=>{if(event.key!=='Escape')return;event.preventDefault();event.stopImmediatePropagation();closeGeneratedPreview();};document.addEventListener('keydown',close,true);return()=>document.removeEventListener('keydown',close,true);},[previewingGeneratedPdf]);
  ...
  function openGeneratedPreview(){if(!generatedPreviewUrl)return;setPreviewingGeneratedPdf(true);}
  ...
  return <div className="form-field pm-form-wide pm-work-order-file">
    ...
    {shownAttachment&&<div className={`pm-work-order-attachment-summary${shownAttachment.kind==='photos'?' is-generated':''}`} aria-label="Attached work-order PDF">
      <MccFileTypeIcon type="pdf" className="pm-work-order-pdf-icon"/><span><strong title={shownAttachment.filename}>{shownAttachment.filename}</strong><small>{shownAttachment.pageCount?`${shownAttachment.pageCount} page${shownAttachment.pageCount===1?'':'s'} · `:''}{formatWorkOrderFileSize(shownAttachment.size)}{shownAttachment.kind==='photos'?' · generated from photos':' · selected PDF'}</small></span><div className="pm-work-order-attachment-actions">{shownAttachment.kind==='photos'&&<button ref={generatedPreviewButtonRef} className="secondary-button compact-button glass-button glass-button--secondary" type="button" disabled={disabled||processing||!generatedPreviewUrl} onClick={openGeneratedPreview}>Preview PDF</button>}<button className="link-button compact-button glass-button glass-button--secondary" type="button" disabled={disabled||processing} onClick={removeAttachment}>Remove</button></div>
    </div>}
    ...
    {previewingGeneratedPdf&&shownAttachment?.kind==='photos'&&generatedPreviewUrl&&createPortal(<div className="modal-backdrop glass-modal-backdrop pm-photo-preview-backdrop" role="presentation"><section ref={generatedPreviewRef} tabIndex={-1} className="mcc-card glass-modal-shell pm-generated-pdf-preview-modal" role="dialog" aria-modal="true" aria-labelledby="pm-generated-pdf-preview-title">
      <div className="modal-heading"><div><p className="eyebrow">Generated work-order PDF</p><h3 id="pm-generated-pdf-preview-title">Preview generated work-order PDF</h3><p>Verify readability, orientation, and page order before completing the PM.</p></div><button className="link-button compact-button glass-button glass-button--secondary" type="button" onClick={closeGeneratedPreview}>Close</button></div>
      <div className="pm-photo-preview-meta"><strong title={shownAttachment.filename}>{shownAttachment.filename}</strong><span>{shownAttachment.pageCount} page{shownAttachment.pageCount===1?'':'s'} · {formatWorkOrderFileSize(shownAttachment.size)}</span></div>
      <div className="pm-generated-pdf-preview-frame"><iframe src={generatedPreviewUrl} title="Generated work-order PDF pages" /></div>
      <p className="pm-generated-pdf-preview-help">If the embedded viewer is unavailable, open the PDF in the browser viewer.</p>
      <div className="modal-actions pm-generated-pdf-preview-actions"><a className="secondary-button glass-button glass-button--secondary" href={generatedPreviewUrl} target="_blank" rel="noreferrer">Open PDF in browser</a><button className="primary-button glass-button glass-button--primary" type="button" onClick={closeGeneratedPreview}>Done reviewing</button></div>
    </section></div>,document.body)}
  </div>;
}
