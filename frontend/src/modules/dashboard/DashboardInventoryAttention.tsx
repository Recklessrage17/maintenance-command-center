...
export function DashboardInventoryAttention({effectivePermissions}:{effectivePermissions:string[]}) {
  ...
  return <section className="mcc-card dashboard-inventory-attention" aria-labelledby="dashboard-inventory-title">
    <header className="dashboard-inventory-heading"><div><p className="eyebrow">Inventory control</p><h2 id="dashboard-inventory-title">Inventory Attention</h2></div><span aria-hidden="true" className="dashboard-inventory-live-indicator">Live</span></header>
    <p className="dashboard-inventory-caption">Opt-in stock alerts · refreshes while visible</p>
    {loading?<p className="dashboard-inventory-state" role="status">Loading inventory attention…</p>:<div className="dashboard-stock-counters">{(['Out of Stock','Low Stock'] as StockStatus[]).map(label=>{const count=label==='Out of Stock'?data.outOfStockCount:data.lowStockCount;return <button type="button" key={label} className={`dashboard-stock-counter ${label==='Out of Stock'?'is-danger':'is-warning'}`} onClick={()=>{setStatus(label);setNotice('');setActionError('');setRequestPart(null);}} aria-label={`${label}: ${count}. Open inventory attention`}><span className="dashboard-stock-counter-copy"><span>{label}</span><small>{count===0?'Clear':'Review parts'}</small></span><strong>{count.toLocaleString()}</strong><span className="dashboard-stock-counter-arrow" aria-hidden="true"><svg viewBox="0 0 20 20"><path d="M5 10h9m-3.5-3.5L14 10l-3.5 3.5"/></svg></span></button>;})}</div>}
    {!loading&&!error&&data.items.length===0&&<p className="dashboard-inventory-healthy">All enabled stock alerts are clear.</p>}
