...
function WarningNoteViewer({group,onClose}:{group:PmAssetGroup;onClose:()=>void}) {
  ...
  return createPortal(<div className="modal-backdrop glass-modal-backdrop dashboard-tech-note-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget)onClose();}}><section ref={dialogRef} className="mcc-card glass-modal-shell dashboard-tech-note-viewer" role="dialog" aria-modal="true" aria-labelledby={`dashboard-tech-note-title-${group.library}-${group.assetId}`}>
    <div className="modal-heading"><div><p className="eyebrow">Warning / Needs Attention</p><h2 id={`dashboard-tech-note-title-${group.library}-${group.assetId}`}>{group.warningFilter==='hold'?'Hold':'Tech Open'} for {group.assetNumber}</h2><p>{assetLabel} · {group.brand||'Brand / manufacturer not set'}</p></div><button className="link-button compact-button" type="button" onClick={onClose}>Close</button></div>
    <div className="dashboard-tech-note-list">{group.warningNotes.map(note=>{const expanded=expandedId===note.id;const panelId=`dashboard-tech-note-body-${note.assetLibrary}-${note.id}`;return <article className={`dashboard-tech-note-row${note.hold?' is-on-hold':''}${expanded?' is-expanded':''}`} key={`${note.assetLibrary}:${note.id}`}>
      <button className="dashboard-tech-note-toggle" type="button" aria-expanded={expanded} aria-controls={panelId} onClick={()=>setExpandedId(current=>current===note.id?null:note.id)}><span className="dashboard-tech-note-warning-icon" aria-hidden="true">!</span><span className="dashboard-tech-note-copy"><strong>{note.title}</strong>{note.hold&&<span className="asset-note-status-badge is-hold">On Hold</span>}<span className="dashboard-tech-note-metadata"><span className="dashboard-tech-note-meta-line"><span className="dashboard-tech-note-meta-label">Date</span><time dateTime={note.noteDate}>{formatDate(note.noteDate)}</time></span><span className="dashboard-tech-note-meta-line dashboard-tech-note-meta-age"><span className="dashboard-tech-note-meta-label">Date age</span><span>{relativeNoteAge(note.noteDate,note.createdAt)}</span></span><span className="dashboard-tech-note-meta-line"><span className="dashboard-tech-note-meta-label">Technician</span><span>{note.createdBy}</span></span></span></span><span className="dashboard-tech-note-expand" aria-hidden="true">{expanded?'−':'+'}</span></button>
      <div id={panelId} className="dashboard-tech-note-body" hidden={!expanded}>{note.workOrder&&<p className="dashboard-tech-note-work-order"><span>Work Order</span><strong>{note.workOrder}</strong></p>}<p>{note.body}</p><div className="dashboard-tech-note-actions"><button className="secondary-button compact-button" type="button" onClick={()=>window.open(note.pdfUrl,'_blank','noopener,noreferrer')}>Print / PDF</button><a className="link-button compact-button" href={`/${note.assetLibrary}-library?asset=${note.assetId}&note=${note.id}`}>Open full asset detail</a></div></div>
    </article>;})}</div>
