...
function EquipmentDetail({asset,canEdit,canDelete,canManagePm,performedBy,onBack,onUpdated,onRemoved}:{asset:EquipmentAsset;canEdit:boolean;canDelete:boolean;canManagePm:boolean;performedBy:string;onBack:()=>void;onUpdated:(asset:EquipmentAsset)=>void;onRemoved:()=>void}){
  ...
  async function save(section:EquipmentSection){setSaving(true);setError('');try{const data=await api<{asset:EquipmentAsset}>(`/api/equipment-library/assets/${current.id}`,{method:'PUT',body:JSON.stringify(normalizedEquipmentDraft(draft))});setCurrent(data.asset);setEditing(null);onUpdated(data.asset);}catch(value){setError((value as Error).message);}finally{setSaving(false);}}
