...
const optionalEquipmentTextFields:ReadonlyArray<keyof EquipmentDraft>=['equipmentType','manufacturer','model','serialNumber','equipmentYear','location','department','powerType','voltage','phase','amperage','airRequirement','waterRequirement','specificationNotes'];
const zeroEquipmentTextFields:ReadonlyArray<keyof EquipmentDraft>=['capacityRating','dimensions','weight'];

function normalizedEquipmentDraft(draft:EquipmentDraft):EquipmentDraft{
  const normalized={...draft,customCategory:draft.category==='Other / Custom'?draft.customCategory.trim():''};
  for(const key of optionalEquipmentTextFields)normalized[key]=(String(normalized[key]??'').trim()||'N/A') as never;
  for(const key of zeroEquipmentTextFields)normalized[key]=(String(normalized[key]??'').trim()||'0') as never;
  return normalized;
}
...
export function EquipmentLibraryPage({userFullName=''}:{userFullName?:string}){
  ...
  async function load(){
    ...
    try{const data=await api<EquipmentResponse>('/api/equipment-library/assets');setAssets(data.assets);setPermissions(data.permissions);const requestedAsset=!requestedAssetOpenedRef.current&&requestedAssetIdRef.current?data.assets.find(item=>item.id===requestedAssetIdRef.current):null;if(requestedAsset){requestedAssetOpenedRef.current=true;setDetailAsset(requestedAsset);}else if(detailAsset){const refreshed=data.assets.find(item=>item.id===detailAsset.id);if(refreshed)setDetailAsset(refreshed);}}
...
function EquipmentDetail({asset,canEdit,canDelete,canManagePm,performedBy,onBack,onUpdated,onRemoved}:{asset:EquipmentAsset;canEdit:boolean;canDelete:boolean;canManagePm:boolean;performedBy:string;onBack:()=>void;onUpdated:(asset:EquipmentAsset)=>void;onRemoved:()=>void}){
  const [current,setCurrent]=useState(asset);const [open,setOpen]=useState<EquipmentSection|null>('basic');const [editing,setEditing]=useState<EquipmentSection|null>(null);const [draft,setDraft]=useState(()=>assetToDraft(asset));const saveAction=useActionProgress();const saving=saveAction.active;const [error,setError]=useState('');const [history,setHistory]=useState<HistoryRecord[]>([]);const [historyOpen,setHistoryOpen]=useState(false);
  ...
  function begin(section:EquipmentSection){saveAction.reset();setEditing(section);setOpen(section);setDraft(assetToDraft(current));setError('');}
  async function save(section:EquipmentSection){setError('');const result=await saveAction.run(()=>api<{asset:EquipmentAsset}>(`/api/equipment-library/assets/${current.id}`,{method:'PUT',body:JSON.stringify(normalizedEquipmentDraft(draft))}));if(result.status==='duplicate')return;if(result.status==='error'){setError((result.error as Error).message);return;}setCurrent(result.value.asset);setEditing(null);onUpdated(result.value.asset);}
  ...
  return <div className="page-stack equipment-detail-page machine-library-page is-detail-view mcc-detail-workspace">
    <header className="mcc-card glass-panel equipment-detail-header" style={{'--asset-detail-accent':categoryAccent(current.category||current.manufacturer)} as CSSProperties}><button className="secondary-button compact-button glass-button glass-button--secondary asset-detail-back" type="button" onClick={onBack}>← Equipment Library</button><div className="asset-detail-heading-copy"><p className="eyebrow">Equipment Asset {current.assetNumber}</p><h2>{current.equipmentName}</h2><p className="asset-detail-context">{display(current.category)} · {display(current.location)}</p><div className="asset-detail-identity-strip" aria-label="Equipment identity"><span className="asset-detail-brand-chip"><span className="machine-detail-brand-dot" aria-hidden="true" /><span className="asset-detail-brand-copy"><span>Brand</span><strong>{display(current.manufacturer)}</strong></span></span><span className="asset-detail-data-chip"><span>Model</span><strong>{display(current.model)}</strong></span><span className="asset-detail-data-chip"><span>Serial #</span><strong>{display(current.serialNumber)}</strong></span></div></div><div className="equipment-detail-actions">{history.length>1&&<button className="secondary-button compact-button" type="button" onClick={()=>setHistoryOpen(true)}>View Full History</button>}<a className="primary-button compact-button glass-button glass-button--primary" href={`/api/equipment-library/assets/${current.id}/specification.pdf?download=true`} download>Equipment Specification PDF</a>{canEdit&&<button className="secondary-button compact-button" type="button" onClick={()=>void toggleDisabled()}>{current.status==='disabled'?'Enable Equipment':'Disable Equipment'}</button>}{canDelete&&<button className="danger-button compact-button" type="button" onClick={()=>void remove()}>Remove</button>}</div></header>
    ...
    <div className="machine-detail-accordion-list">{sections.map(section=>{const expanded=open===section.key;const isEditing=editing===section.key;return <MccCategoryAccordion key={section.key} accent={section.accent} expanded={expanded} editing={isEditing}><MccAccordionHeader title={section.title} summary={section.summary} expanded={expanded} controls={`equipment-${section.key}`} onToggle={()=>{if(!editing)setOpen(value=>value===section.key?null:section.key);}} actions={canEdit&&(isEditing?<div className="glass-button-group"><button className="secondary-button compact-button" type="button" disabled={saving} onClick={()=>{saveAction.reset();setEditing(null);setDraft(assetToDraft(current));}}>Cancel</button><button className="primary-button compact-button" type="button" disabled={saving} aria-busy={saveAction.pending} onClick={()=>void save(section.key)}><ActionButtonProgress phase={saveAction.phase} idleLabel="Save" pendingLabel="Saving..." successLabel="Saved" errorLabel="Try Save" /></button></div>:<button className="secondary-button compact-button" type="button" onClick={()=>begin(section.key)}>Edit Mode</button>)}/><div className="machine-detail-accordion-panel" id={`equipment-${section.key}`} aria-hidden={!expanded}>{isEditing?<div className="equipment-edit-grid">{section.edit}</div>:section.view}</div></MccCategoryAccordion>;})}</div>
    <PreventiveMaintenanceTracking asset={{id:current.id,assetNumber:current.assetNumber,assetName:current.equipmentName,brand:current.manufacturer}} canEdit={canManagePm} library="equipment" performedBy={performedBy}/>
...
function EquipmentSummaryTile({label,value,tone}:{label:string;value:ReactNode;tone:string}){return <div className="machine-detail-summary-card glass-card glass-card--nested"><span className="machine-detail-summary-label">{label}</span><strong className={`machine-detail-summary-pill glass-pill ${tone}`}>{value}</strong></div>;}
function Field({label,value,onChange,required=false,wide=false,className=''}:{label:string;value:string;onChange:(value:string)=>void;required?:boolean;wide?:boolean;className?:string}){return <label className={`form-field${wide?' equipment-field-wide':''}${className?` ${className}`:''}`}><span>{label}{required?' *':''}</span><input className="glass-input" value={value} onChange={event=>onChange(event.target.value)} required={required}/></label>;}
...
function BasicFields({draft,setDraft}:{draft:EquipmentDraft;setDraft:(value:EquipmentDraft)=>void}){const set=<K extends keyof EquipmentDraft>(key:K,value:EquipmentDraft[K])=>setDraft({...draft,[key]:value});const customMode=draft.category==='Other / Custom';const setCategory=(value:string)=>setDraft({...draft,category:value,customCategory:value==='Other / Custom'?draft.customCategory:''});return <><Field label="Equipment Name" value={draft.equipmentName} onChange={value=>set('equipmentName',value)} required/><Field label="Equipment Asset #" value={draft.assetNumber} onChange={value=>set('assetNumber',value)} required/><MccSearchableCombobox className={customMode?'equipment-custom-category-control':''} label="Category *" value={draft.category} options={categories} onChange={setCategory} required/>{customMode&&<Field className="equipment-custom-category-field" label="Custom Category" value={draft.customCategory} onChange={value=>set('customCategory',value)} required/>}<Field label="Equipment Type" value={draft.equipmentType} onChange={value=>set('equipmentType',value)}/><Field label="Manufacturer / Brand" value={draft.manufacturer} onChange={value=>set('manufacturer',value)}/><Field label="Model" value={draft.model} onChange={value=>set('model',value)}/><Field label="Serial Number" value={draft.serialNumber} onChange={value=>set('serialNumber',value)}/><Field label="Year" value={draft.equipmentYear} onChange={value=>set('equipmentYear',value)}/><Field label="Location" value={draft.location} onChange={value=>set('location',value)}/><Field label="Department / Area" value={draft.department} onChange={value=>set('department',value)}/><SelectField label="Status" value={draft.status} onChange={value=>set('status',value as EquipmentStatus)} options={[['active','Active'],['down','Down'],['disabled','Disabled']]}/><SelectField label="Criticality" value={draft.criticality} onChange={value=>set('criticality',value)} options={[['','Not Set'],['low','Low'],['medium','Medium'],['high','High'],['critical','Critical']]}/></>;}
