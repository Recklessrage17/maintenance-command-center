...
function PmExcelSyncControls({canEdit,onImported}:{canEdit:boolean;onImported:()=>void|Promise<void>}){
  ...
  async function cancelPreview(){const current=operation;setPreview(null);setError('');const restoreFocus=()=>window.setTimeout(()=>previewButtonRef.current?.focus(),100);if(!current?.active){setOperation(null);restoreFocus();return;}try{const data=await requestJson<{operation:PmExcelOperation}>(`/api/pm-excel/operations/${encodeURIComponent(current.id)}/cancel`,{method:'POST',body:'{}'});setOperation(data.operation);}catch{setOperation(null);}finally{restoreFocus();}}
  ...
  const operationMessage=operation?.message||(requestBusy?busyLabel:'');const visibleError=operation?.errorMessage||(!file&&!operation?sync?.errorMessage:'');const previewContinuation=operation?.stage==='preparing';
  return <><section className="pm-excel-sync glass-card glass-card--nested" aria-label="PM Excel synchronization">
    ...
    {(operationActive||requestBusy)&&<div className="pm-excel-operation-progress" role="status" aria-live="polite" data-stage={operation?.stage??'preparing'}><span className="pm-action-spinner" aria-hidden="true"/><span><strong>{operationMessage}</strong><small>PM Excel controls remain locked until this operation finishes.</small></span></div>}
    {canEdit&&<div className="pm-excel-import-controls"><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" disabled={requestBusy||operationActive} onClick={()=>fileRef.current?.click()}>Import PM Excel</button><input ref={fileRef} hidden type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event=>{const selected=event.target.files?.[0]??null;event.currentTarget.value='';void selectFile(selected);}}/><span>{file?.name??'No workbook selected'}</span><button ref={previewButtonRef} className="primary-button compact-button glass-button glass-button--primary" type="button" disabled={!file||requestBusy||(operationActive&&!previewContinuation)} onClick={()=>void previewFile()}>{requestBusy?'Validating...':'Preview Changes'}</button></div>}
