...
function PmTaskCard({asset,task,canEdit,busy,onView,onEdit,onComplete,onDeactivate,onDelete,onHistory}:{asset:AssetIdentity;task:PmTask;canEdit:boolean;busy:boolean;onView:()=>void;onEdit:()=>void;onComplete:()=>void;onDeactivate:()=>void;onDelete:()=>void;onHistory:()=>void}) {
  ...
  return <article className="pm-task-card glass-card">
    ...
    <div className={`pm-task-metrics${meter?' pm-task-metrics--meter':''}`}><div className="pm-metric-pill pm-metric-pill--last"><span>Last completed</span><strong>{meter?formatMeterMetric(task.lastCompletedMeter,task.intervalType):formatDate(task.lastCompletedDate)}</strong></div>{meter&&<div className="pm-metric-pill pm-metric-pill--current"><span>Current meter</span><strong>{formatMeterMetric(task.currentMeter,task.intervalType)}</strong></div>}<div className={`pm-metric-pill pm-metric-pill--next-due pm-metric-pill--${statusClass}`}><span>Next due</span><strong>{meter?formatMeterMetric(task.nextDueMeter,task.intervalType):formatDate(task.nextDueDate)}</strong></div></div>
...
type PmImportItem={sheet?:string;rowNumber?:number;rowNumbers?:number[];assetNumber?:string;assetName?:string;taskTitle?:string;taskType?:string;workOrderNumber?:string;message?:string;reason?:string;code?:string;overrideRequired?:boolean;originalValue?:number;oldValue?:Record<string,unknown>;newValue?:number|Record<string,unknown>};
type PmImportPreview={token:string;filename:string;expiresAt:string;additions:PmImportItem[];updates:PmImportItem[];historyAdditions:PmImportItem[];conflicts:PmImportItem[];warnings:PmImportItem[];rejectedRows:PmImportItem[];ignoredRows?:PmImportItem[];confirmEligibility:{importableRows:number;resolutionRequiredRows:number[];replacementEligible?:boolean;canConfirm:boolean};summary:{additions:number;updates:number;historyAdditions:number;conflicts:number;warnings:number;rejectedRows:number;ignoredRows?:number;ignoredAssets?:number}};
type PmImportOverride={type:'replacement'|'correction'|'override';reason:string};
type PmExcelDownloadKind='excel'|'package';

function pmDownloadFilename(disposition:string|null,fallback:string){
  const encoded=/filename\*=UTF-8''([^;]+)/i.exec(disposition??'')?.[1];
  if(encoded){try{return decodeURIComponent(encoded).trim()||fallback;}catch{/* Fall through to the basic filename. */}}
  return /filename="?([^";]+)"?/i.exec(disposition??'')?.[1]?.trim()||fallback;
}
async function downloadPmExcelArtifact(url:string,fallbackFilename:string){
  const response=await fetch(url,{credentials:'include'});
  if(!response.ok){const data=await response.json().catch(()=>({})) as {error?:string};throw new Error(data.error||'The PM download could not be generated.');}
  const blob=await response.blob();if(!blob.size)throw new Error('The generated PM download was empty.');
  const objectUrl=window.URL.createObjectURL(blob);const link=document.createElement('a');
  try{link.href=objectUrl;link.download=pmDownloadFilename(response.headers.get('content-disposition'),fallbackFilename);document.body.appendChild(link);link.click();link.remove();}
  finally{window.setTimeout(()=>window.URL.revokeObjectURL(objectUrl),0);}
}
...
function canConfirmPmReplacement(preview:PmImportPreview,overrides:Record<number,PmImportOverride>){const unresolved=preview.confirmEligibility.resolutionRequiredRows.some(rowNumber=>!hasMeaningfulPmImportReason(overrides[rowNumber]));return preview.confirmEligibility.canConfirm&&!unresolved;}
...
function PmImportPreviewRow({item,status,tone='neutral',overrides,setOverrides}:{item:PmImportItem;status:string;tone?:'ready'|'history'|'warning'|'danger'|'neutral';overrides:Record<number,PmImportOverride>;setOverrides:(update:(current:Record<number,PmImportOverride>)=>Record<number,PmImportOverride>)=>void}) {
  ...
  return <article className={`pm-import-preview-row pm-import-preview-row--${tone}`}><div className="pm-import-row-heading"><strong>{previewRowLabel(item)}</strong><span>{status}</span></div><div className="pm-import-row-copy"><strong>{previewRowSubject(item)}</strong><span>{previewRowDescription(item)}</span>{reason&&<p>{reason}</p>}{item.oldValue&&typeof item.newValue==='object'&&Object.entries(item.newValue).filter(([key,value])=>['intervalType','intervalValue','lastCompletedDate','lastCompletedMeter','currentMeter'].includes(key)&&item.oldValue?.[key]!==value).map(([key,value])=><p key={key}>{({intervalType:'Interval type',intervalValue:'Interval value',lastCompletedDate:'Last completed date',lastCompletedMeter:'Last completed meter',currentMeter:'Current meter'} as Record<string,string>)[key]}: {String(item.oldValue?.[key]??'None')} &rarr; {String(value??'None')}</p>)}</div>{decreasing&&item.rowNumber&&<div className="pm-import-override-grid"><label className="form-field"><span>Override type</span><select className="glass-input" value={overrides[item.rowNumber]?.type??'replacement'} onChange={event=>setOverrides(current=>({...current,[item.rowNumber!]:{type:event.target.value as PmImportOverride['type'],reason:current[item.rowNumber!]?.reason??''}}))}><option value="replacement">Meter replacement</option><option value="correction">Meter correction</option><option value="override">Authorized override</option></select></label><label className="form-field"><span>Audit reason *</span><input className="glass-input" value={overrides[item.rowNumber]?.reason??''} onChange={event=>setOverrides(current=>({...current,[item.rowNumber!]:{type:current[item.rowNumber!]?.type??'replacement',reason:event.target.value}}))}/></label></div>}</article>;
}
...
function PmImportPreviewModal({preview,busy,error,overrides,setOverrides,onClose,onConfirm}:{preview:PmImportPreview;busy:boolean;error:string;overrides:Record<number,PmImportOverride>;setOverrides:(update:(current:Record<number,PmImportOverride>)=>Record<number,PmImportOverride>)=>void;onClose:()=>void;onConfirm:()=>void}) {
  ...
  return createPortal(<div className="modal-backdrop glass-modal-backdrop pm-import-preview-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)onClose();}}><section ref={dialogRef} className="mcc-card glass-modal-shell pm-import-preview-modal" role="dialog" aria-modal="true" aria-labelledby="pm-import-preview-title" aria-describedby="pm-import-preview-help" tabIndex={-1}>
    ...
    <div className="pm-import-preview-body"><div className="pm-import-summary-grid" aria-label="Import preview summary">{([['Additions',preview.summary.additions,'success'],['Updates',preview.summary.updates,'neutral'],['History rows',preview.summary.historyAdditions,'history'],['Ignored / Skipped',preview.summary.ignoredRows??0,'neutral'],['Conflicts',preview.summary.conflicts,'warning'],['Rejected',preview.summary.rejectedRows,'danger']] as Array<[string,number,string]>).map(([label,count,tone])=><div className={`pm-import-summary-card pm-import-summary-card--${tone}`} key={label}><span>{label}</span><strong>{count}</strong></div>)}</div>
      <div className="pm-import-sections"><PmImportPreviewSection id="ready" title="Ready to Apply" items={readyItems} tone="ready" description="Task additions, updates, and restorations" emptyText="No database task changes are required." {...sectionProps('ready')} renderItem={(item,index)=>row(item,preview.additions.includes(item)?'Addition · Ready':'Update · Ready','ready',index)} /><PmImportPreviewSection id="history" title="History" items={preview.historyAdditions} tone="history" description="Valid PM history rows" emptyText="No history rows are ready." {...sectionProps('history')} renderItem={(item,index)=>row(item,'History · Ready','history',index)} /><PmImportPreviewSection id="conflicts" title="Conflicts" items={preview.conflicts} tone="warning" description="Repairable conflicts are normalized during apply" emptyText="No conflicts detected." {...sectionProps('conflicts')} renderItem={(item,index)=>row(item,item.code==='INCONSISTENT_SHARED_METER'?'Repairable · Canonical meter retained':item.code==='DECREASING_METER'&&item.rowNumber&&hasMeaningfulPmImportReason(overrides[item.rowNumber])?'Resolved · Will apply':'Conflict · Will be skipped','warning',index)} /><PmImportPreviewSection id="ignored" title="Ignored / Skipped" items={preview.ignoredRows??[]} tone="neutral" description={`${preview.summary.ignoredAssets??0} workbook-only assets; machine not in MCC`} emptyText="No ignored rows." {...sectionProps('ignored')} renderItem={(item,index)=>row(item,'Ignored - Machine not in MCC','neutral',index)} /><PmImportPreviewSection id="rejected" title="Rejected" items={preview.rejectedRows} tone="danger" description="Invalid rows will be skipped" emptyText="No rejected rows." {...sectionProps('rejected')} renderItem={(item,index)=>row(item,'Rejected · Will be skipped','danger',index)} /><PmImportPreviewSection id="noChanges" title="No Changes" items={preview.warnings} tone="neutral" description="Rows that require no database write" emptyText="No unchanged rows or workbook notices." {...sectionProps('noChanges')} renderItem={(item,index)=>row(item,'No database change','neutral',index)} /></div>{error&&<p className="form-message error" role="alert">{error}</p>}</div>
    <footer className="pm-import-preview-footer"><p id="pm-import-preview-help">Apply Workbook updates MCC where needed{preview.conflicts.some(item=>item.code==='INCONSISTENT_SHARED_METER')?', repairs shared-meter conflicts,':''} and replaces the active workbook even when the database result is a no-op.</p>{!canConfirm&&<p className="pm-import-disabled-reason" role="status">{preview.summary.conflicts>0?'MCC cannot apply this workbook until blocking conflicts or ambiguous asset matches are resolved.':'MCC cannot apply this workbook. Check its required structure and asset resolution details.'}</p>}<div className="modal-actions glass-modal__actions"><button className="secondary-button glass-button glass-button--secondary" type="button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button glass-button glass-button--success" type="button" disabled={busy||!canConfirm} onClick={onConfirm}>{busy?'Applying Workbook...':importableCount?`Apply Workbook (${importableCount} database row${importableCount===1?'':'s'})`:'Apply Workbook'}</button></div></footer>
  </section></div>,document.body);
}

function PmExcelSyncControls({canEdit,onImported}:{canEdit:boolean;onImported:()=>void|Promise<void>}){
  const [sync,setSync]=useState<PmExcelSyncStatus|null>(null);const [operation,setOperation]=useState<PmExcelOperation|null>(null);const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState<PmImportPreview|null>(null);const [busyLabel,setBusyLabel]=useState('Processing PM workbook');const [resetting,setResetting]=useState(false);const [error,setError]=useState('');const [notice,setNotice]=useState('');const [downloadError,setDownloadError]=useState('');const [downloadBusy,setDownloadBusy]=useState<Record<PmExcelDownloadKind,boolean>>({excel:false,package:false});const downloadBusyRef=useRef<Record<PmExcelDownloadKind,boolean>>({excel:false,package:false});const [overrides,setOverrides]=useState<Record<number,PmImportOverride>>({});const fileRef=useRef<HTMLInputElement>(null);const importButtonRef=useRef<HTMLButtonElement>(null);const previewButtonRef=useRef<HTMLButtonElement>(null);const previewWasOpenRef=useRef(false);const dismissedOperationIdsRef=useRef(new Set<string>());const confirmRequestId=useRequestId();const completedOperationRef=useRef('');const workbookAction=usePmActionProgress({label:busyLabel});const requestBusy=workbookAction.pending;const operationActive=Boolean(operation?.active);const operationProcessing=Boolean(operation?.active&&pmExcelProcessingStages.has(operation.stage));const operationAwaitingUser=Boolean(operation?.active&&(operation.stage==='preparing'||operation.stage==='preview_ready'));const controlsLocked=requestBusy||resetting||operationProcessing;
  ...
  async function downloadArtifact(kind:PmExcelDownloadKind){
    if(downloadBusyRef.current[kind])return;
    downloadBusyRef.current={...downloadBusyRef.current,[kind]:true};setDownloadBusy(current=>({...current,[kind]:true}));setDownloadError('');
    const isExcel=kind==='excel';
    try{await downloadPmExcelArtifact(isExcel?'/api/pm-excel/download':'/api/pm-excel/package/download',isExcel?'PM_report_1.2v.xlsx':`PM_Package_${new Date().toISOString().slice(0,10)}.zip`);}
    catch(value){setDownloadError((value as Error).message||(isExcel?'The latest PM workbook could not be downloaded.':'The PM package could not be downloaded.'));}
    finally{downloadBusyRef.current={...downloadBusyRef.current,[kind]:false};setDownloadBusy(current=>({...current,[kind]:false}));}
  }
  ...
  return <><section className="pm-excel-sync glass-card glass-card--nested" aria-label="PM Excel synchronization">
    <div className="pm-excel-sync-heading"><div><strong>PM Excel synchronization</strong><small>{operation?operationMessage:syncStatusText(sync)}</small>{visibleError&&<small className="pm-sync-error">{visibleError}</small>}</div><div className="glass-button-group">{sync?.downloadAvailable&&<><button className="primary-button compact-button glass-button glass-button--primary pm-excel-download-button" type="button" disabled={downloadBusy.package} aria-busy={downloadBusy.package} onClick={()=>void downloadArtifact('package')}>{downloadBusy.package&&<span className="pm-action-button-spinner" aria-hidden="true"/>}<span aria-live="polite">{downloadBusy.package?`Building PM Package\u2026`:'Download PM Package (.zip)'}</span></button><button className="secondary-button compact-button glass-button glass-button--secondary pm-excel-download-button" type="button" disabled={downloadBusy.excel} aria-busy={downloadBusy.excel} onClick={()=>void downloadArtifact('excel')}>{downloadBusy.excel&&<span className="pm-action-button-spinner" aria-hidden="true"/>}<span aria-live="polite">{downloadBusy.excel?`Generating Excel\u2026`:'Download Excel Only (.xlsx)'}</span></button></>}{canEdit&&sync?.status==='failed'&&!operation&&<button className="secondary-button compact-button glass-button glass-button--warning" type="button" disabled={requestBusy} onClick={()=>void retry()}>Retry Sync</button>}</div></div>
    ...
    {error&&<p className="form-message error" role="alert">{error}</p>}{downloadError&&<p className="form-message error" role="alert">{downloadError}</p>}{notice&&<p className="form-message" role="status">{notice}</p>}
