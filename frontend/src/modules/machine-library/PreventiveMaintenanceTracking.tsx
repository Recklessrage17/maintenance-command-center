...
function PmImportPreviewModal({preview,busy,error,overrides,setOverrides,onClose,onConfirm}:{preview:PmImportPreview;busy:boolean;error:string;overrides:Record<number,PmImportOverride>;setOverrides:(update:(current:Record<number,PmImportOverride>)=>Record<number,PmImportOverride>)=>void;onClose:()=>void;onConfirm:()=>void}) {
  ...
  const updateRows=new Set(preview.updates.map(item=>item.rowNumber).filter((rowNumber):rowNumber is number=>Boolean(rowNumber)));const decreasingRows=new Set(preview.conflicts.filter(item=>item.code==='DECREASING_METER'&&item.rowNumber&&updateRows.has(item.rowNumber)).map(item=>item.rowNumber!));const unresolvedDecreasing=[...decreasingRows].filter(rowNumber=>!overrides[rowNumber]?.reason.trim()).length;const previewImportable=preview.summary.additions+preview.summary.updates+preview.summary.historyAdditions;const importableCount=Math.max(0,previewImportable-unresolvedDecreasing);const readyItems=[...preview.additions,...preview.updates];
  ...
  return createPortal(<div className="modal-backdrop glass-modal-backdrop pm-import-preview-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!busy)onClose();}}><section ref={dialogRef} className="mcc-card glass-modal-shell pm-import-preview-modal" role="dialog" aria-modal="true" aria-labelledby="pm-import-preview-title" aria-describedby="pm-import-preview-help" tabIndex={-1}>
    ...
    <div className="pm-import-preview-body"><div className="pm-import-summary-grid" aria-label="Import preview summary">{([['Additions',preview.summary.additions,'success'],['Updates',preview.summary.updates,'neutral'],['History rows',preview.summary.historyAdditions,'history'],['Conflicts',preview.summary.conflicts,'warning'],['Rejected',preview.summary.rejectedRows,'danger']] as Array<[string,number,string]>).map(([label,count,tone])=><div className={`pm-import-summary-card pm-import-summary-card--${tone}`} key={label}><span>{label}</span><strong>{count}</strong></div>)}</div>
      <div className="pm-import-sections"><PmImportPreviewSection id="ready" title="Ready to Import" items={readyItems} tone="ready" description="Valid additions and updates" emptyText="No task additions or updates are ready." {...sectionProps('ready')} renderItem={(item,index)=>row(item,preview.additions.includes(item)?'Addition · Ready':'Update · Ready','ready',index)} /><PmImportPreviewSection id="history" title="History" items={preview.historyAdditions} tone="history" description="Valid PM history rows" emptyText="No history rows are ready." {...sectionProps('history')} renderItem={(item,index)=>row(item,'History · Ready','history',index)} /><PmImportPreviewSection id="conflicts" title="Conflicts" items={preview.conflicts} tone="warning" description="Unresolved rows will be skipped" emptyText="No conflicts detected." {...sectionProps('conflicts')} renderItem={(item,index)=>row(item,item.code==='DECREASING_METER'&&item.rowNumber&&overrides[item.rowNumber]?.reason.trim()?'Resolved · Will import':'Conflict · Will be skipped','warning',index)} /><PmImportPreviewSection id="rejected" title="Rejected" items={preview.rejectedRows} tone="danger" description="Invalid rows will be skipped" emptyText="No rejected rows." {...sectionProps('rejected')} renderItem={(item,index)=>row(item,'Rejected · Will be skipped','danger',index)} /><PmImportPreviewSection id="noChanges" title="No Changes" items={preview.warnings} tone="neutral" description="Rows that require no database write" emptyText="No unchanged rows or workbook notices." {...sectionProps('noChanges')} renderItem={(item,index)=>row(item,'No database change','neutral',index)} /></div>{error&&<p className="form-message error" role="alert">{error}</p>}</div>
    <footer className="pm-import-preview-footer"><p id="pm-import-preview-help">Only valid, unambiguous rows will be imported. Conflicting and rejected rows will be skipped.</p>{importableCount===0&&<p className="pm-import-disabled-reason" role="status">No valid rows are currently available to import. Resolve a decreasing-meter row or correct the workbook and preview it again.</p>}<div className="modal-actions glass-modal__actions"><button className="secondary-button glass-button glass-button--secondary" type="button" disabled={busy} onClick={onClose}>Cancel</button><button className="primary-button glass-button glass-button--success" type="button" disabled={busy||importableCount===0} onClick={onConfirm}>{busy?'Importing...':`Import Valid Rows${importableCount?` (${importableCount})`:''}`}</button></div></footer>
  </section></div>,document.body);
}

function PmExcelSyncControls({canEdit,onImported}:{canEdit:boolean;onImported:()=>void|Promise<void>}){
  const [sync,setSync]=useState<PmExcelSyncStatus|null>(null);const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState<PmImportPreview|null>(null);const [busyLabel,setBusyLabel]=useState('Processing PM workbook');const [error,setError]=useState('');const [notice,setNotice]=useState('');const [overrides,setOverrides]=useState<Record<number,PmImportOverride>>({});const fileRef=useRef<HTMLInputElement>(null);const confirmRequestId=useRequestId();const workbookAction=usePmActionProgress({label:busyLabel,pollStatus:()=>pollPmWorkflowStatus('machine')});const busy=workbookAction.pending;
  ...
  async function confirmImport(){if(!preview)return;const updateRows=new Set(preview.updates.map(item=>item.rowNumber).filter((rowNumber):rowNumber is number=>Boolean(rowNumber)));const unresolvedDecreasing=new Set(preview.conflicts.filter(item=>item.code==='DECREASING_METER'&&item.rowNumber&&updateRows.has(item.rowNumber)&&!overrides[item.rowNumber]?.reason.trim()).map(item=>item.rowNumber!));const importableCount=preview.summary.additions+preview.summary.updates+preview.summary.historyAdditions-unresolvedDecreasing.size;if(importableCount<=0)return;setBusyLabel('Importing and synchronizing PM workbook');setError('');try{await workbookAction.run(async()=>{const meterOverrides=Object.entries(overrides).filter(([,value])=>value.reason.trim()).map(([rowNumber,value])=>({rowNumber:Number(rowNumber),...value,reason:value.reason.trim()}));const data=await requestJson<{import:{added:number;updated:number;historyAdded:number};sync:PmExcelSyncStatus;syncError?:string|null}>('/api/pm-excel/confirm',{method:'POST',headers:{'Idempotency-Key':confirmRequestId.current},body:JSON.stringify({previewToken:preview.token,meterOverrides})});setSync(data.sync);setPreview(null);setFile(null);setNotice(`PM import confirmed: ${data.import.added} added, ${data.import.updated} updated, ${data.import.historyAdded} history rows added.${data.syncError?` Workbook sync needs retry: ${data.syncError}`:' Workbook synchronized.'}`);await onImported();});}catch(value){setError((value as Error).message||'The PM import could not be confirmed.');}}
