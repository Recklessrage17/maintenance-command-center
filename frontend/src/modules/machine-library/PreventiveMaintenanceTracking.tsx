  ...
  return <>
    <MccCategoryAccordion accent="pm" expanded={expanded} className="pm-tracking-card glass-panel glass-panel--nested">
      ...
      <div className="machine-detail-accordion-panel" id={`pm-tracking-panel-${asset.id}`} aria-hidden={!expanded}>
        ...
        <PmMachineMeterPanel asset={asset} library={library} canEdit={canEdit} onUpdated={load}/>
        ...
        {!loading&&safeTasks.length>0&&<div className="pm-task-grid">{safeTasks.map(task=><PmTaskCard key={task.id} asset={asset} task={task} canEdit={canEdit} onView={()=>setViewTask(task)} onEdit={()=>setFormTask(task)} onComplete={()=>setCompleteTask(task)} onDeactivate={()=>void deactivate(task)} onDelete={()=>setDeleteTask(task)} onHistory={()=>setHistoryTask(task)} />)}</div>}
      </div>
    ...
    {completeTask&&<PmCompleteWorkflowModal asset={asset} task={completeTask} library={library} performedBy={performedBy} onClose={()=>setCompleteTask(null)} onSaved={async()=>{setCompleteTask(null);await load();}} />}
    {historyTask&&<PmHistoryModal task={historyTask} apiBase={apiBase} onClose={()=>setHistoryTask(null)} />}
    {deleteTask&&<PmDeleteScheduleModal task={deleteTask} library={library} onClose={()=>setDeleteTask(null)} onDeleted={async()=>{setDeleteTask(null);await load();}}/>}
    {assetHistoryOpen&&<PmAssetHistoryModal asset={asset} library={library} onClose={()=>setAssetHistoryOpen(false)}/>}
  </>;
}

function PmTaskCard({asset,task,canEdit,onView,onEdit,onComplete,onDeactivate,onDelete,onHistory}:{asset:AssetIdentity;task:PmTask;canEdit:boolean;onView:()=>void;onEdit:()=>void;onComplete:()=>void;onDeactivate:()=>void;onDelete:()=>void;onHistory:()=>void}) {
  ...
  return <article className="pm-task-card glass-card">
    ...
    <div className="pm-task-card-heading"><div><h4>{safeString(task.title,'Untitled PM task')}</h4><span className="pm-interval-label"><span>Interval:</span> <strong className={`pm-interval-value pm-interval-value--${intervalTone(task.intervalType,task.intervalValue)}`}>{cadenceLabel(task.intervalType,task.intervalValue)}</strong></span></div><span className={`glass-pill pm-status pm-status--${statusClass}`}>{status==='Hold'?'HOLD':status}</span></div>
    ...
    <div className="pm-card-actions"><button className="secondary-button compact-button glass-button glass-button--secondary pm-card-action pm-card-action--view" type="button" onClick={onView}>View</button><button className="secondary-button compact-button glass-button glass-button--secondary pm-card-action pm-card-action--history" type="button" onClick={onHistory}>History ({safeCount(task.historyCount)})</button>{canEdit&&<button className="secondary-button compact-button glass-button glass-button--warning pm-card-action pm-card-action--edit" type="button" onClick={onEdit}>Edit</button>}{canEdit&&task.scheduleStatus==='active'&&<button className="primary-button compact-button glass-button glass-button--success pm-card-action pm-card-action--complete" type="button" onClick={onComplete}>Complete PM</button>}{canEdit&&<details className="pm-more-menu"><summary className="secondary-button compact-button glass-button glass-button--secondary" aria-label={`More actions for ${task.title}`}>More</summary><div className="pm-more-menu-popover">{task.scheduleStatus!=='inactive'&&<button type="button" onClick={onDeactivate}>Deactivate</button>}<button className="pm-more-delete" type="button" onClick={onDelete}>Delete Schedule</button></div></details>}</div>
  </article>;
}
