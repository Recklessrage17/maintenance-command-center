...
function PmExcelSyncControls({canEdit,onImported}:{canEdit:boolean;onImported:()=>void|Promise<void>}){
  ...
  async function previewFile(){if(!file)return;setBusy(true);setError('');setNotice('');try{const body=new FormData();body.append('file',file,file.name);const data=await requestJson<{preview:PmImportPreview}>('/api/pm-excel/preview',{method:'POST',body});setPreview(data.preview);setOverrides(Object.fromEntries(data.preview.conflicts.filter(item=>item.code==='DECREASING_METER'&&item.rowNumber).map(item=>[item.rowNumber!,{type:'replacement' as const,reason:''}])));confirmRequestId.current=crypto.randomUUID();}catch(value){setError((value as Error).message||'The PM workbook could not be previewed.');}finally{setBusy(false);}}
