...
type PmHistory={id:number;workOrderNumber:string;followUpRequired:boolean;followUpReason:string;attachment:PmWorkOrderAttachment|null;completionDate:string;completedMeter:number|null;performedBy:string;completionNotes:string;previousDueDate:string|null;previousDueMeter:number|null;nextDueDate:string|null;nextDueMeter:number|null;createdAt:string};
...
function normalizePmHistory(value:unknown):PmHistory[]{
  ...
  return value.map(item=>{
    ...
    return {id:safeNumber(item.id)!,workOrderNumber:safeString(item.workOrderNumber,'N/A'),followUpRequired:item.followUpRequired===true,followUpReason:safeString(item.followUpReason),attachment,completionDate:safeDateValue(item.completionDate)??'',completedMeter:safeNumber(item.completedMeter),performedBy:safeString(item.performedBy,'Unknown user'),completionNotes:safeString(item.completionNotes),previousDueDate:safeDateValue(item.previousDueDate),previousDueMeter:safeNumber(item.previousDueMeter),nextDueDate:safeDateValue(item.nextDueDate),nextDueMeter:safeNumber(item.nextDueMeter),createdAt:safeString(item.createdAt)};
  ...
  return <>
    <MccCategoryAccordion accent="pm" expanded={expanded} className="pm-tracking-card glass-panel glass-panel--nested">
      ...
      <div className="machine-detail-accordion-panel" id={`pm-tracking-panel-${asset.id}`} aria-hidden={!expanded}>
        ...
        {library==='machine'&&<PmExcelSyncControls canEdit={canEdit} onImported={load} />}
    ...
    {completeTask&&<PmCompleteModal asset={asset} task={completeTask} apiBase={apiBase} performedBy={performedBy} onClose={()=>setCompleteTask(null)} onSaved={async()=>{setCompleteTask(null);await load();}} />}
...
function PmTaskCard({task,canEdit,onView,onEdit,onComplete,onDeactivate,onHistory}:{task:PmTask;canEdit:boolean;onView:()=>void;onEdit:()=>void;onComplete:()=>void;onDeactivate:()=>void;onHistory:()=>void}) {
  ...
  return <article className="pm-task-card glass-card">
    ...
    <div className="pm-card-actions"><button className="secondary-button compact-button glass-button glass-button--secondary pm-card-action pm-card-action--view" type="button" onClick={onView}>View</button><button className="secondary-button compact-button glass-button glass-button--secondary pm-card-action pm-card-action--history" type="button" onClick={onHistory}>View History ({safeCount(task.historyCount)})</button>{canEdit&&<button className="secondary-button compact-button glass-button glass-button--warning pm-card-action pm-card-action--edit" type="button" onClick={onEdit}>Edit</button>}{canEdit&&task.scheduleStatus==='active'&&<button className="primary-button compact-button glass-button glass-button--success pm-card-action pm-card-action--complete" type="button" onClick={onComplete}>Complete PM</button>}{canEdit&&task.scheduleStatus!=='inactive'&&<button className="secondary-button compact-button glass-button glass-button--danger pm-card-action pm-card-action--deactivate" type="button" onClick={onDeactivate}>Deactivate</button>}</div>
  </article>;
}
...
function PmFormModal({asset,task,apiBase,onClose,onSaved}:{asset:AssetIdentity;task:PmTask|null;apiBase:string;onClose:()=>void;onSaved:()=>void|Promise<void>}) {
  ...
  const [saving,setSaving]=useState(false);const [error,setError]=useState('');const [submitted,setSubmitted]=useState(false);const [overrideType,setOverrideType]=useState<'replacement'|'correction'|'override'>('correction');const [overrideReason,setOverrideReason]=useState('');const formRef=useRef<HTMLFormElement>(null);const createRequestIdRef=useRequestId();
...
type PmImportItem={sheet?:string;rowNumber?:number;assetNumber?:string;taskTitle?:string;taskType?:string;workOrderNumber?:string;message?:string;reason?:string;code?:string;overrideRequired?:boolean;originalValue?:number;newValue?:number};
type PmImportPreview={token:string;filename:string;expiresAt:string;additions:PmImportItem[];updates:PmImportItem[];historyAdditions:PmImportItem[];conflicts:PmImportItem[];warnings:PmImportItem[];rejectedRows:PmImportItem[];summary:{additions:number;updates:number;historyAdditions:number;conflicts:number;warnings:number;rejectedRows:number}};
...
function previewItemText(item:PmImportItem){return [`Row ${item.rowNumber??'—'}`,item.assetNumber,item.taskTitle??item.taskType,item.workOrderNumber,item.message??item.reason].filter(Boolean).join(' · ');}

function PmExcelSyncControls({canEdit,onImported}:{canEdit:boolean;onImported:()=>void|Promise<void>}){
  const [sync,setSync]=useState<PmExcelSyncStatus|null>(null);const [file,setFile]=useState<File|null>(null);const [preview,setPreview]=useState<PmImportPreview|null>(null);const [busy,setBusy]=useState(false);const [error,setError]=useState('');const [notice,setNotice]=useState('');const [overrides,setOverrides]=useState<Record<number,{type:'replacement'|'correction'|'override';reason:string}>>({});const fileRef=useRef<HTMLInputElement>(null);const confirmRequestId=useRequestId();
  async function loadStatus(){try{const data=await requestJson<{sync:PmExcelSyncStatus}>('/api/pm-excel/status');setSync(data.sync);}catch{/* PM task data remains usable when workbook status is unavailable. */}}
  useEffect(()=>{void loadStatus();},[]);
  async function previewFile(){if(!file)return;setBusy(true);setError('');setNotice('');try{const body=new FormData();body.append('file',file,file.name);const data=await requestJson<{preview:PmImportPreview}>('/api/pm-excel/preview',{method:'POST',body});setPreview(data.preview);setOverrides(Object.fromEntries(data.preview.conflicts.filter(item=>item.code==='DECREASING_METER'&&item.rowNumber).map(item=>[item.rowNumber!,{type:'replacement' as const,reason:''}])));confirmRequestId.current=createRequestId();}catch(value){setError((value as Error).message||'The PM workbook could not be previewed.');}finally{setBusy(false);}}
  async function confirmImport(){if(!preview||preview.summary.additions+preview.summary.updates+preview.summary.historyAdditions===0)return;const unresolved=preview.conflicts.filter(item=>item.code==='DECREASING_METER'&&item.rowNumber&&!overrides[item.rowNumber]?.reason.trim());if(unresolved.length){setError('Enter an audit reason for every decreasing meter row.');return;}setBusy(true);setError('');try{const meterOverrides=Object.entries(overrides).map(([rowNumber,value])=>({rowNumber:Number(rowNumber),...value}));const data=await requestJson<{import:{added:number;updated:number;historyAdded:number};sync:PmExcelSyncStatus;syncError?:string|null}>('/api/pm-excel/confirm',{method:'POST',headers:{'Idempotency-Key':confirmRequestId.current},body:JSON.stringify({previewToken:preview.token,meterOverrides})});setSync(data.sync);setPreview(null);setFile(null);setNotice(`PM import confirmed: ${data.import.added} added, ${data.import.updated} updated, ${data.import.historyAdded} history rows added.${data.syncError?` Workbook sync needs retry: ${data.syncError}`:' Workbook synchronized.'}`);await onImported();}catch(value){setError((value as Error).message||'The PM import could not be confirmed.');}finally{setBusy(false);}}
  async function retry(){setBusy(true);setError('');try{const data=await requestJson<{sync:PmExcelSyncStatus}>('/api/pm-excel/sync/retry',{method:'POST',body:'{}'});setSync(data.sync);setNotice('PM workbook synchronization completed.');}catch(value){setError((value as Error).message||'Workbook synchronization could not be retried.');await loadStatus();}finally{setBusy(false);}}
  return <><section className="pm-excel-sync glass-card glass-card--nested" aria-label="PM Excel synchronization"><div className="pm-excel-sync-heading"><div><strong>PM Excel synchronization</strong><small>{syncStatusText(sync)}</small>{sync?.errorMessage&&<small className="pm-sync-error">{sync.errorMessage}</small>}</div><div className="glass-button-group">{sync?.downloadAvailable&&<><a className="primary-button compact-button glass-button glass-button--primary" href="/api/pm-excel/package/download" download>Download PM Package (.zip)</a><a className="secondary-button compact-button glass-button glass-button--secondary" href="/api/pm-excel/download" download>Download Excel Only (.xlsx)</a></>}{canEdit&&sync?.status==='failed'&&<button className="secondary-button compact-button glass-button glass-button--warning" type="button" disabled={busy} onClick={()=>void retry()}>Retry Sync</button>}</div></div>{sync?.downloadAvailable&&<p className="pm-package-warning">PDF hyperlinks require the matching PDF - Work orders directory from the full PM package.</p>}{canEdit&&<div className="pm-excel-import-controls"><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" disabled={busy} onClick={()=>fileRef.current?.click()}>Import PM Excel</button><input ref={fileRef} hidden type="file" accept=".xlsx,application/vnd.openxmlformats-officedocument.spreadsheetml.sheet" onChange={event=>{const selected=event.target.files?.[0]??null;event.currentTarget.value='';setFile(selected);setPreview(null);setError('');setNotice('');}}/><span>{file?.name??'No workbook selected'}</span><button className="primary-button compact-button glass-button glass-button--primary" type="button" disabled={!file||busy} onClick={()=>void previewFile()}>{busy?'Validating...':'Preview Changes'}</button></div>}{error&&<p className="form-message error" role="alert">{error}</p>}{notice&&<p className="form-message" role="status">{notice}</p>}</section>{preview&&createPortal(<div className="modal-backdrop glass-modal-backdrop" role="presentation"><section className="mcc-card glass-modal-shell pm-import-preview-modal mcc-wide-modal" role="dialog" aria-modal="true" aria-labelledby="pm-import-preview-title"><div className="modal-heading"><div><p className="eyebrow">Database remains authoritative · preview only</p><h3 id="pm-import-preview-title">PM Excel Import Preview</h3><p>{preview.filename}</p></div><button className="link-button compact-button glass-button glass-button--secondary" type="button" disabled={busy} onClick={()=>setPreview(null)}>Close</button></div><div className="pm-import-summary"><MccSummaryTokenGroup><MccSummaryToken tone="success">{preview.summary.additions} additions</MccSummaryToken><MccSummaryToken>{preview.summary.updates} updates</MccSummaryToken><MccSummaryToken>{preview.summary.historyAdditions} history rows</MccSummaryToken><MccSummaryToken tone="warning">{preview.summary.conflicts} conflicts</MccSummaryToken><MccSummaryToken tone="danger">{preview.summary.rejectedRows} rejected</MccSummaryToken></MccSummaryTokenGroup></div>{([['Additions',preview.additions],['Updates',preview.updates],['PMHistory additions',preview.historyAdditions],['Conflicts',preview.conflicts],['Warnings',preview.warnings],['Rejected rows',preview.rejectedRows]] as Array<[string,PmImportItem[]]>).map(([label,items])=>items.length?<section className="pm-import-preview-group" key={label}><h4>{label} ({items.length})</h4>{items.map((item,index)=><div className="pm-import-preview-row" key={`${item.rowNumber}-${index}`}><span>{previewItemText(item)}</span>{item.code==='DECREASING_METER'&&item.rowNumber&&<div className="pm-import-override-grid"><label className="form-field"><span>Override type</span><select className="glass-input" value={overrides[item.rowNumber]?.type??'replacement'} onChange={event=>setOverrides(current=>({...current,[item.rowNumber!]:{type:event.target.value as 'replacement'|'correction'|'override',reason:current[item.rowNumber!]?.reason??''}}))}><option value="replacement">Meter replacement</option><option value="correction">Meter correction</option><option value="override">Authorized override</option></select></label><label className="form-field"><span>Audit reason *</span><input className="glass-input" value={overrides[item.rowNumber]?.reason??''} onChange={event=>setOverrides(current=>({...current,[item.rowNumber!]:{type:current[item.rowNumber!]?.type??'replacement',reason:event.target.value}}))}/></label></div>}</div>)}</section>:null)}{error&&<p className="form-message error" role="alert">{error}</p>}<p className="pm-history-notice">Confirm Import is explicit and applies only valid, unambiguous rows. Previewing has not changed MCC data.</p><div className="modal-actions glass-modal__actions"><button className="secondary-button glass-button glass-button--secondary" type="button" disabled={busy} onClick={()=>setPreview(null)}>Cancel</button><button className="primary-button glass-button glass-button--success" type="button" disabled={busy||preview.summary.additions+preview.summary.updates+preview.summary.historyAdditions===0||preview.conflicts.some(item=>item.code!=='DECREASING_METER')} onClick={()=>void confirmImport()}>{busy?'Confirming...':'Confirm Import'}</button></div></section></div>,document.body)}</>;
}

function PmCompleteModal({asset,task,apiBase,performedBy,onClose,onSaved}:{asset:AssetIdentity;task:PmTask;apiBase:string;performedBy:string;onClose:()=>void;onSaved:()=>void}) {
  const meter=meterIntervals.has(task.intervalType);const [date,setDate]=useState(localIsoDate(new Date()));const [completedMeter,setCompletedMeter]=useState(task.currentMeter===null?'':String(task.currentMeter));const [workOrderNumber,setWorkOrderNumber]=useState('');const [workOrderPdf,setWorkOrderPdf]=useState<File|null>(null);const [followUpRequired,setFollowUpRequired]=useState<'no'|'yes'>('no');const [followUpReason,setFollowUpReason]=useState('');const [noIssuesFound,setNoIssuesFound]=useState(true);const [notes,setNotes]=useState('');const [overrideType,setOverrideType]=useState<'replacement'|'correction'|'override'>('replacement');const [overrideReason,setOverrideReason]=useState('');const [saving,setSaving]=useState(false);const [error,setError]=useState('');const requestId=useRequestId();
  const numericMeter=completedMeter===''?null:Number(completedMeter);const decreasing=meter&&numericMeter!==null&&Number.isFinite(numericMeter)&&task.currentMeter!==null&&numericMeter<task.currentMeter;
  const defaultNote=noIssuesFound&&!notes.trim()?(task.intervalType==='hourly'&&numericMeter!==null?`PM completed at ${numericMeter.toLocaleString()} machine hours. No issues found.`:task.intervalType==='cycles'&&numericMeter!==null?`PM completed at ${numericMeter.toLocaleString()} cycles. No issues found.`:'PM completed. No issues found.'):'Your Task Note will be stored as entered.';
  async function submit(event:FormEvent){event.preventDefault();const workOrder=workOrderNumber.trim();const notApplicable=/^n\s*\/\s*a$/i.test(workOrder);if(!workOrder){setError('Work Order Number is required. Enter N/A when no work order exists.');return;}if(!notApplicable&&!workOrderPdf){setError('Select the matching work-order PDF for this Work Order Number.');return;}if(workOrderPdf&&(workOrderPdf.type!=='application/pdf'||!workOrderPdf.name.toLowerCase().endsWith('.pdf'))){setError('Work-order attachment must be a PDF file.');return;}if(workOrderPdf&&workOrderPdf.size>12*1024*1024){setError('Work-order PDF must be 12 MB or smaller.');return;}if(followUpRequired==='yes'&&(followUpReason.trim().length<5||!/[A-Za-z0-9]/.test(followUpReason))){setError('Enter a meaningful Follow-up Reason when follow-up is required.');return;}if(!noIssuesFound&&(notes.trim().length<5||!/[A-Za-z0-9]/.test(notes))){setError('Enter a meaningful Task Note when issues were found.');return;}if(decreasing&&(overrideReason.trim().length<5||!/[A-Za-z0-9]/.test(overrideReason))){setError('Enter a reason for the meter replacement, correction, or override.');return;}setSaving(true);setError('');try{const body=new FormData();body.append('completionDate',date);if(numericMeter!==null)body.append('completedMeter',String(numericMeter));body.append('workOrderNumber',workOrder);body.append('followUpRequired',followUpRequired);body.append('followUpReason',followUpRequired==='yes'?followUpReason:'');body.append('noIssuesFound',String(noIssuesFound));body.append('taskNote',notes);if(decreasing){body.append('meterOverrideType',overrideType);body.append('meterOverrideReason',overrideReason);}if(workOrderPdf)body.append('workOrderPdf',workOrderPdf,workOrderPdf.name);await requestJson(`${apiBase}/preventive-maintenance/${task.id}/complete`,{method:'POST',headers:{'Idempotency-Key':requestId.current},body});onSaved();}catch(value){setError((value as Error).message||'PM completion could not be saved.');}finally{setSaving(false);}}
  return createPortal(<div className="modal-backdrop glass-modal-backdrop" role="presentation" onMouseDown={event=>{if(event.target===event.currentTarget&&!saving)onClose();}}><section className="mcc-card glass-modal-shell pm-modal pm-complete-modal mcc-wide-modal" role="dialog" aria-modal="true" aria-labelledby="pm-complete-title"><form onSubmit={submit}><div className="modal-heading"><div><p className="eyebrow">Complete PM · immutable audit record</p><h3 id="pm-complete-title">Complete {safeString(task.title,'PM task')}</h3></div><button className="link-button compact-button glass-button glass-button--secondary" type="button" onClick={onClose} disabled={saving}>Close</button></div><div className="pm-view-grid"><PmValue label="Asset number / name" value={`${asset.assetNumber}${asset.assetName?` — ${asset.assetName}`:''}`}/><PmValue label="PM task" value={task.title}/><PmValue label="Interval type" value={task.intervalLabel}/><PmValue label="Performed by" value={performedBy||'Signed-in maintenance user'}/></div><div className="pm-form-grid"><MccDateInput label="Completion Date *" value={date} onChange={setDate} required />{meter&&<label className="form-field"><span>Current machine {task.intervalType==='hourly'?'hour':'cycle'} reading *</span><input className="glass-input" type="number" min="0" step={task.intervalType==='cycles'?'1':'0.01'} value={completedMeter} onChange={e=>setCompletedMeter(e.target.value)} required /></label>}<label className="form-field"><span>Work Order Number *</span><input className="glass-input" value={workOrderNumber} maxLength={120} onChange={event=>setWorkOrderNumber(event.target.value)} placeholder="Enter work order or N/A" required/><small>Enter N/A only when no work order exists.</small></label><label className="form-field pm-work-order-file"><span>Work-order PDF{!/^n\s*\/\s*a$/i.test(workOrderNumber.trim())?' *':''}</span><input className="glass-input" type="file" accept=".pdf,application/pdf" onChange={event=>setWorkOrderPdf(event.target.files?.[0]??null)} required={Boolean(workOrderNumber.trim())&&!/^n\s*\/\s*a$/i.test(workOrderNumber.trim())}/><small>PDF only, up to 12 MB.</small></label><label className="form-field"><span>Follow-up Required *</span><select className="glass-input" value={followUpRequired} onChange={event=>setFollowUpRequired(event.target.value as 'no'|'yes')} required><option value="no">No</option><option value="yes">Yes</option></select></label>{followUpRequired==='yes'&&<label className="form-field pm-form-wide"><span>Follow-up Reason *</span><textarea className="glass-input" rows={3} maxLength={2000} value={followUpReason} onChange={event=>setFollowUpReason(event.target.value)} required/></label>}<label className="form-field pm-form-wide pm-no-issues-check"><span><input type="checkbox" checked={noIssuesFound} onChange={event=>setNoIssuesFound(event.target.checked)} /> No issues found</span><small>Checked by default. Leave Task Note blank to use the approved completion wording.</small></label><label className="form-field pm-form-wide"><span>Task Note{!noIssuesFound?' *':''}</span><textarea className="glass-input" rows={4} maxLength={12000} value={notes} onChange={e=>setNotes(e.target.value)} required={!noIssuesFound} /></label><p className="pm-history-notice pm-form-wide">{defaultNote}</p>{decreasing&&<div className="pm-meter-override pm-form-wide glass-card glass-card--nested"><strong>Decreasing meter reading requires authorization</strong><p>The stored current reading is {formatMeter(task.currentMeter)}. Confirm the type and record an audit reason.</p><label className="form-field"><span>Override type *</span><select className="glass-input" value={overrideType} onChange={event=>setOverrideType(event.target.value as typeof overrideType)}><option value="replacement">Meter replacement</option><option value="correction">Meter correction</option><option value="override">Authorized override</option></select></label><label className="form-field"><span>Override reason *</span><textarea className="glass-input" rows={3} maxLength={1000} value={overrideReason} onChange={event=>setOverrideReason(event.target.value)} required /></label></div>}</div><p className="pm-history-notice">Confirming stores the current reading or completion date as the new PM baseline; it never resets the physical machine meter.</p>{error&&<p className="form-message error" role="alert">{error}</p>}<div className="modal-actions glass-modal__actions"><button className="secondary-button glass-button glass-button--secondary" type="button" onClick={onClose} disabled={saving}>Cancel</button><button className="primary-button glass-button glass-button--success" type="submit" disabled={saving}>{saving?'Completing PM...':'Confirm Completion'}</button></div></form></section></div>,document.body);
}
