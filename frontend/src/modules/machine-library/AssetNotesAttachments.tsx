  ...
  return <article className={`machine-detail-accordion-card asset-notes-card glass-panel glass-panel--nested${expanded?' is-open':''}`}>
    ...
    <div className="machine-detail-accordion-panel asset-notes-panel" id={`asset-notes-panel-${asset.id}`} aria-hidden={!expanded}>
      ...
      {!adding&&!editing&&notes.length>0&&<div className="asset-note-list">{notes.map(note=><section className="asset-note-entry glass-card glass-card--nested" key={note.id}>
        <div className="asset-note-entry-heading"><div><span className="asset-note-date-pill glass-pill glass-pill--cyan">{formatDate(note.noteDate)}</span><h5>{note.title}</h5><small>Created by {note.createdBy} · {formatDateTime(note.createdAt)}{note.updatedAt!==note.createdAt?' · edited':''}</small></div><div className="asset-note-actions glass-button-group"><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" onClick={()=>setViewer({filename:note.pdfFilename,mimeType:'application/pdf',contentUrl:note.pdfUrl,downloadUrl:note.pdfDownloadUrl,label:`${asset.assetNumber} asset note`})}>Open Note PDF</button><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" onClick={()=>triggerDownload(note.pdfDownloadUrl,note.pdfFilename)}>Download Note PDF</button><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" onClick={()=>window.open(note.pdfUrl,'_blank','noopener,noreferrer')}>Print Note PDF</button>{canEdit&&<><button className="secondary-button compact-button glass-button glass-button--secondary" type="button" onClick={()=>beginEdit(note)}>Edit Note</button><button className="danger-button compact-button glass-button glass-button--danger" type="button" onClick={()=>void deleteNote(note)}>Delete Note</button></>}</div></div>
...
function AttachmentChip({attachment,canDelete,onOpen,onDownload,onDelete}:{attachment:AssetNoteAttachment;canDelete:boolean;onOpen:()=>void;onDownload:()=>void;onDelete:()=>void}) {
  const kind=fileKind(attachment);
  return <div className={`asset-attachment-chip glass-attachment kind-${kind}`}><button className={`asset-file-icon asset-file-icon-button glass-file-icon glass-file-icon--${kind==='word'?'docx':kind}`} type="button" onClick={onOpen} aria-label={`Open ${attachment.filename}`}>{kind==='pdf'?'PDF':kind==='word'?'W':'IMG'}</button><button className="asset-attachment-name glass-attachment__copy" type="button" onClick={onOpen}><strong>{attachment.filename}</strong><small>{fileKindLabel(attachment)}{attachment.fileSize?` · ${formatFileSize(attachment.fileSize)}`:''}</small></button><div className="asset-attachment-actions"><button className="link-button compact-button glass-button glass-button--secondary" type="button" onClick={onDownload}>Download</button>{canDelete&&<button className="link-button compact-button danger-text glass-button glass-button--danger" type="button" onClick={onDelete}>Delete</button>}</div></div>;
}
