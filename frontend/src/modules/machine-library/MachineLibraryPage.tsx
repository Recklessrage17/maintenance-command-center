...
function MachineEditorModal({form,setField,onClose,onSubmit,canEdit,asset,onReplacement,onInspection}:{form:AssetForm;setField:<K extends keyof AssetForm>(key:K,value:AssetForm[K])=>void;onClose:()=>void;onSubmit:(event:FormEvent)=>void;canEdit:boolean;asset:MachineAsset|null;onReplacement:(asset:MachineAsset,field:ReplacementField)=>void;onInspection:()=>void}) {
  ...
  return <div className="modal-backdrop" role="dialog" aria-modal="true"><form className="mcc-card machine-modal" onSubmit={onSubmit}>
    ...
    <section className="machine-form-section"><span>Screw / Barrel</span><div className="machine-screw-barrel-grid">
      <div className="machine-component-box"><h4>Screw Box</h4><Text label="Screw Type" value={form.screwType} set={v=>setField('screwType',v)} disabled={disabled}/><Text label="Screw Tip Type" value={form.screwTipType} set={v=>setField('screwTipType',v)} disabled={disabled}/><Check label="Screw Rebuild / Repaired" checked={form.screwRebuildRepaired} set={setScrewRebuild} disabled={disabled}/><DateWithAge label="Screw Installed Date" value={form.screwInstalledDate} set={v=>setField('screwInstalledDate',v)} disabled={disabled}/><DateWithAge label="Screw Tip Installed Date" value={form.screwTipInstalledDate} set={v=>setField('screwTipInstalledDate',v)} disabled={disabled}/><UnitDimensionField label="Screw Length" value={form.screwLength} set={v=>setField('screwLength',v)} disabled={disabled}/><ConditionBadge label="Screw condition" status={effectiveCondition(form.screwRebuildRepaired, form.screwConditionStatus)} /></div>
      <div className="machine-component-box"><h4>Barrel Box</h4><Text label="Barrel Diameter" value={form.barrelDiameter} set={v=>setField('barrelDiameter',v)} disabled={disabled}/><Check label="Barrel Rebuild / Repaired" checked={form.barrelRebuildRepaired} set={setBarrelRebuild} disabled={disabled}/><DateWithAge label="Barrel Installed Date" value={form.barrelInstalledDate} set={v=>setField('barrelInstalledDate',v)} disabled={disabled}/><DateWithAge label="Barrel End Cap Installed Date" value={form.barrelEndCapInstalledDate} set={v=>setField('barrelEndCapInstalledDate',v)} disabled={disabled}/><UnitDimensionField label="Barrel Length" value={form.barrelLength} set={v=>setField('barrelLength',v)} disabled={disabled}/><ConditionBadge label="Barrel condition" status={effectiveCondition(form.barrelRebuildRepaired, form.barrelConditionStatus)} /></div>
    </div><div className="measurement-inspection-row">{canEdit&&<button className="machine-action-badge measurement-inspection-button" type="button" onClick={onInspection}>Measurement Inspection</button>}<small>Measurement Inspection later will update New to Used to Worn.</small></div></section>
...
function DateWithAge({label,value,set,disabled}:{label:string;value:string;set:(value:string)=>void;disabled:boolean}) {
  ...
  return <label className="form-field"><span>{label}</span><input type={useDatePicker ? 'date' : 'text'} value={useDatePicker ? isoValue : value} disabled={disabled} onChange={event=>set(event.target.value)} placeholder="YYYY-MM-DD or known text" /><small className="machine-age-label">Year count: {ageYears(value)}</small></label>;
}
