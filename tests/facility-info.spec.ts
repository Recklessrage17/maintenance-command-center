...
test('Facility folders, gallery, image viewer, and video controls remain compact and accessible',async({page},testInfo)=>{
  ...
  const electricalToggle=page.getByRole('button',{name:'Close Electrical Prints'});await expect(electricalToggle).toBeVisible();const treeItems=page.locator('.facility-folder-list .mcc-folder-tree-item');await expect(treeItems).toHaveCount(2);const rootTreeItem=treeItems.nth(0);const childTreeItem=treeItems.nth(1);await expect(rootTreeItem).toHaveAttribute('data-depth','0');await expect(rootTreeItem.locator('.mcc-folder-icon')).toHaveAttribute('data-state','open');await expect(rootTreeItem.locator('.mcc-folder-tree-chevron')).toHaveAttribute('data-state','open');await expect(childTreeItem).toHaveAttribute('data-depth','1');const facilityTreeGeometry=await Promise.all([rootTreeItem,childTreeItem].map(item=>item.evaluate(element=>{const card=element.querySelector('.facility-folder-card') as HTMLElement;const guide=getComputedStyle(element,'::before');const elbow=getComputedStyle(element,'::after');return{left:card.getBoundingClientRect().left,paddingLeft:parseFloat(getComputedStyle(element).paddingLeft),guide:guide.backgroundImage,elbow:elbow.borderTopStyle};})));expect(facilityTreeGeometry[1].left).toBeGreaterThan(facilityTreeGeometry[0].left);expect(facilityTreeGeometry[1].guide).not.toBe('none');expect(facilityTreeGeometry[1].elbow).toBe('solid');expect(facilityTreeGeometry[1].paddingLeft).toBeLessThanOrEqual(testInfo.project.name==='mobile-chromium'?72:108);await page.getByRole('button',{name:'Open Panels'}).click();await expect(childTreeItem.locator('.mcc-folder-icon')).toHaveAttribute('data-state','open');await expect(childTreeItem.locator('.mcc-folder-tree-chevron')).toHaveAttribute('data-state','open');
  const panelRow=page.locator('.facility-item-row').filter({hasText:'Panel Photo.png'});await expect(panelRow).toBeVisible();await expect(panelRow.locator('.mcc-library-file-thumbnail')).toHaveCount(1);await expect(panelRow.locator('.mcc-file-type-icon--image')).toHaveCount(0);await expect(panelRow.getByLabel('Folder path: Production > Electrical Prints > Panels')).toHaveAttribute('title','Production > Electrical Prints > Panels');await expect(panelRow.locator('.mcc-resource-row__actions > .mcc-action-group > button,.mcc-resource-row__actions > .mcc-action-group > .mcc-overflow-menu')).toHaveCount(3);
  ...
  const videoRow=page.locator('.facility-item-row').filter({hasText:'Panel Walkthrough.mp4'});await expect(videoRow.locator('.mcc-file-type-icon--video')).toHaveCount(1);await expect(videoRow.getByLabel('Folder path: Production > Electrical Prints > Panels')).toBeVisible();await page.getByRole('button',{name:'Gallery'}).click();const videoCard=page.getByRole('button',{name:/Panel Walkthrough.mp4/});await expect(videoCard).toBeVisible();await expect(videoCard.getByLabel('Folder path: Production > Electrical Prints > Panels')).toBeVisible();await videoCard.click();const videoViewer=page.locator('.facility-viewer');const video=videoViewer.locator('video');await expect(video).toBeVisible();await expect(video).toHaveAttribute('controls','');await expect(videoViewer.getByRole('button',{name:'Print'})).toHaveCount(0);await expect(videoViewer).toHaveAttribute('aria-label','Panel Walkthrough.mp4');await video.focus();await expect(video).toBeFocused();await page.keyboard.press('ArrowRight');await expect(videoViewer).toBeVisible();await expect(videoViewer).toHaveAttribute('aria-label','Panel Walkthrough.mp4');await expect(videoViewer).not.toContainText('Backup Walkthrough.mp4');await page.keyboard.press('Escape');await expect(videoViewer).toHaveCount(0);
  const dimensions=await page.evaluate(()=>({scroll:document.documentElement.scrollWidth,client:document.documentElement.clientWidth}));expect(dimensions.scroll).toBeLessThanOrEqual(dimensions.client);
});

test('Issue #114 Facility Library renders image thumbnails and keeps non-image icons without video thumbnail requests',async({page})=>{
  let videoContentRequests=0;
  page.on('request',request=>{if(request.url().endsWith('/api/facility-info/items/43/content'))videoContentRequests+=1;});
  ...
  const imageRow=page.locator('.facility-item-row',{hasText:'Panel Photo.png'});
  const image=imageRow.locator('.mcc-library-file-thumbnail');
  await expect(image).toHaveAttribute('src','/api/facility-info/items/42/content');
  await expect.poll(()=>image.evaluate(element=>(element as HTMLImageElement).naturalWidth)).toBeGreaterThan(0);
  await expect(imageRow.locator('.mcc-file-type-icon')).toHaveCount(0);
  const pdfRow=page.locator('.facility-item-row',{hasText:'Panel Schedule.pdf'});
  await expect(pdfRow.locator('.mcc-file-type-icon--pdf')).toHaveCount(1);
  await expect(pdfRow.locator('.mcc-library-file-thumbnail')).toHaveCount(0);
  const videoRow=page.locator('.facility-item-row',{hasText:'Panel Walkthrough.mp4'});
  ...
  expect(videoContentRequests).toBe(0);
});

test('Facility uses the shared compact upload modal with folder export and accessible responsive controls',async({page},testInfo)=>{
  await mockFacility(page);await page.goto('/facility-info');await page.getByRole('button',{name:'Open Production'}).click();
  const folderCard=page.locator('.facility-folder-card').filter({hasText:'Electrical Prints'}).first();const manage=folderCard.getByRole('button',{name:'Manage'});await manage.click();await expect(page.getByRole('menuitem',{name:'Export Folder ZIP'})).toBeVisible();await page.keyboard.press('Escape');
  const uploadTrigger=folderCard.getByRole('button',{name:'Upload',exact:true});await uploadTrigger.click();const modal=page.getByRole('dialog',{name:'Upload Files'});await expect(modal).toBeVisible();await expect(modal.getByRole('navigation',{name:'Upload destination'})).toContainText('Production');await expect(modal.getByRole('navigation',{name:'Upload destination'})).toContainText('Electrical Prints');await expect(modal).toContainText('General files up to 50 MB');await expect(modal).toContainText('videos up to 500 MB');await expect(modal.getByRole('button',{name:'Choose Files'})).toBeVisible();await expect(modal.getByRole('button',{name:'Upload Folder'})).toBeVisible();await expect(modal.getByRole('button',{name:'Upload Files',exact:true})).toBeVisible();
  await modal.getByLabel('Choose files').setInputFiles([{name:'Inspection.txt',mimeType:'text/plain',buffer:Buffer.from('inspection')},{name:'Panel.png',mimeType:'image/png',buffer:Buffer.from([0x89,0x50,0x4e,0x47,0x0d,0x0a,0x1a,0x0a])}]);await expect(modal.getByText('2 files')).toBeVisible();await expect(modal.getByText('Inspection.txt')).toBeVisible();await expect(modal.getByText('Document',{exact:true})).toBeVisible();await expect(modal.getByText('Picture',{exact:true})).toBeVisible();await modal.getByLabel('Choose folder').evaluate((element:HTMLInputElement)=>{const file=new File(['nested'],'Nested.txt',{type:'text/plain'});Object.defineProperty(file,'webkitRelativePath',{value:'Vendor Set/Sub/Nested.txt'});const transfer=new DataTransfer();transfer.items.add(file);Object.defineProperty(element,'files',{configurable:true,value:transfer.files});element.dispatchEvent(new Event('change',{bubbles:true}));});await expect(modal.getByText('Vendor Set/Sub')).toBeVisible();await expect(modal.getByText('1 file')).toBeVisible();await expect(modal.getByText('Inspection.txt')).toHaveCount(0);await expect(modal.getByLabel('Description')).toBeVisible();await expect(modal.getByLabel('Caption')).toBeVisible();await expect(modal.getByLabel('Revision')).toBeVisible();await expect(modal.getByLabel('Date')).toBeVisible();
  const layout=await modal.evaluate(element=>{const rect=element.getBoundingClientRect();const queue=element.querySelector<HTMLElement>('.library-upload-queue')!;const row=element.querySelector<HTMLElement>('.library-upload-row')!;const remove=row.querySelector<HTMLElement>('.library-upload-row__remove')!;const main=row.querySelector<HTMLElement>('.library-upload-row__main')!;const description=element.querySelector<HTMLElement>('.library-upload-metadata-wide')!;return {left:rect.left,right:rect.right,bottom:rect.bottom,width:innerWidth,height:innerHeight,queueOverflow:getComputedStyle(queue).overflowY,queueMaxHeight:getComputedStyle(queue).maxHeight,removeLeft:remove.getBoundingClientRect().left,mainRight:main.getBoundingClientRect().right,descriptionColumn:getComputedStyle(description).gridColumn};});expect(layout.left).toBeGreaterThanOrEqual(0);expect(layout.right).toBeLessThanOrEqual(layout.width);expect(layout.bottom).toBeLessThanOrEqual(layout.height);expect(layout.queueOverflow).toBe('auto');expect(layout.queueMaxHeight).not.toBe('none');expect(layout.removeLeft).toBeGreaterThanOrEqual(layout.mainRight);expect(layout.descriptionColumn).toMatch(/1 \/ -1|1 \/ 4|auto/);
  await page.emulateMedia({reducedMotion:'reduce'});await expect(modal.getByRole('button',{name:'Remove Nested.txt'})).toHaveCSS('transition-duration','0s');await page.keyboard.press('Escape');await expect(modal).toHaveCount(0);await expect(uploadTrigger).toBeFocused();
  if(testInfo.project.name==='mobile-chromium')expect(await page.evaluate(()=>document.documentElement.scrollWidth-document.documentElement.clientWidth)).toBeLessThanOrEqual(1);
});

test('Facility folder import immediately reconciles nested folders, uploaded files, and summary counts',async({page})=>{
  const liveFolders=folders.map(folder=>({...folder}));const liveItems=items.map(item=>({...item}));let nextFolderId=100;let nextItemId=200;
  const liveArea=()=>({...area,summary:{folderCount:liveFolders.length,documentCount:liveItems.filter(item=>item.mediaType==='document').length,pictureCount:liveItems.filter(item=>item.mediaType==='picture').length,videoCount:liveItems.filter(item=>item.mediaType==='video').length}});
  await mockFacility(page);await page.route(/\/api\/facility-info$/,route=>route.fulfill({json:{ok:true,areas:[liveArea(),secondArea],limits:{documentsMb:50,picturesMb:50,videosMb:500}}}));await page.route(/\/api\/facility-info\/areas\/21$/,route=>route.fulfill({json:{ok:true,area:liveArea(),folders:liveFolders,items:liveItems}}));
  await page.route(/\/api\/facility-info\/areas\/21\/folders$/,async route=>{const body=route.request().postDataJSON() as {name:string;parentId:number|null};const parentId=body.parentId??null;const existing=liveFolders.find(folder=>(folder.parentId??null)===parentId&&folder.name.toLowerCase()===body.name.trim().toLowerCase());if(existing)return route.fulfill({status:400,json:{ok:false,error:'A folder with this name already exists in this location.'}});const parent=parentId===null?null:liveFolders.find(folder=>folder.id===parentId);const folder={id:nextFolderId++,areaId:21,parentId,name:body.name.trim(),description:'',path:parent?`${parent.path} / ${body.name.trim()}`:body.name.trim(),itemCount:0,childCount:0,createdAt:new Date().toISOString(),updatedAt:new Date().toISOString()};liveFolders.push(folder);for(const item of liveFolders)item.childCount=liveFolders.filter(child=>child.parentId===item.id).length;await route.fulfill({status:201,json:{ok:true,folder}});});
  await page.route(/\/api\/facility-info\/areas\/21\/folders\/(\d+)\/items$/,async route=>{const folderId=Number(new URL(route.request().url()).pathname.split('/').at(-2));const body=route.request().postData()??'';const filename=/filename="([^"]+)"/.exec(body)?.[1]??'Uploaded.txt';const folder=liveFolders.find(item=>item.id===folderId)!;const item={...baseItem,id:nextItemId++,folderId,folderName:folder.name,folderPath:folder.path,mediaType:'document' as const,originalFilename:filename,displayFilename:filename,extension:'.txt',mimeType:'text/plain',sizeBytes:13,contentUrl:`/api/facility-info/items/${nextItemId}/content`,downloadUrl:`/api/facility-info/items/${nextItemId}/download`,canPrint:false};liveItems.push(item);folder.itemCount=liveItems.filter(entry=>entry.folderId===folderId).length;await route.fulfill({status:201,json:{ok:true,items:[item]}});});
  await page.goto('/facility-info');await page.getByRole('button',{name:'Open Production'}).click();await page.getByRole('button',{name:'Upload Folder',exact:true}).click();const modal=page.getByRole('dialog',{name:'Upload Files'});await modal.getByLabel('Choose folder').evaluate((element:HTMLInputElement)=>{const transfer=new DataTransfer();for(const [relativePath,name] of [['Facility Vendor/Root.txt','Root.txt'],['Facility Vendor/Manuals/Guide.txt','Guide.txt'],['Facility Vendor/Manuals/Electrical/Panel.txt','Panel.txt']]){const file=new File(['facility data'],name,{type:'text/plain'});Object.defineProperty(file,'webkitRelativePath',{value:relativePath});transfer.items.add(file);}Object.defineProperty(element,'files',{configurable:true,value:transfer.files});element.dispatchEvent(new Event('change',{bubbles:true}));});await modal.getByRole('button',{name:'Upload Files',exact:true}).click();await expect(modal).toHaveCount(0);for(const folderName of ['Facility Vendor','Manuals','Electrical'])await expect(page.locator('.facility-folder-card').filter({hasText:folderName}).first()).toBeVisible();for(const filename of ['Root.txt','Guide.txt','Panel.txt'])await expect(page.locator('.facility-item-row').filter({hasText:filename})).toBeVisible();const heading=page.locator('.facility-area-heading');await expect(heading).toContainText('5 folders');await expect(heading).toContainText('4 documents');await expect(page.getByText(/already exists in this location/i)).toHaveCount(0);
});

test('Global Facility search exposes Facility and folder context and opens the matching item',async({page})=>{
  await mockFacility(page);await page.goto('/facility-info');const search=page.getByPlaceholder('Facility, folder, filename, caption, revision, uploader');await search.fill('panel');await expect(page.getByRole('heading',{name:'3 results'})).toBeVisible();const result=page.locator('.facility-item-row').filter({hasText:'Panel Photo.png'});await expect(result.getByLabel('Folder path: Production > Electrical Prints > Panels')).toBeVisible();await result.getByRole('button',{name:'Open'}).click();await expect(page.locator('.facility-viewer')).toBeVisible();await expect(page.locator('.facility-viewer')).toContainText('Production / Electrical Prints / Panels');
});
