...
test('Facility uses the shared compact upload modal with folder export and accessible responsive controls',async({page},testInfo)=>{
  ...
  const uploadTrigger=folderCard.getByRole('button',{name:'Upload',exact:true});await uploadTrigger.click();const modal=page.getByRole('dialog',{name:'Upload Files'});await expect(modal).toBeVisible();await expect(modal.getByRole('navigation',{name:'Upload destination'})).toContainText('Production');await expect(modal.getByRole('navigation',{name:'Upload destination'})).toContainText('Electrical Prints');await expect(modal).toContainText('Documents up to 50 MB');await expect(modal).toContainText('videos up to 500 MB');await expect(modal.getByRole('button',{name:'Choose Files'})).toBeVisible();await expect(modal.getByRole('button',{name:'Upload Folder'})).toBeVisible();await expect(modal.getByRole('button',{name:'Upload Files',exact:true})).toBeVisible();
