...
test('asset libraries use compact shared rows and page titles hug their content',async({page},testInfo)=>{
  ...
  const topbar=page.locator('.mcc-page-topbar');const machineRows=page.locator('.machine-asset-card');await expect(machineRows).toHaveCount(2);await expect(machineRows.first()).not.toContainText('Type / Brand');await expect(machineRows.first()).not.toContainText('Barrel & Screw Logs');await expect(machineRows.first()).not.toContainText('History Preview');await expect(machineRows.first()).toContainText('Year / Age');await expect(machineRows.first()).toContainText('Barrel Size');await expect(machineRows.first()).toContainText('Model');await expect(machineRows.first()).toContainText('Serial #');await expect(machineRows.first()).toContainText('Active');await expect(machineRows.first()).toContainText('PM: Current');
  ...
  await page.getByRole('button',{name:'Open command menu'}).click();await page.getByRole('button',{name:/Equipment Library/}).click();await expect(page.getByRole('heading',{name:'Equipment Library'})).toBeVisible();const equipmentRows=page.locator('.equipment-asset-card');await expect(equipmentRows).toHaveCount(2);await expect(equipmentRows.first()).not.toContainText('Newest History');await expect(equipmentRows.first()).toContainText('Type / Category');await expect(equipmentRows.first()).toContainText('Brand / Model');await expect(equipmentRows.first()).toContainText('Serial #');await expect(equipmentRows.first()).toContainText('Year / Age');await expect(equipmentRows.first()).toContainText('Active');
