...
assert.match(installedTest, /Invoke-MccHttpJsonProbe[\s\S]*managed-readiness[\s\S]*AllowedFailureStatusCodes @\(404\)[\s\S]*RequireFailureJson/);
assert.match(installedTest, /Assert-MccManagedReadinessPayload/);
assert.match(installedTest, /verificationContext = 'InstalledValidator'/);
assert.match(installedTest, /Assert-MccLegacyManagedRuntimeEvidence/);
assert.match(installedTest, /Test-MccScheduledTaskIdentity/);
assert.match(installedTest, /updateStatusUnauthorized/);
