...
test('surfaces visually distinct Machine and Equipment Tech Notes with accessible viewers and reduced motion',async({page},testInfo)=>{
  ...
  await expect(machinePill).toContainText('Open 2');await expect(equipmentPill).toContainText('Open 1');await expect(machinePill).toHaveCSS('color','rgb(99, 239, 184)');await expect(equipmentPill).toHaveCSS('color','rgb(99, 239, 184)');await expect(machine.locator('.dashboard-pm-section-counts')).toHaveCount(0);await expect(machine.getByRole('group',{name:'Work order counts for Press 900'})).toHaveText('WO(Open 2/Hold 0)');await expect(machine.getByRole('button',{name:'Open 0 Hold work orders for Press 900'})).toBeDisabled();
