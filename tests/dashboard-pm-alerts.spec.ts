...
test('surfaces visually distinct Machine and Equipment Tech Notes with accessible viewers and reduced motion',async({page},testInfo)=>{
  ...
  await expect(machinePill).toContainText('Tech Note 2');await expect(equipmentPill).toContainText('Tech Note 1');await expect(machinePill).toHaveCSS('color','rgb(238, 233, 255)');await expect(machinePill).toHaveCSS('animation-name','dashboard-tech-note-pulse');await expect(machinePill).toHaveCSS('animation-duration','2.2s');await expect(machine.locator('.dashboard-tech-note-summary')).toHaveCSS('border-color','rgba(167, 139, 250, 0.58)');expect(await machinePill.evaluate(element=>getComputedStyle(element,'::after').animationName)).toBe('dashboard-tech-note-shine');
