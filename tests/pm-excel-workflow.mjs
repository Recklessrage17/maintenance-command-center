...
try {
  ...
  assert.equal(insertedTracker.getCell('G10').value,'New guarded inspection');assert.equal(insertedTracker.getCell('A10').value,'Days');assert.equal(insertedTracker.getCell('B10').value,60);assert.equal(insertedTracker.getCell('C10').value.toISOString().slice(0,10),'2026-08-01');assert.equal(insertedTracker.getCell('D10').result.toISOString().slice(0,10),'2026-08-06');assert.equal(insertedTracker.getCell('D10').formula,'DATE(2026,7,20)','workbook-managed current-date formula must remain intact');
