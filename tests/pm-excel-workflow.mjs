...
try {
  ...
  for(let row=10;row<=12;row+=1)for(let column=11;column<=20;column+=1)assert.equal(changedHistory.getCell(row,column).value,null,`new PMHistory row ${row} must remain formula-free beyond the supported A:J contract`);
  ...
  await assert.rejects(()=>synchronizePmWorkbook({sourcePath:destination,destinationPath:destination,backupDir:backups,trackerUpdates:[{...repeatedUpdate,currentMeter:3610,remaining:3150}],historyRows:[],onStage(stage){if(stage==='verifying')throw new Error('simulated promoted-workbook verification failure');}}),/promoted-workbook verification failure/);
  assert.equal(sha(destination),beforeFailure,'failed verification after promotion must restore the previous active workbook');

  const compactSource=path.join(temporaryRoot,'compact-source.xlsx');const compactDestination=path.join(temporaryRoot,'compact-destination.xlsx');const compact=new ExcelJS.Workbook();const compactTracker=compact.addWorksheet('Machine Pm Tracker');compactTracker.getRow(1).values=['Press:','M-300'];compactTracker.getRow(2).values=['Interval Type','Interval Cycles','Last Completed Date / Last hourly','Today Date / Hourly','Due Date','Status','Task Description'];compactTracker.getRow(3).values=['Hourly',100,10,20,{formula:'C3+B3',result:110},'Current','Compact task'];const compactHistory=compact.addWorksheet('PMHistory');const compactHeaders=['AssetNo','Work order #','Task Status','Start Date','End Date','Work Order Type','Perform By:','Interval Type','Task Type','Task Note',...Array.from({length:10},(_,index)=>`Helper ${index+1}`)];const compactHelpers=Array.from({length:10},(_,index)=>({formula:index===0?'A3&"-"&B3':`K3&"-H${index+1}"`,result:`COMPACT-H${index+1}`}));compactHistory.addTable({name:'CompactHistory',ref:'A2',headerRow:true,totalsRow:false,columns:compactHeaders.map(name=>({name})),rows:[['M-300','WO-COMPACT-SHARED','Completed',new Date('2026-01-01T12:00:00Z'),new Date('2026-01-01T12:00:00Z'),'Preventive Maintenance','Sanitized Technician','Hourly','Compact task','Completed.',...compactHelpers]]});await compact.xlsx.writeFile(compactSource);const compactAppend={assetNumber:'M-300',workOrderNumber:'WO-COMPACT-SHARED',taskStatus:'Completed',startDate:'2026-01-02',completionDate:'2026-01-02',workOrderType:'Preventive Maintenance',performedBy:'Sanitized Technician',intervalType:'hourly',taskType:'Compact second task',taskNote:'Completed second task.'};result=await synchronizePmWorkbook({sourcePath:compactSource,destinationPath:compactDestination,backupDir:backups,trackerUpdates:[],historyRows:[compactAppend]});assert.equal(result.appendedHistory,1);const compactChanged=new ExcelJS.Workbook();await compactChanged.xlsx.readFile(compactDestination);const compactChangedHistory=compactChanged.getWorksheet('PMHistory');const compactTable=compactChangedHistory.getTable('CompactHistory');assert.equal(compactTable.ref??compactTable.table?.tableRef,'A2:T4','a legacy PMHistory table may retain its historical width');assert.equal(compactChangedHistory.getCell('B4').value,'WO-COMPACT-SHARED','same work order with a different task must remain exportable');for(let column=11;column<=20;column+=1)assert.equal(compactChangedHistory.getCell(4,column).value,null,'MCC must not recreate legacy Helper 1-10 formulas on appended rows');

  const simpleSource=path.join(temporaryRoot,'simple-aj-source.xlsx');const simpleDestination=path.join(temporaryRoot,'simple-aj-destination.xlsx');const simple=new ExcelJS.Workbook();const simpleTracker=simple.addWorksheet('Machine Pm Tracker');simpleTracker.getRow(1).values=['Press:','M-300'];simpleTracker.getRow(2).values=['Interval Type','Interval Cycles','Last Completed Date / Last hourly','Today Date / Hourly','Due Date','Status','Task Description'];simpleTracker.getRow(3).values=['Hourly',100,10,20,{formula:'C3+B3',result:110},'Current','Compact task'];const simpleHistory=simple.addWorksheet('PMHistory');simpleHistory.addTable({name:'SimpleHistory',ref:'A2',headerRow:true,totalsRow:false,columns:compactHeaders.slice(0,10).map(name=>({name})),rows:[['M-300','WO-SIMPLE-1','Completed',new Date('2026-01-01T12:00:00Z'),new Date('2026-01-01T12:00:00Z'),'Preventive Maintenance','Sanitized Technician','Hourly','Compact task','Completed.']]});await simple.xlsx.writeFile(simpleSource);result=await synchronizePmWorkbook({sourcePath:simpleSource,destinationPath:simpleDestination,backupDir:backups,trackerUpdates:[],historyRows:[compactAppend]});const simpleChanged=new ExcelJS.Workbook();await simpleChanged.xlsx.readFile(simpleDestination);const simpleChangedHistory=simpleChanged.getWorksheet('PMHistory');assert.equal(simpleChangedHistory.getTable('SimpleHistory').table.tableRef,'A2:J4','the official simplified PMHistory table must remain A:J');assert.equal(simpleChangedHistory.columnCount,10,'synchronization must not add legacy Helper columns');assert.equal(simpleChangedHistory.getCell('K4').value,null);

  console.log(`PM Excel workflow tests passed on ${process.platform}: grouped sections, repeated headers, real aliases, Annual 365, repeated work orders, exact duplicate prevention, formula-free PMHistory A:J compatibility, targeted preservation, backups, and atomic failure handling.`);
} finally {
