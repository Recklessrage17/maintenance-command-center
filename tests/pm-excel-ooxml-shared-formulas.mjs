...
try{
  ...
  assert.match(formulaXml(cellXml(sourceXml,'E107')),/^<f t="shared" ref="E107:E112" si="0">/,'fixture must contain the real boundary shared due-formula master');
  assert.match(formulaXml(cellXml(sourceXml,'F107')),/^<f t="shared" ref="F107:F112" si="1">/,'fixture must contain the real boundary shared status-formula master');
  ...
  assert.deepEqual(await validatePmWorkbookOoxml(sourceBuffer),{worksheets:3,formulaCells:21});
  const malformedZip=await JSZip.loadAsync(sourceBuffer);malformedZip.file('xl/worksheets/sheet1.xml',sourceXml.replace('E107&lt;0','E107<0'));const malformedBuffer=Buffer.from(await malformedZip.generateAsync({type:'nodebuffer'}));await assert.rejects(()=>validatePmWorkbookOoxml(malformedBuffer),/not well-formed XML/i);
  ...
  assert.doesNotMatch(dueCell,/\bt="(?:shared|e)"|\bsi=|\bref=|#N\/A|#REF!/);assert.doesNotMatch(statusCell,/\bt="(?:shared|e)"|\bsi=|\bref=|#N\/A|#REF!/);assert.match(dueCell,/<v>188<\/v>/);assert.match(statusCell,/\bt="str"/);assert.match(statusCell,/<v>OK<\/v>/);assert.doesNotMatch(outputXml,/#REF!|<v>#N\/A<\/v>/);
  assert.equal(formulaXml(cellXml(outputXml,'E112')),'<f t="shared" si="0"/>','existing preceding due formula must remain in its valid shared group');assert.equal(formulaXml(cellXml(outputXml,'F112')),'<f t="shared" si="1"/>','existing preceding status formula must remain in its valid shared group');assert.match(formulaXml(cellXml(outputXml,'E107')),/ref="E107:E112"/);assert.match(formulaXml(cellXml(outputXml,'F107')),/ref="F107:F112"/);
  ...
  assert.equal(tracker.getCell('E113').formula,dueFormula);assert.equal(tracker.getCell('E113').result,188);assert.equal(tracker.getCell('F113').formula,statusFormula);assert.equal(tracker.getCell('F113').result,'OK');assert.equal(tracker.getCell('E112').formula,'IF(OR(B112="",C112="",D112=""),"",B112-(D112-C112))');assert.equal(tracker.getCell('F112').formula,'IF(E112="","Needs Date",IF(E112<0,"Past Due",IF(E112=0,"Due Today",IF(E112<=7,"Due Soon","OK"))))');
  assert.equal(tracker.getCell('H113').formula,'IF("B112"="B112",SUM(B108:B113)+\'Reference Only\'!$B$3,0)+$D113+E$6','true A1 references and ranges must translate while strings, sheet qualifiers, absolute rows, and absolute columns remain intact');
  assert.equal(tracker.getTable('SanitizedPress23').table.tableRef,'A106:G113');assert.deepEqual(tracker.getCell('F113').dataValidation,tracker.getCell('F112').dataValidation);assert.ok(tracker.conditionalFormattings.some(item=>String(item.ref).split(/\s+/).includes('F107:F113')));assert.equal(tracker.pageSetup.printArea,'A1:G113');
  ...
  console.log('Remaining shared formula refs: E107:E112, F107:F112. E113/F113 shared attributes: none. Direct OOXML validation: passed.');
