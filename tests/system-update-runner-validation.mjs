...
assert.match(windowsProductionRunner, /Invoke-RestMethod[\s\S]*-Uri \$healthUri/);
assert.match(windowsProductionRunner, /AllowLegacyHealthPayload[\s\S]*\/api\/health/);
...
assert.match(windowsProductionRunner, /systemUpdate\.configured[\s\S]*systemUpdate\.enabled[\s\S]*applicationMatchesConfiguration/);
